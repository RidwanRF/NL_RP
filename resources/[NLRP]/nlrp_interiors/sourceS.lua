...
exports.nlrp_admin:addAdminCommand("setinteriorflag", 9, "Interior jelölésének módosítása (statikus/dinamikus)")
	...
	function (thePlayer, commandName, interiorId, flag)
		if getElementData(thePlayer, "acc.adminLevel") >= 9 then
			if not tonumber(interiorId) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. commandName .. " [Interior ID] [< static | dynamic >]", thePlayer, 0, 0, 0, true)
				outputChatBox("#b7ff00>> Interior: #ffffffStatic: #ffff99fix interior (nem törölhető) #ffffff| Dynamic: #ffff99törölhető interior", thePlayer, 0, 0, 0, true)
			else
	...
	function (interiorId, colShapeType, data, adminWarp)
		if isElement(source) and availableInteriors[interiorId] then
			if availableInteriors[interiorId].locked == "N" or adminWarp then
				...
				exports.nlrp_hud:showAlert(source, "error", "Az ingatlan ajtaja zárva van.")
						...
						for k, v in ipairs(getElementsByType("player")) do
							if isElement(v) then
								if availableInteriors[interiorId].ownerId == getElementData(v, "char.ID") then
									exports.nlrp_inventory:removeItemByData(playerSource, 1, "data1", interiorId)
									break
								end
							end
						end
...
addEventHandler("resetInterior", getRootElement(),
	function (interiorId)
		if interiorId and availableInteriors[interiorId] then
			exports.nlrp_inventory:removeItemFromCharacter(availableInteriors[interiorId].ownerId, 1, "data1", interiorId)
				...
				dbQuery(
					function (qh, sourceElement)
						...
						if isElement(sourceElement) then
							exports.nlrp_core:takeMoney(sourceElement, availableInteriors[interiorId].price, "buyInterior")

							exports.nlrp_hud:showAlert(sourceElement, "success", "Sikeresen megvásároltad a kiválasztott ingatlant.")

							if not exports.nlrp_inventory:hasItemWithData(sourceElement, 1, "data1", interiorId) then
								exports.nlrp_inventory:giveItem(sourceElement, 1, 1, interiorId)
							end
						end
				...
				if canRent then
					...
					dbQuery(
						function (qh, sourceElement)
							...
							if isElement(sourceElement) then
								exports.nlrp_core:takeMoney(sourceElement, availableInteriors[interiorId].price * 5, "rentInterior")

								exports.nlrp_hud:showAlert(sourceElement, "success", "Sikeresen kibérelted a kiválasztott ingatlant.")

								if not exports.nlrp_inventory:hasItemWithData(sourceElement, 1, "data1", interiorId, "rent") then
									exports.nlrp_inventory:giveItem(sourceElement, 1, 1, interiorId, "rent")
								end
							end
					...
					exports.nlrp_hud:showAlert(source, "warning", "Már rendelkezel egy kibérelt ingatlannal! (ID: " .. rentedInterior .. ")")
				end
			end
		end
	end
		...
		if isElement(source) then
			...
			if characterId then
				...
				if availableInteriors[interiorId].renewalTime - dayTimeDuration >= currentTime.timestamp then
					exports.nlrp_hud:showAlert(source, "warning", "Még nem tudod megújítani az albérletet. (Maximum 24 órával a lejárta előtt lehet)")
				else
					...
					dbQuery(
						function (qh, sourceElement)
							...
							if isElement(sourceElement) then
								exports.nlrp_core:takeMoney(sourceElement, availableInteriors[interiorId].price, "renewalRent")

								exports.nlrp_hud:showAlert(sourceElement, "success", "Sikeresen megújítottad az albérletet.")
...
addEventHandler("unRentInterior", getRootElement(),
	function (interiorId)
		if isElement(source) then
			...
			if characterId then
				exports.nlrp_inventory:removeItemFromCharacter(availableInteriors[interiorId].ownerId, 1, "data1", interiorId)
				exports.nlrp_core:giveMoney(source, availableInteriors[interiorId].price * 4)

				resetInterior(interiorId)
				triggerClientEvent("resetInterior", resourceRoot, interiorId)

				exports.nlrp_hud:showAlert(source, "success", "Sikeresen felmondtad az ingatlan bérlését!")
			end
		end
	end
)
		...
		if isElement(source) then
			...
			if characterId and availableInteriors[interiorId] then
				...
				if availableInteriors[interiorId].type == "building" and availableInteriors[interiorId].ownerId == characterId then
					havePermission = true
				elseif exports.nlrp_inventory:hasItemWithData(source, 1, "data1", interiorId) then
					havePermission = true
				...
				if havePermission then
					...
					if locked == "N" then
						exports.nlrp_hud:showAlert(source, "info", "Sikeresen kinyitottad az ingatlan ajtaját.")
					else
						exports.nlrp_hud:showAlert(source, "info", "Sikeresen bezártad az ingatlan ajtaját.")
					end
					...
					exports.nlrp_hud:showAlert(source, "error", "Nincs kulcsod ehhez az ingatlanhoz.")
				end
			end
		end
	end
...
function resetInterior(interiorId)
	if availableInteriors[interiorId] then
		dbExec(connection, "UPDATE interiors SET ownerId = '0',  announced = '0', renewalTime = '0' WHERE interiorId = ?; DELETE FROM items WHERE itemId = '1' AND data1 = ? AND ownerId = ?", interiorId, interiorId, availableInteriors[interiorId].ownerId)
		...
		if v.ownerId > 0 and v.renewalTime > 0 then
			...
			if currentTime >= v.renewalTime then
				...
				if isElement(playerSource) then
					exports.nlrp_hud:showAlert(playerSource, "error", "Albérlet", "Nem fizetted ki az albérleted, ezért lejárt!")

					exports.nlrp_inventory:removeItemFromCharacter(v.ownerId, 1, "data1", k)
				end
			elseif v.renewalTime - dayTimeDuration <= currentTime then
				if isElement(playerSource) then
					...
					exports.nlrp_hud:showAlert(playerSource, "warning", "Hamarosan lejár az albérleted, részletek a chatboxban!")
...
addEvent("updateAnnounce",true)
addEventHandler("updateAnnounce",resourceRoot,
	function(dbid,state)
		availableInteriors[dbid].announced = state
		dbExec(connection,"UPDATE interiors SET announced=? WHERE interiorId=?",state,dbid)
	end
)

addCommandHandler("checkinteriors",
	function(player,cmd)
		if getElementData(player,"acc.adminLevel") >= 6 then
			local announced = 0
			local un_accounced = 0
			for dbid,v in pairs(availableInteriors) do
				if v.announced == 1 then announced = announced + 1 end
				if v.announced == 0 then un_accounced = un_accounced + 1 end
			end
			outputChatBox("#b7ff00 >> NexLevel:#ffffff Interiorok állapota:",player,0,0,0,true)
			outputChatBox("#ffffff -- Bejelentve: #b7ff00" .. announced .. " db",player,0,0,0,true)
			outputChatBox("#ffffff -- Nincs bejelentve: #b7ff00" .. un_accounced .. " db",player,0,0,0,true)
		end
	end
)

--[[addCommandHandler("csuhy",
	function(player,cmd)
		if getElementData(player,"acc.adminLevel") >= 9 then
			local un_accounced = 0
			local reseted = 0
			for dbid,v in pairs(availableInteriors) do
				if v.announced == 1 then
					triggerEvent("updateAnnounce",resourceRoot,dbid,0)
					un_accounced = un_accounced + 1
				elseif v.announced == 0 then
					resetInterior(dbid)
					triggerClientEvent("resetInterior",resourceRoot,dbid)
			...
			outputChatBox("#b7ff00 >> NexLevel:",player,0,0,0,true)
			outputChatBox("#ffffff -- Bejelentés levéve: #b7ff00" .. un_accounced .. " db#ffffff interior-ról!",player,0,0,0,true)
			outputChatBox("#ffffff -- Interior reset-elve: #b7ff00" .. reseted .. " db",player,0,0,0,true)
			exports.nlrp_core:sendMessageToAdmins(adminName .. " resetelt " .. reseted .. " db interiort!")
		end
	end
)]]--