...
--// Csuhy-kám ezt remélem meg fogod majd találni <3
local lehetebejelenteni = false
...
local responsiveMultipler = exports.nlrp_hud:getResponsiveMultipler()
		...
		if interiorDatas then
			if interiorDatas.type == "house" or interiorDatas.type == "garage" or string.find(interiorDatas.type, "rentable") then
				if lastKnockTick + 5000 <= getTickCount() then
					...
					exports.nlrp_chat:sendLocalMeAction(localPlayer, "kopogtat egy ajtón.")

					lastKnockTick = getTickCount()
				else
					exports.nlrp_hud:showAlert("error", "Csak 5 másodpercenként használhatod ezt a parancsot.")
				end
			else
				exports.nlrp_hud:showAlert("error", "Csak házba vagy garázsba kopogtathatsz.")
			end
		end
	end
end
		...
		if interiorDatas then
			if interiorDatas.type == "house" or interiorDatas.type == "garage" or string.find(interiorDatas.type, "rentable") then
				if getElementDimension(localPlayer) ~= 0 then
					exports.nlrp_hud:showAlert("error", "Csak kintről csengethetsz be.")
				else
					if lastBellTick + 5000 <= getTickCount() then
						...
						exports.nlrp_chat:sendLocalMeAction(localPlayer, "megnyomja a csengőt.")
				...
				exports.nlrp_hud:showAlert("error", "Csak házba vagy garázsba csengethetsz.")
			end
		end
	end
end
			...
			if interiorDatas then
				if getElementData(localPlayer, "char.ID") then
					if lastLockTick + 5000 <= getTickCount() then
						...
						exports.nlrp_hud:showAlert("error", "Csak 5 másodpercenként nyithatod / zárhatod az ajtót.")
					end
				end
			end
		end
			...
			if interiorDatas then
				if interiorDatas.type == "rentable" then
					if interiorDatas.ownerId == getElementData(localPlayer, "char.ID") then
						...
						exports.nlrp_hud:showAlert("error", "Ezt az ingatlant nem te bérled.")
					end
				end
			end
		end
			...
			if interiorDatas then
				if interiorDatas.type == "rentable" then
					if interiorDatas.ownerId == 0 then
						if exports.nlrp_core:getMoney(localPlayer) >= interiorDatas.price * 5 then
							...
							exports.nlrp_hud:showAlert("error", "Nincs elég pénzed, hogy kifizesd a bérleti díjat és kauciót. (" .. formatNumber(interiorDatas.price * 5) .. "Ft)")
						end
					elseif interiorDatas.ownerId == getElementData(localPlayer, "char.ID") then
						if exports.nlrp_core:getMoney(localPlayer) >= interiorDatas.price then
							if lastBuyTick + 5000 <= getTickCount() then
							...
							exports.nlrp_hud:showAlert("error", "Nincs elég pénzed, hogy kifizesd a bérleti díjat. (" .. formatNumber(interiorDatas.price) .. "Ft)")
						end
					else
						exports.nlrp_hud:showAlert("error", "Ez az ingatlan nem kiadó.")
					end
				else
					exports.nlrp_hud:showAlert("error", "Ezt az ingatlant nem lehet kibérelni.")
				end
			end
		end
	end
...
addCommandHandler("buy",
	function ()
		if activeInteriorMarker then
			...
			if interiorDatas then
				if interiorDatas.type == "rentable" then
					exports.nlrp_hud:showAlert("error", "Bérlakást nem vásárolhatsz meg! A bérléshez használd a /rent parancsot.")
					return
				end
				...
				if interiorDatas.type ~= "building" or interiorDatas.type ~= "door" then
					if interiorDatas.ownerId == 0 then
						if exports.nlrp_core:getMoney(localPlayer) >= interiorDatas.price then
							...
							exports.nlrp_hud:showAlert("error", "Nincs elég pénzed az ingatlan megvásárlásához. (" .. formatNumber(interiorDatas.price) .. "Ft)")
						end
					else
						exports.nlrp_hud:showAlert("error", "Ez az ingatlan nem eladó.")
					end
				else
					exports.nlrp_hud:showAlert("error", "Középületet nem vásárolhatsz meg.")
				end
			end
		end
	end
...
addCommandHandler("bejelentes",
	function()
		if not lehetebejelenteni then return end
		if activeInteriorMarker then
			local interiorDatas = availableInteriors[interiorMarkerDetails[activeInteriorMarker].interiorId]
			if interiorDatas.ownerId == getElementData(localPlayer,"char.ID") then
				if tonumber(interiorDatas.announced) == 0 then
					outputChatBox("#b7ff00 >> NextLevel:#ffffff Sikeresen bejelentetted az ingatlant!",0,0,0,true)
					triggerServerEvent("updateAnnounce",resourceRoot,interiorMarkerDetails[activeInteriorMarker].interiorId,1)
					interiorDatas.announced = 1
				else
					outputChatBox("#b7ff00 >> NextLevel:#ffffff Az ingatlanod jelenleg is be van jelentve!",0,0,0,true)
				end
			else
				outputChatBox("#b7ff00 >> NextLevel:#ffffff Csak a saját ingatlanodat tudod bejelenteni!",0,0,0,true)
			end
		end
	end
)
				...
				if not availableInteriors[interiorId].dummy or availableInteriors[interiorId].dummy == "N" then
					if not getElementData(localPlayer, "player.Cuffed") and not getElementData(localPlayer, "player.Grabbed") then
						if lastEnterTick + 3000 <= getTickCount() then
							...
							exports.nlrp_hud:showAlert("error", "Várj egy kicsit, mielőtt újra használod az ajtót.")
						end
					end
				else
					exports.nlrp_hud:showAlert("error", "Ez az interior nem rendelkezik tényleges belső térrel.")
				end
			end
		end
	end
	...
	if isElement(sourceElement) then
		...
		if markerDetails then
			...
			elseif interiorData.ownerId == 0 and interiorData.type == "rentable" then
				interiorInfo.infoText = "KIADÓ - Bérleti díj: " .. formatNumber(interiorData.price) .. " Ft/hét. Kaució: " .. formatNumber(interiorData.price * 4) .. " Ft"
				exports.nlrp_hud:showAlert("info", "Ez az albérlet kiadó. A kibérléshez használd a /rent parancsot.")
			else
				interiorInfo.infoText = "ELADÓ - Ár: " .. formatNumber(interiorData.price) .. " Ft"
				exports.nlrp_hud:showAlert("info", "Ez az ingatlan eladó. A megvételhez használd a /buy parancsot.")
			end
	...
	function ()
		...
		if interiorInfo then
			...
			if interiorData then
				...
				dxDrawRectangle(x, y + sy - 2, sx, 2, tocolor(183,255,0, 255 * progress))

				dxDrawImage(math.floor(x + respc(10)), math.floor(y + (sy - respc(64) - fontHeight) / 2), respc(64), respc(64), interiorIcon.icon, 0, 0, 0, tocolor(183,255,0, 225 * progress))
				dxDrawText("[" .. interiorInfo.interiorId .. "]", x + respc(10), math.floor(y + (sy - respc(64) - fontHeight) / 2) + respc(64), x + respc(74), 0, tocolor(183,255,0, 255 * progress), 0.75, Roboto, "center", "top")
		...
		if itemListState then
			...
			dxDrawImage(math.floor(itemListPosX + 3), math.floor(itemListPosY + 3), 24, 24, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(183,255,0))
			...
			for i = 1, 12 do
				...
				if interiorId then
					dxDrawText("#b7ff00[" .. interiorId .. "] #ffffff" .. gameInteriors[interiorId].name, x + 5, y, 0, y + 36, tocolor(255, 255, 255), 0.75, Roboto, "left", "center", false, false, false, true)
				end
...
exports.nlrp_admin:addAdminCommand("gameintlist", 6, "Elérhető interior belsők")
...
exports.nlrp_admin:addAdminCommand("setinteriorowner", 6, "Interior tulajdonosának beállítása")
	...
	function (command, interiorId, targetPlayer)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (interiorId and targetPlayer) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. command .. " [Interior ID] [Játékos név / ID]", 0, 0, 0, true)
			else
				interiorId = tonumber(interiorId)

				if availableInteriors[interiorId] then
					targetPlayer = exports.nlrp_core:findPlayer(localPlayer, targetPlayer)
...
exports.nlrp_admin:addAdminCommand("setinteriorid", 6, "Interior belsőjének megváltoztatása")
addCommandHandler("setinteriorid",
	function (command, interiorId, interior)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			if not (tonumber(interiorId) and tonumber(interior)) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. command .. " [Interior ID] [Belső Azonosító]", 0, 0, 0, true)
			else
				interiorId = tonumber(interiorId)
...
exports.nlrp_admin:addAdminCommand("gotointerior", 1, "Interiorhoz teleportálás")
	...
	function (command, interiorId)
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			if not tonumber(interiorId) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. command .. " [Interior ID]", 0, 0, 0, true)
			else
				interiorId = tonumber(interiorId)
...
exports.nlrp_admin:addAdminCommand("nearbyinteriors", 1, "Közelben lévő interiorok listázása")
...
addEventHandler("resetInterior", getRootElement(),
	function (interiorId)
		...
		availableInteriors[interiorId].announced = 0
...
exports.nlrp_admin:addAdminCommand("resetinterior", 6, "Interior visszaállítása")
...
exports.nlrp_admin:addAdminCommand("setinteriorname", 6, "Interior nevének módosítása")
addCommandHandler("setinteriorname",
	function (command, interiorId, ...)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			if not (tonumber(interiorId) and {...}) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. command .. " [Interior ID] [Név]", 0, 0, 0, true)
			else
				interiorId = tonumber(interiorId)
...
exports.nlrp_admin:addAdminCommand("getinteriorname", 6, "Interior nevének lekérdezése")
...
exports.nlrp_admin:addAdminCommand("setinteriorprice", 6, "Interior árának módosítása")
addCommandHandler("setinteriorprice",
	function (command, interiorId, price)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			if not (tonumber(interiorId) and tonumber(price)) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. command .. " [Interior ID] [Ár]", 0, 0, 0, true)
			else
				interiorId = tonumber(interiorId)
...
exports.nlrp_admin:addAdminCommand("getinteriorprice", 6, "Interior árának lekérdezése")
	...
	function (command, interiorId)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			if not tonumber(interiorId) then
				...
				if availableInteriors[interiorId] then
					outputChatBox("#cdcdcd>> Interior: #ffff99A kiválasztott interior ára #ffa600" .. formatNumber(availableInteriors[interiorId].price) .. "Ft.", 0, 0, 0, true)
				else
					outputChatBox("#ff4646>> Interior: #ffff99A kiválasztott interior nem létezik.", 0, 0, 0, true)
				end
			end
...
exports.nlrp_admin:addAdminCommand("setinteriortype", 6, "Interior típusának módosítása")
addCommandHandler("setinteriortype",
	function (command, interiorId, typeId)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			if not (tonumber(interiorId) and tonumber(typeId)) then
				outputChatBox("#b7ff00>> Használat: #ffffff/" .. command .. " [Interior ID] [Típus]", 0, 0, 0, true)
				outputChatBox("#b7ff00>> Típusok: #ffffffKözépület (1), Ház (2), Garázs (3), Bérlakás (4), Ajtó (5)", 0, 0, 0, true)
			else
				interiorId = tonumber(interiorId)
...
exports.nlrp_admin:addAdminCommand("getinteriortype", 6, "Interior típusának lekérdezése")
...
exports.nlrp_admin:addAdminCommand("setinteriorentrance", 6, "Interior bejáratának módosítása")
...
exports.nlrp_admin:addAdminCommand("setinteriorexit", 6, "Interior kijáratának módosítása")
...
exports.nlrp_admin:addAdminCommand("deleteinterior", 6, "Interior törlése")
...
exports.nlrp_admin:addAdminCommand("createinterior", 6, "Interior létrehozása")
addCommandHandler("createinterior",
	function (command, interior, typeId, price, ...)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			if not (tonumber(interior) and tonumber(typeId)) then
				outputChatBox("#b7ff00>> Használat: #FFFFFF/" .. command .. " [Belső Azonosító (/gameintlist) (0: Átjáróhoz)] [Típus] [Ár] [Név]", 0, 0, 0, true)
				outputChatBox("#b7ff00>> Típusok: #ffffffKözépület (1), Ház (2), Garázs (3), Bérlakás (4), Ajtó (5)", 0, 0, 0, true)
			else
				interior = tonumber(interior)
				typeId = tonumber(typeId)
