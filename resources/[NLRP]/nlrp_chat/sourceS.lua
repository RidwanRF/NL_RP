			...
			if tradeContractDatas[source] then
				...
				if currentTime <= tradeContractDatas[source]["tradeEnd"] then
					...
					if currentMoney - price >= 0 then
						...
						exports.nlrp_core:takeMoney(source, price, "buyInterior")

						exports.nlrp_core:giveMoney(seller, price, "sellInterior")
						...
						dbQuery(
							...
							function(qh, player, oldowner)
								...
								exports.nlrp_interiors:changeInteriorOwner(interiorId, characterId)
								...
								if isElement(player) then
									...
									exports.nlrp_hud:showInfobox(player, "success", "Sikeresen megvásároltad az ingatlant " .. formatNumber(price) .. " Ft-ért.")
								...
								if isElement(oldowner) then
									...
									exports.nlrp_hud:showInfobox(oldowner, "success", "Sikeresen eladtad az ingatlant " .. formatNumber(price) .. " Ft-ért.")
						...
						exports.nlrp_hud:showInfobox(source, "error", "Nincs nálad elegendő pénz!")



						outputChatBox(exports.nlrp_core:getServerTag("info") .. "Nincs nálad elegendő pénz az ingatlan megvételéhez!", source, 0, 0, 0, true)

						outputChatBox(exports.nlrp_core:getServerTag("info") .. "Az adásvételi szerződés automatikusan visszavonva.", source, 0, 0, 0, true)

					end

				else

					exports.nlrp_hud:showInfobox(source, "error", "Nincs folyamatban adásvételi szerződés!")
						...
						function(qh, theSeller, thePerson, intiId, thePrice, intiData)
							...
							if result then
								...
								if result < interiorSlot then
									...
									exports.nlrp_hud:showInfobox(theSeller, "info", "Átnyújtottál egy adásvételi szerződést.")
			...
			if tradeContractDatas[source] then
				...
				if currentTime <= tradeContractDatas[source]["tradeEnd"] then
					...
					if currentMoney - price >= 0 then
						...
						exports.nlrp_core:takeMoney(source, price, "buyVehicle")

						exports.nlrp_core:giveMoney(seller, price, "sellVehicle")
						...
						dbQuery(
							...
							function(qh, player, oldowner)
								...
								if isElement(player) then
									...
									exports.nlrp_hud:showInfobox(player, "success", "Sikeresen megvásároltad a járművet " .. formatNumber(price) .. " Ft-ért.")
								...
								if isElement(oldowner) then
									...
									exports.nlrp_hud:showInfobox(oldowner, "success", "Sikeresen eladtad a járművet " .. formatNumber(price) .. " Ft-ért.")
						...
						outputChatBox(exports.nlrp_core:getServerTag("info") .. "Nincs nálad elegendő pénz a jármű megvételéhez!", source, 0, 0, 0, true)
	...
	function(personElement, vehicleElement, price)
		...
		if isElement(source) and isElement(personElement) and isElement(vehicleElement) then
			...
			if characterId > 0 and personId > 0 then
				...
				if ownerId == characterId or getElementData(source, "acc.adminLevel") >= 9 then
...
function payTheMoney(sourcePlayer, targetPlayer, amount)
	...
	if isElement(sourcePlayer) and isElement(targetPlayer) then
		...
		exports.nlrp_core:takeMoney(sourcePlayer, amount, "pay")



		exports.nlrp_core:giveMoney(targetPlayer, amount, "pay")



		outputChatBox(exports.nlrp_core:getServerTag("info") .. "A pénz átadása #b7ff00sikeresen #ffffffmegtörtént. Összeg: #32b3ef" .. amount .. " Ft", sourcePlayer, 0, 0, 0, true)

		outputChatBox(exports.nlrp_core:getServerTag("info") .. "#b7ff00" .. playerName .. " #ffffffadott neked #32b3ef" .. amount .. " #ffffffFt-ot.", targetPlayer, 0, 0, 0, true)
...
function isPlayerPayActive(player)
	if payTimers[player] then
		return true
	else
		return false
	end
...
addCommandHandler("pay",
	...
	function(sourcePlayer, cmd, targetPlayer, amount)
		...
		if not (targetPlayer and amount) then
			...
			outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. cmd .. " [Játékos név / ID] [Összeg]", sourcePlayer, 0, 0, 0, true)
						...
						if amount > 0 then
							...
							if currentMoney - amount >= 0 then
								...
								if not payTimers[sourcePlayer] then
									...
									outputChatBox(exports.nlrp_core:getServerTag("info") .. "A pénz átadása #b7ff005 #ffffffmásodpercen belül megkezdődik.", sourcePlayer, 0, 0, 0, true)

								else

									outputChatBox(exports.nlrp_core:getServerTag("error") .. "Még az előző pénzt sem adtad át, hova ilyen gyorsan?!", sourcePlayer, 0, 0, 0, true)

								end

							else

								outputChatBox(exports.nlrp_core:getServerTag("error") .. "Nincs nálad ennyi pénz!", sourcePlayer, 0, 0, 0, true)

							end

						else

							outputChatBox(exports.nlrp_core:getServerTag("error") .. "Maradjunk a nullánál nagyobb egész számoknál.", sourcePlayer, 0, 0, 0, true)

						end

					else

						outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled.", sourcePlayer, 0, 0, 0, true)

					end

				else

					outputChatBox(exports.nlrp_core:getServerTag("error") .. "Magadnak nem tudsz pénzt adni!", sourcePlayer, 0, 0, 0, true)
...
function outputErrorText(text, element)
    ...
    triggerClientEvent(element, "playClientSound", element, ":nlrp_assets/audio/admin/error.ogg")

    assert(type(text) == "string", "Bad argument @ 'outputErrorText' [expected string at argument 1, got "..type(text).."]")

    outputChatBox(exports.nlrp_core:getServerTag("error") .. text, element, 0, 0, 0, true)
...
function outputInfoText(text, element)
    ...
    outputChatBox(exports.nlrp_core:getServerTag("info") .. text, element, 0, 0, 0, true)
		...
		for i = 1, #pendingPlayers do
			...
			if isElement(pendingPlayers[i][1]) then
				...
				if not adminDuty then
					...
					outputChatBox(pendingPlayers[i][2] .. visibleName .. " (OOC)" .. additionalStr .. ": " .. message, pendingPlayers[i][1], 16, 16, 16, true)

				else

					outputChatBox(adminTitleColor .. "[" .. adminTitle .. "] " .. visibleName .. " (OOC)" .. additionalStr .. ": " .. message, pendingPlayers[i][1], 16, 16, 16, true)
	...
	function (pendingPlayers, visibleName, message, additionalStr)
		...
		for i = 1, #pendingPlayers do
			...
			if isElement(pendingPlayers[i][1]) then
				...
				outputChatBox(pendingPlayers[i][2] .. visibleName .. " (OOC)" .. additionalStr .. ": " .. message, pendingPlayers[i][1], 128, 128, 128, true)
	...
	function (pendingPlayers, visibleName, message)
		...
		for i = 1, #pendingPlayers do
			...
			if isElement(pendingPlayers[i][1]) then
				...
				outputChatBox(pendingPlayers[i][2] .. visibleName .. " (OOC) (rádióba): " .. message, pendingPlayers[i][1], 255, 255, 255, true)
...
function onRadioMessage(player, command, ...)
	...
	if exports.nlrp_inventory:hasItem(player, 79) then
		...
		if not radioTune then
			...
			outputChatBox(exports.nlrp_core:getServerTag("info") .. "Előbb vegyél elő egy walkie-talkiet!", player, 200, 50, 50, true)

		elseif not radioTune[1] or radioTune[1] == 0 then

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "Előbb állíts be egy frekvenciát!", player, 200, 50, 50, true)

		elseif radioTune[2] ~= "Y" then

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "A Walkie-Talkie beállított frekvenciája némítva van!", player, 200, 50, 50, true)

		elseif radioTune[3] then

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "Nincs jogosultságod a beállított frekvencia használatához!", player, 200, 50, 50, true)
			...
			if #message > 0 and utf8.len(message) > 0 and utf8.len(message) <= 128 then
				...
				for k, v in ipairs(getElementsByType("player")) do
					...
					if v ~= player then
						...
						local radioData = exports.nlrp_inventory:hasItemWithData(v, 79, "data1", radioTune[1])
		...
		outputChatBox(exports.nlrp_core:getServerTag("error") .. "Nincs nálad rádió!", player, 200, 50, 50, true)
...
addCommandHandler("d",
	...
	function (player, command, ...)
		...
		if exports.nlrp_inventory:hasItem(player, 79) then
			...
			elseif not exports.nlrp_groups:isPlayerHavePermission(player, "departmentRadio") then

				outputChatBox(exports.nlrp_core:getServerTag("error") .. "Nincs jogosultságod használni ezt a frekvenciát!", player, 200, 50, 50, true)
				...
				if #message > 0 and utf8.len(message) > 0 and utf8.len(message) <= 128 then
					...
					for k,v in ipairs(getElementsByType("player")) do
						...
						if v ~= player then
							...
							local havePermission = exports.nlrp_groups:isPlayerHavePermission(v, "departmentRadio")
...
exports.nlrp_admin:addAdminCommand("a", 1, "Admin chat")
	...
	function (player, command, ...)
		...
		if getElementData(player, "acc.adminLevel") >= 1 then
			...
			if not (...) then
				...
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Üzenet]", player, 255, 255, 255, true)
				...
				if #message > 0 and utf8.len(message) > 0 then
					...
					if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
						...
						exports.nlrp_core:sendMessageToAdmins("[AdminChat] " .. exports.nlrp_core:getAdminLevelColor(getElementData(player, "acc.adminLevel")) .. exports.nlrp_core:getPlayerAdminTitle(player) .. " " .. (getElementData(player, "acc.adminNick") or "Admin") .. ": #ffffff" .. message)
...
exports.nlrp_admin:addAdminCommand("tc", 8, "Admin chat")

addCommandHandler("tc",
			...
			if not (...) then
				...
				if #message > 0 and utf8.len(message) > 0 then
					...
					if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
						...
						exports.nlrp_core:sendMessageToAdmins("[TulajdonosChat] " .. exports.nlrp_core:getAdminLevelColor(getElementData(player, "acc.adminLevel")) .. exports.nlrp_core:getPlayerAdminTitle(player) .. " " .. (getElementData(player, "acc.adminNick") or "Admin") .. ": #ffffff" .. message)
...
exports.nlrp_admin:addAdminCommand("togadminchat", 6, "Admin chat elrejtése")

addCommandHandler("togadminchat",

	function(player,cmd)

		if getElementData(player, "acc.adminLevel") >= 6 then

			if getElementData(player,"acc.showAdminChat") then

				setElementData(player,"acc.showAdminChat",false)

				outputChatBox(exports.nlrp_core:getServerTag("info") .. "Sikeresen elrejtetted az adminchatet!",player,255,255,255,true)

			else

				setElementData(player,"acc.showAdminChat",true)

				outputChatBox(exports.nlrp_core:getServerTag("info") .. "Sikeresen megjelenítetted az adminchatet!",player,255,255,255,true)
...
exports.nlrp_admin:addAdminCommand("as", -2, "Adminsegéd chat")
			...
			if not (...) then
				...
				if #message > 0 and utf8.len(message) > 0 then
					...
					if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
						...
						exports.nlrp_core:sendMessageToAdmins("[AS Chat] " .. exports.nlrp_core:getHelperLevelColor(getElementData(player, "acc.helperLevel")) .. exports.nlrp_core:getPlayerHelperTitle(player) .. " " .. getElementData(player, "visibleName") .. ": #ffffff" .. message, 2)
	...
	function (player, command, target, ...)
		...
		--if getElementData(player, "acc.adminLevel") >= 1 then
			...
			if not (...) or not target then
				...
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [ID] [Üzenet]", player, 255, 255, 255, true)
