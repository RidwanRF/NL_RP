...
addCommandHandler("say",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Üzenet]", 255, 255, 255, true)
			else
...
addCommandHandler("me",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Cselekvés]", 255, 255, 255, true)
			else
...
addCommandHandler("do",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Történés]", 255, 255, 255, true)
			else
...
addCommandHandler("ame",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Vizuális leírás]", 255, 255, 255, true)
			else
...
function sendLocalMessage(player, message)
	if string.len(message) > 0 then
		...
		if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
			...
			local adminTitle = exports.nlrp_core:getPlayerAdminTitleByLevel(adminLevel)
			local adminTitleColor = exports.nlrp_core:getAdminLevelColor(adminLevel)

			if not adminDuty or player ~= localPlayer then
				outputChatBox("#FFFFFF" .. visibleName .. " (OOC)" .. additionalStr .. ": " .. firstToUpper(message), 16, 16, 16, true)
			else
				outputChatBox(adminTitleColor .. "[" .. adminTitle .. "] " .. visibleName .. " (OOC)" .. additionalStr .. ": " .. firstToUpper(message), 16, 16, 16, true)
			end
...
function sendLocalMeAction(player, message)
	if string.len(message) > 0 then
		...
		outputChatBox("#C2A2DA*** " .. visibleName:gsub("#%x%x%x%x%x%x","") .. " #C2A2DA" .. message:gsub("#%x%x%x%x%x%x",""), 32, 32, 32, true)
	...
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				...
				local currentStandingMarker = exports.nlrp_interiors:getCurrentStandingMarker()

				if currentStandingMarker then
					local interiorData = exports.nlrp_interiors:getInteriorData(currentStandingMarker)
					...
					if interiorData then
						...
						exports.nlrp_hud:showInfobox("error", "A parancs használatához egy interior markerben kell lenned!")
					end
				else
					exports.nlrp_hud:showInfobox("error", "A parancs használatához egy interior markerben kell lenned!")
				end
				...
				if currentStandingMarker then
					...
					if interiorData then
						if not isPedInVehicle(localPlayer) then
							...
							exports.nlrp_hud:showInfobox("error", "Járműben ülve nem ordíthatsz be az ingatlanba!")
						end
					else
						exports.nlrp_hud:showInfobox("error", "A parancs használatához egy interior markerben kell lenned!")
					end
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		setElementData(localPlayer,"acc.showAdminChat",true)
...
addCommandHandler("m",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") and exports.nlrp_groups:isPlayerHavePermission(localPlayer, "megaPhone") then
			if not (...) then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Üzenet]", 255, 255, 255, true)
			else
				local occupiedVehicle = getPedOccupiedVehicle(localPlayer)
				...
				if not canUseMegaphone then
					outputChatBox(exports.nlrp_core:getServerTag("error") .. "A jármű amiben ülsz, nem rendelkezik megafonnal, vagy nincs a kezedben megafon!", 215, 89, 89, true)
					return
				end
...
addEventHandler("sellInteriorNotification", getRootElement(),
	function(seller, inti, price, data)
		if not tradeContract then
			...
			exports.nlrp_hud:showInfobox("info", sellerName .. " el akar adni neked egy ingatlant " .. formatNumber(price) .. " Ft-ért!")

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "#32b3ef" .. sellerName .. " #ffffffel akar adni neked egy ingatlant #FF9600" .. formatNumber(price) .. " Ft#ffffff-ért!", 0, 0, 0, true)

			outputChatBox("    * #FF9600Ingatlan: #ffffff" .. data.name, 255, 255, 255, true)

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "#FF96005 #ffffffperced van elfogadni az ingatlant a #32b3ef/acceptsell #ffffffparanccsal.", 0, 0, 0, true)
...
addEventHandler("sellVehicleNotification", getRootElement(),
	function(seller, veh, price)
		if not tradeContract then
			...
			exports.nlrp_hud:showInfobox("info", sellerName .. " el akar adni neked egy járművet " .. formatNumber(price) .. " Ft-ért!")

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "#32b3ef" .. sellerName .. " #ffffffel akar adni neked egy járművet #FF9600" .. formatNumber(price) .. " Ft#ffffff-ért!", 0, 0, 0, true)

			outputChatBox("    * #FF9600Típus: #ffffff" .. exports.nlrp_vehs:getVehicleName(veh), 255, 255, 255, true)

			outputChatBox(exports.nlrp_core:getServerTag("info") .. "#FF96005 #ffffffperced van elfogadni a járművet a #32b3ef/acceptsell #ffffffparanccsal.", 0, 0, 0, true)
...
addEventHandler("failedToSell", getRootElement(),
	function(errno)
		if errno then
			exports.nlrp_hud:showInfobox("error", errno)
		end
...
addCommandHandler("sell",
	function(cmd, targetPlayer, amount)
		...
		if not (targetPlayer and amount) then
			outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. cmd .. " [Játékos név / ID] [Összeg]", 0, 0, 0, true)
		else
				...
				if targetPlayer ~= localPlayer then
					...
					if dist <= 5 and pi == ti and pd == td then
						...
						if amount >= 0 then
							...
							local inti = exports.nlrp_interiors:getCurrentStandingMarker()
							...
							if isElement(pedveh) then
								if not gotSelling then
									...
									outputChatBox(exports.nlrp_core:getServerTag("error") .. "Egy adásvételi szerződés már folyamatban van!", 0, 0, 0, true)
								end
							elseif inti then
								local intidata = exports.nlrp_interiors:getInteriorData(inti)
								...
								if intidata then
									if intidata.type == "house" or intidata.type == "garage" then
										if intidata.ownerId == getElementData(localPlayer, "char.ID") then
											...
											outputChatBox(exports.nlrp_core:getServerTag("error") .. "Ez az ingatlan nem a tiéd.", 0, 0, 0, true)
										end
									end
								end
							end
						else
							outputChatBox(exports.nlrp_core:getServerTag("error") .. "Maradjunk a nullánál nagyobb egész számoknál.", 0, 0, 0, true)
						end
					else
						outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled.", 0, 0, 0, true)
					end
				else
					outputChatBox(exports.nlrp_core:getServerTag("error") .. "Magadnak nem adhatod el.", 0, 0, 0, true)
				end
			end
		end
	end)
