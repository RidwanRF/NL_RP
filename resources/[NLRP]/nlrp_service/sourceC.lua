...
boxCols[1] = createColRectangle( -2273.3491210938, -1645.5, 15, 10)
boxCols[2] = createColRectangle(-2273.3491210938, -1624, 15, 10)
...
addEvent("nlrp_serviceC:startService", true)
addEventHandler("nlrp_serviceC:startService", root, startService)

addEvent("nlrp_serviceC:showService", true)
addEventHandler("nlrp_serviceC:showService", root, function()
    print("ok")
    selectedVehicle = source
...
function renderService()
    if startTick then
        ...
        if barProgress >= 1 then
            ...
            triggerServerEvent("nlrp_serviceS:repairComponent", localPlayer, selectedVehicle, selectedComponent)
            isServicing = false
        end
    end
end
...
addEventHandler("onClientRender", root, function()
    if selectedVehicle then
        if isElementInServiceZone(selectedVehicle) then
            ...
            if exports.nlrp_core:inDistance3D(localPlayer, selectedVehicle, 4) and not isServicing then
                ...
                --dxDrawImage(math.floor(panelX + 3), math.floor(panelY + 3), 28, 24, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(183, 255, 0))
                dxDrawText("Sérült alkatrészek", panelX + 10, panelY, 0, panelY + 30, tocolor(255, 255, 255), 1, fonts.RobotoL, "left", "center")
                ...
                if kilometersToChangeOil <= 30 then
            ...
            for k, v in pairs(getVehicleComponents(selectedVehicle)) do
                if components[k] then
                    if activeButton == "component:" .. k then
                        --triggerServerEvent("nlrp_serviceS:repairComponent", localPlayer, selectedVehicle, k)
                        triggerServerEvent("nlrp_serviceS:startService", localPlayer, selectedVehicle, k)
                    end
                end
            end
