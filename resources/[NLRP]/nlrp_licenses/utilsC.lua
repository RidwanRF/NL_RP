...
local buttonIsProcessing = false
...
function dxDrawMetroButtonWithBorder(key, label, x, y, w, h, buttonColor, activeColor, textColor, font, textAlign1, textAlign2, imagePath, imageW, imageH, imageColor)
    ...
    if imagePath then
        ...
        dxDrawText(label, x + respc(5), y, x + w, y + h, tocolor(textColor[1], textColor[2], textColor[3], 255 * fadeAmount), 1, font, textAlign1, textAlign2, false, true)
    end
    ...
    if activeDirectX ~= lastActiveDirectX and activeDirectX then
        playSound(":nlrp_assets/audio/interface/hover.ogg")
    end
end
...
function drawScrollbar(key, x, y, w, h, visibleItems, currentItems)
    ...
    dxDrawImage(x, trackY - w, w, w, ":nlrp_groups/files/icons/arrow.png", 0, 0, 0, colorOfArrow)
    ...
    dxDrawImage(x, trackY + trackHeight, w, w, ":nlrp_groups/files/icons/arrow.png", 180, 0, 0, colorOfArrow)
...
function drawButtonSlider(key, state, x, y, h, offColor, onColor)
    ...
    dxDrawImage(x, y, 64, 32, ":nlrp_groups/files/toggleSwitch/off.png", 0, 0, 0, tocolor(255, 255, 255, (255 - alphaDifference) * fadeAmount))
    dxDrawImage(x, y, 64, 32, ":nlrp_groups/files/toggleSwitch/on.png", 0, 0, 0, tocolor(buttonColor[1], buttonColor[2], buttonColor[3], alphaDifference * fadeAmount))
    dxDrawImage(x + buttonSlider.offsets[key], y, 64, 32, ":nlrp_groups/files/toggleSwitch/circle.png", 0, 0, 0, tocolor(255, 255, 255, 255 * fadeAmount))
end
