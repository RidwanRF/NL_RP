...
function renderDrivePanel()
	...
	--dxDrawImage(math.floor(driveX + 3), math.floor(driveY + 3), 24, 24, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(183, 255, 0))
    ...
    if page == "main" then
        ...
        dxDrawText("", driveX + 10, driveY + 40, driveW + driveX - 10, driveH + driveY, tocolor(255, 255, 255, 255), 1, fonts.Roboto13, "center", "center", false, true)--Szovegs
...
function startTestDrive()
    ...
    exports.nlrp_alert:showAlert("info", "Kövesse az útvonalat")
    triggerServerEvent("nlrp_licenesesS:createTestVehicle", localPlayer, localPlayer)
end

function createNextCheckpoint()
    currentCheckpoint = currentCheckpoint + 1
    if currentCheckpoint > #drivePositions then
        exports.nlrp_alert:showAlert("info", "Visga vége")
        destroyCheckpoint()
        endOfTestDrive()
        return
    end
...
function createCheckpoint(x, y, z, speedlimit)
    ...
    if speedlimit then
        outputChatBox(exports.nlrp_core:getServerTag("info") .. "Sebesség korlát #ff4646" .. currentSpeedLimit .. "#ffffffmph lett.", 0, 0, 0, true)
    end
end
...
function endOfTestDrive(failTest)
    if failTest then
        ...
        triggerServerEvent("nlrp_licenesesS:destroyTestVehicle", localPlayer, localPlayer)
        return
    end
...
addEventHandler("onClientVehicleStartEnter", root, function(player, seat)
    if player == localPlayer then 
        if getElementData(source, "vehicle.test") and player ~= getElementData(source, "vehicle.testOwner") then
            --if seat == 0 then
                exports.nlrp_hud:showAlert("error", "Ez nem a te oktató járműved")
                cancelEvent()
            --end
        end
    end
...
addEventHandler("onClientVehicleExit", getRootElement(), function(player, seat)
    if player == localPlayer then
        if getElementData(source, "vehicle.test") and player == getElementData(source, "vehicle.testOwner") then
            exports.nlrp_hud:showAlert("info", "Az oktató járműved törlődni fog.", "Ha nem szálsz vissza a járművedbe, akkor " .. destroyTime .. " perc után törlődik")
            destroyTimer = setTimer(function()
                endOfTestDrive(true)
                exports.nlrp_hud:showAlert("info", "A oktató járműved törlődött")
