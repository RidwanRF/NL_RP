...
function createTestVehicle(player)
    ...
    setVehiclePlateText(testVehicles[player], "NLRP-" .. getElementData(source, "char.ID")) 
    warpPedIntoVehicle(player, testVehicles[player])

    triggerClientEvent("vehicleSpawnProtect", source, testVehicles[player])
end
registerEvent("nlrp_licenesesS:createTestVehicle", root, createTestVehicle)
...
registerEvent("nlrp_licenesesS:destroyTestVehicle", root, destroyTestVehicle)