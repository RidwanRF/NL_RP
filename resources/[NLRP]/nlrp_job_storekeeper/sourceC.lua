...
local function loadFonts()
	local fonts = {
		ocr10bold = exports.nlrp_assets:loadFont("ocr.ttf", 10, false, "cleartype"),
		ocr25bold = exports.nlrp_assets:loadFont("ocr.ttf", 25, false, "cleartype"),
		RobotoLabel = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", respc(14), false, "antialiased"),
		RobotoBold = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", 24, false, "antialiased"),
		handFont = exports.nlrp_assets:loadFont("hand.otf", respc(24), false, "antialiased"),
		barCode = exports.nlrp_assets:loadFont("VT323.ttf", respc(10), false, "cleartype"),
		CourierNew = exports.nlrp_assets:loadFont("CourierNew.ttf", respc(18), false, "antialiased"),
		RobotoBoldR = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", respc(24), false, "cleartype"),
...
function playerPressedKey(button, press)
    if (press and button == "lalt" and showSticker) then
...
function startTheJob()
	...
	jobData = {}
	jobData.shelfPlacedItems = math.random(1, 8)--math.random(8, 15) 2-6
	...
	setElementData(mainCarrierObject, "object.name", "Kezi raklapemelo")
	setElementData(mainCarrierObject, "isFactoryObject", true)

	currentInteractions.carrier = {}
	currentInteractions.crate = {}

	table.insert(currentInteractions.carrier, {"Eszkoz felvetele", ":nlrp_job_storekeeper/files/pickUpCarrier.png", "onClientPlayerPickUpCarrier"})

	table.insert(currentInteractions.crate, {"Átrakas a raklapemelore", ":nlrp_job_storekeeper/files/moveCrate.png", "onClientPlayerPickUpCrate"})
end
...
function endJob(finish)
	...
	if finish then
		local payment = crateCount * math.random(1000,15000)

		exports.nlrp_hud:showAlert("success", "Sikeresen vegeztel a munkaval!", "Fizetes: " .. payment .. " Ft")
		outputChatBox(exports.nlrp_core:getServerTag("info") .. "Elvegezted a munkad, a fizetesed: #acd373" .. payment .. " Ft", 50, 179, 239, true)

		triggerServerEvent("giveStorekeeperJobCash",resourceRoot,localPlayer,payment)
	end
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		...
		setElementData(jobPed, "visibleName", "Muszakvezeto")
	...
	function (button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedWorld)
		if state == "up" and clickedWorld == jobPed and getElementData(localPlayer, "char.Job") == JOB_ID then
			...
			if getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, pedPosX, pedPosY, pedPosZ) <= 5 then
				if jobStarted then
					exports.nlrp_hud:showAlert("error", "Mar dolgozol!")
				else
					startTheJob()

					outputChatBox(exports.nlrp_core:getServerTag("info") .. "Elkezdted a munkat. A kezi raklapemelot a raktar bejaratanal talalod.", 255, 150, 0, true)
					outputChatBox(exports.nlrp_core:getServerTag("info") .. "Jelenleg #32b3ef" .. jobData.shelfPlacedItems .. " db#ffffff csomagot kell atszallitanod es cimkezned.", 255, 150, 0, true)

					exports.nlrp_hud:showAlert("info", "Elkezdted a munkat.")
				end
			end
		end
	end
...
addEventHandler("onClientPlayerPickUpCarrier", getRootElement(),
	function (sourceElement)
		...
		exports.nlrp_controls:toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "crouch", "sprint"}, false)
	end
)
...
addEventHandler("onClientPlayerPickUpCrate", getRootElement(),
	function (sourceElement)
		if not getElementData(localPlayer, "carrierActive") then
			exports.nlrp_hud:showAlert("error", "Elobb vedd fel a raktar bejaratatol a raklapemelot!")
		else
			local cratesOnCarrier = getElementData(localPlayer, "carrierPlacedObjects") or {}

			if #cratesOnCarrier + 1 > 4 then
				exports.nlrp_hud:showAlert("error", "Nem fer tobb lada az emelore!")
			else
...
addEventHandler("onClientPlayerPickUpCarrierAfterDropped", getRootElement(),
	function (sourceElement)
		if jobData.carryingCrate then
			exports.nlrp_hud:showAlert("error", "Elobb cimkezd fel a kezedben levo ladat!")
		else
...
addEventHandler("onClientPlayerPickUpCrateFromCarrier", getRootElement(),
	function (sourceElement)
		if getElementData(localPlayer, "carryingCrate") then
			exports.nlrp_hud:showAlert("error", "Mar van a kezedben egy lada!")
		else
			setElementData(sourceElement, "isInteractable", false)
	...
	if getElementData(player, "carrierActive") then
		...
		if getElementData(player, "carrierPlacedOnFactory") and syncableCarriers[player] then
			...
			if player == localPlayer then
				...
				setElementData(syncableCarriers[player], "object.name", "Kezi raklapemelo")
				setElementData(syncableCarriers[player], "isFactoryObject", true)

				currentInteractions.carrier = {}
				table.insert(currentInteractions.carrier, {"Eszkoz felvetele", ":nlrp_job_storekeeper/files/pickUpCarrier.png", "onClientPlayerPickUpCarrierAfterDropped"})
			end
			...
			if getElementData(player, "carryingCrate") and not syncableCarryingCrates[player] then
				...
				exports.nlrp_boneattach:attachElementToBone(crateObject, player, 12, 0.25, 0, 0.15, -100, 0, -20)
			...
			for i = 1, #carrierPlacedObjects do
				...
				if player == localPlayer then
					...
					if enableCrateCollisions then
						currentInteractions.carrier = {}
						table.insert(currentInteractions.carrier, {"Eszkoz felvetele", ":nlrp_job_storekeeper/files/pickUpCarrier.png", "onClientPlayerPickUpCarrierAfterDropped"})
						table.insert(currentInteractions.carrier, {"Doboz felvetele", ":nlrp_job_storekeeper/files/pickUpCrate.png", "onClientPlayerPickUpCrateFromCarrier"})
					end
				end
			end
		end
	...
	if jobStarted and isElementWithinColShape(localPlayer, labelingColshape) then
		...
		if jobData.carryingCrate then
			if jobData.carryingCrate.canPlace then
				dxDrawText("A targy lerakasahoz nyomd meg az [E] gombot.", 1, screenY - respc(256) + 1, screenX + 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy lerakasahoz nyomd meg az [E] gombot.", -1, screenY - respc(256) - 1, screenX - 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy lerakasahoz nyomd meg az [E] gombot.", -1, screenY - respc(256) + 1, screenX - 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy lerakasahoz nyomd meg az [E] gombot.", 1, screenY - respc(256) - 1, screenX + 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy lerakasahoz nyomd meg az #32b3ef[E]#ffffff gombot.", 0, screenY - respc(256), screenX, screenY - respc(224), tocolor(255, 255, 255), 1, RobotoLabel, "center", "center", false, false, false, true)
			elseif not showSticker then
				dxDrawText("A targy felcimkezesehez nyomd meg az [E] gombot.", 1, screenY - respc(256) + 1, screenX + 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy felcimkezesehez nyomd meg az [E] gombot.", -1, screenY - respc(256) - 1, screenX - 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy felcimkezesehez nyomd meg az [E] gombot.", -1, screenY - respc(256) + 1, screenX - 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy felcimkezesehez nyomd meg az [E] gombot.", 1, screenY - respc(256) - 1, screenX + 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("A targy felcimkezesehez nyomd meg az #32b3ef[E]#ffffff gombot.", 0, screenY - respc(256), screenX, screenY - respc(224), tocolor(255, 255, 255), 1, RobotoLabel, "center", "center", false, false, false, true)
			else
				sx = respc(822)
				sy = respc(417)
				x = (screenX - sx) / 2
				...
				if signStart then
					...
					if progress > 1.25 then
						...
						exports.nlrp_hud:showAlert("info", "Cimkezes kesz!", "Vidd a ladat a sargan jelolt helyre.")
					end
				end
			end
		elseif syncableCarriers[localPlayer] and isElementAttached(syncableCarriers[localPlayer]) then
			dxDrawText("Az emelo lerakasahoz nyomd meg az [E] gombot.", 1, screenY - respc(256) + 1, screenX + 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
			dxDrawText("Az emelo lerakasahoz nyomd meg az [E] gombot.", -1, screenY - respc(256) - 1, screenX - 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
			dxDrawText("Az emelo lerakasahoz nyomd meg az [E] gombot.", -1, screenY - respc(256) + 1, screenX - 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
			dxDrawText("Az emelo lerakasahoz nyomd meg az [E] gombot.", 1, screenY - respc(256) - 1, screenX + 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
			dxDrawText("Az emelo lerakasahoz nyomd meg az #32b3ef[E]#ffffff gombot.", 0, screenY - respc(256), screenX, screenY - respc(224), tocolor(255, 255, 255), 1, RobotoLabel, "center", "center", false, false, false, true)
		end
	end
end
		...
		if jobStarted and not showSticker then
			if isElementWithinColShape(localPlayer, labelingColshape) then
				if syncableCarriers[localPlayer] and isElementAttached(syncableCarriers[localPlayer]) and canPlaceObject(syncableCarriers[localPlayer]) then
					...
					exports.nlrp_controls:toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "crouch", "sprint"}, true)
				elseif jobData.carryingCrate then
					if jobData.carryingCrate.canPlace then
						if isElementWithinColShape(localPlayer, crateFinalPosition[7]) then
							...
							exports.nlrp_hud:showAlert("error", "Itt nem rakhatod le, menj a sargaval kijelolt helyre!")
						end
					else
						...
						if not workbench then
							exports.nlrp_hud:showAlert("error", "Itt nem cimkezheted fel, menj egy asztalhoz!")
						else
							local playerX, playerY, playerZ = getElementPosition(localPlayer)
							...
							if hit and hitElement and getElementModel(hitElement) == 941 then
								...
								exports.nlrp_hud:showAlert("error", "Fordulj szembe az asztallal, vagy allj kozelebb!")
							end
						end
					end
				end
			...
			if fakeInputs["customerName"] == jobData.carryingCrate.customerName then
				if fakeInputs["deliveryAddress"] == jobData.carryingCrate.address then
					if tonumber(fakeInputs["zipCode"]) == jobData.carryingCrate.zipCode then
						...
						exports.nlrp_hud:showAlert("error", "Az iranyitoszam nem megfelelo!")
					end
				else
					exports.nlrp_hud:showAlert("error", "A megrendelo cime nem megfelelo!")
				end
			else
				exports.nlrp_hud:showAlert("error", "A megrendelo nev nem megfelelo!")
			end
	...
	if isElement(objectElement) then
		...
		for i = 0, 1 do
			...
			if not isLineOfSightClear(objectX, objectY, objectZ, offX, offY, offZ, true, true, false, true, true, false, false, localPlayer) then
				exports.nlrp_hud:showAlert("error", "Ide nem rakhatod le, mert valamivel ütkozik a targy!")
				return
			end
...
function removeGTAObjects()
	...
	createObject(3624, 2776.0703125, -2525.546875, 16.7265625, 0, 0, 270) -- Cimkezo reszleg
