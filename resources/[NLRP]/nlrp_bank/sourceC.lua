...
FontManager = {
	...
	loadFonts = function ()
		...
		FontManager.addFont("Roboto", exports.nlrp_assets:loadFont("Roboto-Regular.ttf", UI:resp(16), false, "cleartype"))
		FontManager.addFont("Roboto11", exports.nlrp_assets:loadFont("Roboto-Regular.ttf", UI:resp(11), false, "cleartype"))
		FontManager.addFont("RobotoL16", exports.nlrp_assets:loadFont("Roboto-Light.ttf", UI:resp(16), false, "cleartype"))
	end
}
...
function Panel.show()
	...
	elseif currentPanel == "manage" then
        ...
        widgets.bankBalance = UI:createCustomLabel({
            ...
            alignX = "center",
            alignY = "center",
            text = "Egyenleg: " .. getElementData(localPlayer, "char.bankMoney") .. "Ft",
        })
...
addEventHandler("sarpUI.click", resourceRoot, function (widget, button)
	if button == "left" then
		...
		elseif widget == widgets.nextButton then
			...
			triggerServerEvent("nlrp_bankS:loadPlayerBankAccount", root, localPlayer, tonumber(UI:getText(widgets.passwordInput)))
		...
		elseif widget == widgets.outButton then
			...
			if num > 0 then
                triggerServerEvent("nlrp_bankS:takePlayerBankMoney", localPlayer, localPlayer, num)
                UI:setText(widgets.bankBalance, "Egyenleg: " .. getElementData(localPlayer, "char.bankMoney") .. "Ft")
			else
				exports.nlrp_hud:showAlert("error", "Az összegnek nagyobbnak kell", "lennie, mint 0")
			end
			changePanel("manage")
		elseif widget == widgets.inButton then
			local num = tonumber(UI:getText(widgets.depositInput)) or 0
			if num > 0 then
                triggerServerEvent("nlrp_bankS:givePlayerBankMoney", localPlayer, localPlayer, num)
                UI:setText(widgets.bankBalance, "Egyenleg: " .. getElementData(localPlayer, "char.bankMoney") .. "Ft")
			else
                exports.nlrp_hud:showAlert("error", "Az összegnek nagyobbnak kell", "lennie, mint 0")
			end
...
registerEvent("nlrp_bankC:updateInfo", root, updateInfo)

function updateBalance()
    UI:setText(widgets.bankBalance, "Egyenleg: " .. getElementData(localPlayer, "char.bankMoney") .. "Ft")
end
registerEvent("nlrp_bankC:updateBalance", root, updateBalance)


addEventHandler("onClientClick", root, function(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
    if button == "right" and state == "up" and clickedElement then
        if getElementType(clickedElement) == "object" and getElementData(clickedElement, "atm.id") and getElementData(clickedElement, "robbable") then
            if exports.nlrp_core:inDistance3D(localPlayer, clickedElement, 1) then
                Panel.show()
				--local version = getVersion()
				--outputChatBox(""..version.netcode)
				--outputChatBox("MTA:SA Hiba miatt jelenleg az ATM nem működik.",255,255,255,true)
				--outputChatBox("Kivételhez ideiglenesen használd a #28db28/kivesz [összeg]#FFFFFF parancsot",255,255,255,true)
				--outputChatBox("Berakáshoz ideiglenesen használd a #28db28/betesz [összeg]#FFFFFF parancsot",255,255,255,true)
            end
        end
    end
end)