...
function loadFonts()
	local fonts = {
		...
		Roboto10 = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", respc(10), false, "antialiased"),
		RobotoB10 = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", respc(10), false, "antialiased"),
	}
...
exports.nlrp_admin:addAdminCommand("debugbilliard", 9, "Biliárd debugolása (fejlesztő mód)")
...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedResource)
		if getResourceName(startedResource) == "nlrp_hud" then
			responsiveMultipler = exports.nlrp_hud:getResponsiveMultipler()
		elseif source == getResourceRootElement() then
			local nlrp_hud = getResourceFromName("nlrp_hud")

			if nlrp_hud and getResourceState(nlrp_hud) == "running" then
				responsiveMultipler = exports.nlrp_hud:getResponsiveMultipler()
			end
...
function getCurrentInteractionList()
	if poolTableObj and poolTableId then
		return {{"Beszállás a játékba", ":nlrp_billiard/files/startgame.png", "startPlayBilliard"}}
	end
	...
	if button == "mouse2" then
		if state ~= crosshairState then
			...
			if state and forcedTableId then
				exports.nlrp_hud:showAlert("error", "Várd meg, amíg a másik asztalon véget ér a játék!")
				cancelEvent()
				return
			end
			...
			if state then
				...
				if getDistanceBetweenPoints3D(playerX, playerY, playerZ, tableX, tableY, tableZ) <= getElementRadius(poolTableObj) * 1.5 then
					if math.abs(playerZ - tableZ) - 0.47832541465757 >= 1 then
						cancelEvent()
						exports.nlrp_hud:showAlert("error", "Előbb szállj le a földre!")
						return
					end
					...
					if rotationBetweenPoints < 180 then
						cancelEvent()
						exports.nlrp_hud:showAlert("error", "Állj szembe az asztallal!")
						return
					end
					...
					if isElement(poolTableInUse) and poolTableInUse ~= localPlayer then
						cancelEvent()
						exports.nlrp_hud:showAlert("error", "Várj egy kicsit, már valaki más céloz!")
						return
					end
		...
		if state ~= crosshairInUse then
			...
			if not state and selectedBall and poolTableObj then
				...
				if isElement(poolTableInUse) and poolTableInUse == localPlayer then
					...
					exports.nlrp_chat:sendLocalMeAction(localPlayer, "meglök egy golyót.")
				end
	...
	function (timeSlice)
		...
		for tableId in pairs(tableObject) do
			...
			for boundId = 1, #tableAreaPos[tableId] do
				...
				if debugMode then
					dxDrawLine3D(bound[1], bound[2], bound[5], bound[3], bound[4], bound[5], tocolor(215, 163, 96), 1)
				end
			...
			for k = 1, 10 do
				...
				if v then
					...
					elseif v == "newgame" then
						dxDrawRectangle(0, y, sx, sy, tocolor(215, 163, 96, 50))
						dxDrawText("*** Új játék kezdődött ***", 0, y, sx, y + sy, tocolor(215, 163, 96), 1, RobotoB10, "center", "center")
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		if poolTableObj and poolTableId then
			...
			dxDrawImage(math.floor(x), math.floor(y - respc(40) + (respc(40) - respc(459 * 0.07)) / 2), respc(403 * 0.07), respc(459 * 0.07), ":nlrp_assets/images/sarplogo_big.png", 0, 0, 0, tocolor(215, 163, 96))
...
function poolTableCommandHandler(command)
	if getElementData(localPlayer, "acc.adminLevel") >= 7 then
		if not placedTable then
			...
			outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Biliárd asztal létrehozás mód #7cc576bekapcsolva!", 255, 255, 255, true)
			outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Az asztal #7cc576lerakásához #ffffffnyomd meg a #FFA600BAL ALT #ffffffgombot.", 255, 255, 255, true)
			outputChatBox(exports.nlrp_core:getServerTag("admin") .. "A #d75959kilépéshez #ffffffírd be a #d75959/" .. command .. " #ffffffparancsot.", 255, 255, 255, true)
		else
			...
			outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Biliárd asztal létrehozás mód #d75959kikapcsolva!", 255, 255, 255, true)
		end
	end
end
addCommandHandler("createpool", poolTableCommandHandler)
addCommandHandler("createbilliard", poolTableCommandHandler)
exports.nlrp_admin:addAdminCommand("createbilliard", 7, "Biliárd asztal létrehozása, elérhető még: /createpool")

function tablePlaceKey(button, state)
	if isElement(placedTable) then
		if button == "lalt" and state then
			outputChatBox(exports.nlrp_core:getServerTag("admin") .. "#7cc576Sikeresen leraktál egy biliárd asztalt!", 255, 255, 255, true)
...
exports.nlrp_admin:addAdminCommand("nearbybilliard", 7, "Közelben lévő biliárd asztalok listázása")
