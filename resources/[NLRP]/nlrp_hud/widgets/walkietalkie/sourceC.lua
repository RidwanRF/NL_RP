...
triggerServerEvent ( "tunrad", resourceRoot, localPlayer, 0)
...
function checkForGroups()
	...
	if loggedIn then
		...
		if currentRadioTune then
				triggerServerEvent ( "tunrad", resourceRoot, localPlayer, tempChannel)
				...
				if getTickCount() >= antiFloodTick then
					...
					if activeRadioItemID then
						if protectedChannels[lastChannel] then
							exports.nlrp_hud:showInfobox("error", "A kiválasztott frekvencia védett!")
						else
    					triggerServerEvent ( "tunrad", resourceRoot, localPlayer, lastChannel)
