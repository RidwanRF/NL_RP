...
addEventHandler("onClientResourceStart", getRootElement(),
	function(startedRes)
		if startedRes == getThisResource() then
			local groupscripting = getResourceFromName("nlrp_groupscripting")

			if groupscripting and getResourceState(groupscripting) == "running" then
				allowedVehicles = exports.nlrp_groupscripting:getAllowedVehicles()
			end
		...
		elseif getResourceName(startedRes) == "nlrp_groupscripting" then
			allowedVehicles = exports.nlrp_groupscripting:getAllowedVehicles()
		end
	end)
		...
		if state == "down" then
			if activeButton and occupiedVehicle then
				...
				if activeButton == "settings" then
					triggerEvent("nlrp_sirenC:showSettings", localPlayer)
				elseif selected[1] == "sound" then
					...
					if activeControls.sound == soundID then
						...
						triggerServerEvent("nlrp_sirenS:toggleSirenSound", localPlayer, soundID)
					end
				elseif selected[1] == "lights" then
					...
					if activeControls.light == lightID or activeControls.strobe == lightID then
						...
						triggerServerEvent("nlrp_sirenS:toggleSirenLights", localPlayer, lightID)
					end
				end
