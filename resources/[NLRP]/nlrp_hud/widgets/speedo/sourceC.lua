		...
		if vehicle and vehicleEngine and not outOfFuel then
			...
			if getVehicleType(vehicle) == "Automobile" then
				...
				if lastOilChange > 515000 and getPedOccupiedVehicleSeat(localPlayer) == 0 and getElementHealth(vehicle) > 321 then
					...
					exports.nlrp_alert:showAlert("error", "Mivel nem cserélted ki a motorolajat, ezért az autód motorja elromlott!")
		...
		if isElement(vehicle) and getVehicleController(vehicle) == localPlayer then
			if getVehicleType(vehicle) == "Automobile" then
				if getElementHealth(vehicle) <= 600 and math.random(100000) <= 20 and getVehicleEngineState(vehicle) then
					...
					setElementData(vehicle, "vehicle.engine", false)
					exports.nlrp_alert:showAlert("error", "A jármű motorja sérült, ezért a jármű lefulladt!")
				end
			end
		end
	end
...
render.speedo = function (x, y)
	if occupiedVehicle and isElement(occupiedVehicle) and (getPedOccupiedVehicleSeat(localPlayer) == 0 or getPedOccupiedVehicleSeat(localPlayer) == 1) then
        ...
        setElementData(getLocalPlayer(), "hud.speedo.x", x)
        setElementData(getLocalPlayer(), "hud.speedo.y", y)
	...
	--	dxDrawText("Speed: " .. math.floor(tostring(vehicleSpeedInMPH)) .. " Km/h [" .. getVehicleGear(occupiedVehicle, vehicleSpeedInMPH) .. "]", x2 + respc(15), y2, 0, y2 + respc(40), tocolor(0, 0, 0), 0.75, LEDCalculator24, "left", "center")

	--	dxDrawText(math.floor((vehicleDistance + currDistanceValue) * 10) / 10 .. " km", x2, y2, x2 + displaySizeX - 60, y2 + 440, tocolor(255, 255, 255), 0.75, LEDCalculator24, "right", "center")
		
	--	dxDrawRectangle(x2 + respc(10), y2 + respc(40), displaySizeX - respc(20), 2, tocolor(0, 0, 0))

   	--	dxDrawText("Fuel: " .. math.floor(currentFuel * 10) / 10 .. "/" .. math.floor(fuelTankSize * 10) / 10 .. " L", x2 + 180, y2 + 205, 0, y2 + respc(40), tocolor(255, 255, 255), 0.75, LEDCalculator24, "left", "center")
	...
	--	dxDrawRectangle(x2 + respc(10), y2 + respc(70), displaySizeX - respc(20), 2, tocolor(0, 0, 0))
		...
		if vehtype == "Automobile" then
			...
			--	dxDrawText("", x2, y2 + respc(73), x2 + displaySizeX, 0, tocolor(0, 0, 0), 0.75, LEDCalculator24, "center", "top")
			else
			--	dxDrawText("", x2, y2 + respc(73), x2 + displaySizeX, 0, tocolor(0, 0, 0), 0.75, LEDCalculator24, "center", "top")
			end
		else
	...
	--	dxDrawRectangle(x2 + respc(10), y2 + respc(110), displaySizeX - respc(20), 2, tocolor(0, 0, 0))
		...
		if getElementHealth(occupiedVehicle) <= 550 then
			-- dxDrawText("- CHECK ENGINE -", x2, y2 + displaySizeY - respc(40), x2 + displaySizeX, y2 + displaySizeY, tocolor(255, 0, 0), 0.75, LEDCalculator24, "center", "center")
		elseif tempomatSpeed then
			local speed = math.floor(getMilesByKilometers(tempomatSpeed))

		--	dxDrawText("- TEMPOMAT: " .. speed .. " Km/H -", x2 + 1, y2 + 1 + displaySizeY - respc(40), x2 + 1 + displaySizeX, y2 + 1 + displaySizeY, tocolor(0, 0, 0), 0.75, LEDCalculator24, "center", "center")
		--	dxDrawText("- TEMPOMAT: " .. speed .. " Km/H -", x2, y2 + displaySizeY - respc(40), x2 + displaySizeX, y2 + displaySizeY, tocolor(50, 179, 239), 0.75, LEDCalculator24, "center", "center")
		end
		...
		if vehicleIndicators[occupiedVehicle] then
			...
			--	drawSpeedoIconSprite(x3 + 245, y3 + 75, 0, "lefticons_glow.png")
			end

			if vehicleIndicators[occupiedVehicle].right and not vehicleIndicatorStates[occupiedVehicle] then
			--	drawSpeedoIconSprite(x3 + 330, y3 + 75, 0, "righticons_glow.png") --
			end
		...
		--	drawSpeedoIconSprite(x3 + 270, y3 + 170, 36, "lefticons_glow.png")
		end

		if handBrakeState then
		--	drawSpeedoIconSprite(x3 + 330, y3 + 170, 36 * 2, "lefticons_glow.png")
		end
		...
		if vehtype == "Automobile" and not nonSeatBeltVehicles[vehicleModel] then
			if not seatBeltState then
				...
				--	drawSpeedoIconSprite(x3 + 300, y3 + 170, 36 * 3, "lefticons_glow.png")
				end
			end
		end
	...
	if isElement(vehicle) and (getVehicleType(vehicle) or "N/A") == "Automobile" then
		...
		if not nonSeatBeltVehicles[model] then
			if getTickCount() - lastSeatBeltStateChange >= 5000 then
				...
				if seatBelt then
					exports.nlrp_chat:sendLocalMeAction(localPlayer, "kicsatolja a biztonsági övét.")
					playSound(":nlrp_assets/audio/vehicles/ovki.ogg", false)
				else
					exports.nlrp_chat:sendLocalMeAction(localPlayer, "becsatolja a biztonsági övét.")
					playSound(":nlrp_assets/audio/vehicles/ovbe.ogg", false)
				end

				lastSeatBeltStateChange = getTickCount()
			else
				exports.nlrp_hud:showAlert("error", "Csak 5 másodpercenként csatolhatod ki/be az öved.")
			end
		end
	end
end
			...
			if dataName == "vehicle.fuel" then
				if dataValue then
					...
					if vehicleFuel <= 0 then
						...
						exports.nlrp_alert:showAlert("error", "Kifogyott az üzemanyag!")
					else
						outOfFuel = false
					end
				end
...
function processIndicatorEffect(vehicle)
	if isElement(vehicle) then
		...
		if vehicle == getPedOccupiedVehicle(localPlayer) and vehicleIndicatorStates[vehicle] then
			playSound(":nlrp_vehicles/files/turnsignal.ogg")
		end
