...
dxSetTextureEdge(mapPicture, "border", tocolor(124,167,208, 255))
...
local gpsRoutePos = {}
local gpsLineColor = tocolor(183,255,0)
...
render.minimap = function (x, y)
	...
	if playerDimension == 0 and playerPosZ <= 10000 then
		...
		for k = 1, #blips do
			...
			if v then
				if getBlipIcon ( v ) == 56 then
					local x, y = getElementPosition(v)
					
					blipCount = blipCount + 1

					renderBlip("package.png", x, y, playerPosX, playerPosY, 14.5, 14.5, tocolor(255,255,255,255), true, cameraRotZ, blipCount)
		...
		-- ** GPS Lokátor
		dxDrawImage(panelPosX + 3, panelPosY + panelHeight + 3, resp(30) - 6, resp(30) - 6, ":nlrp_assets/images/map/location.png")
...
function renderTheBigmap()
	...
	if playerDimension == 0 and playerPosZ <= 10000 then
		...
		-- ** GPS Lokátor
		dxDrawImage(panelPosX + 12, panelPosY + 12, respc(70) - 24, respc(70) - 24, ":nlrp_assets/images/map/location.png")
				...
				if isElement(activeBlip) then
					if getElementType(activeBlip) == "player" then
						if getElementData(activeBlip, "adminDuty") then
							text = "[" .. exports.nlrp_core:getPlayerAdminTitle(activeBlip) .. "] " .. getElementData(activeBlip, "visibleName"):gsub("_", " ") .. " (" .. getElementData(activeBlip, "playerID") .. ")"
						else
