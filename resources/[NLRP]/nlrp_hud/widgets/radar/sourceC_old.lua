...
render.minimap = function (x, y)
	...
	if localDimension == 0 and localZ <= 10000 then
		...
		dxDrawImage(minimapPosX + 3, minimapPosY - size32px + 3, size32px - 6, size32px - 6, ":nlrp_assets/images/map/location.png", 0, 0, 0, tocolor(255, 255, 255, 230))
...
function renderTheBigmap()
	...
	if getElementDimension(localPlayer) == 0 and localZ <= 10000 then
		...
		dxDrawImage(bigmapPosX + 10 + 3.75, bigmapPosY + bigmapHeight - y_offset + 3, zoneLineHeight - 6, zoneLineHeight - 6, ":nlrp_assets/images/map/location.png", 0, 0, 0, tocolor(255, 255, 255, 230))
				...
				elseif getElementType(blipElement) == "player" and canSeePlayers then
					if getElementData(blipElement, "loggedIn") then
						...
						if getElementData(blipElement, "adminDuty") then
							visibleBlipTooltip = "[" .. exports.nlrp_core:getPlayerAdminTitle(blipElement) .. "] " .. utf8.gsub(utf8.gsub(getElementData(blipElement, "acc.adminNick"), "#%x%x%x%x%x%x", ""), "_", " ") .. " (" .. getElementData(blipElement, "playerID") .. ")"
						end
