...
function loadFonts()
	local fonts = {
		Roboto = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", resp(12), false, "antialiased"),
		RobotoB = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", resp(12), false, "antialiased"),
		gtaFont = exports.nlrp_assets:loadFont("gtaFont2.ttf", resp(40), false, "cleartype"),
		chaletcomprime = exports.nlrp_assets:loadFont("chaletcomprime.ttf", resp(24), false, "cleartype"),
		RobotoL = exports.nlrp_assets:loadFont("Roboto-Light.ttf", resp(14), false, "antialiased"),
		RobotoL18 = exports.nlrp_assets:loadFont("Roboto-Light.ttf", respc(18), false, "antialiased"),
		Roboto14 = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", respc(14), false, "antialiased"),
		CenturyBold = exports.nlrp_assets:loadFont("CenturyGothicBold.ttf", respc(14), false, "cleartype"),
		CenturyRegular = exports.nlrp_assets:loadFont("CenturyGothicRegular.ttf", resp(26), false, "cleartype"),
		CenturyBold2 = exports.nlrp_assets:loadFont("CenturyGothicBold.ttf", resp(13), false, "cleartype"),
		LEDCalculator = exports.nlrp_assets:loadFont("LEDCalculator.ttf", 12, false, "cleartype"),
		gtaFont2 = exports.nlrp_assets:loadFont("CapitalDaren.ttf", respc(35), false, "antialiased"),
		numberplateFont = exports.nlrp_assets:loadFont("LicensePlate.ttf", resp(22), false, "cleartype"),
        LEDCalculator8 = exports.nlrp_assets:loadFont("LEDCalculator.ttf", respc(10), false, "cleartype"),
		RobotoL32 = exports.nlrp_assets:loadFont("Roboto-Light.ttf", respc(32), false, "antialiased"),
		LEDCalculator24 = exports.nlrp_assets:loadFont("LEDCalculator.ttf", respc(24), false, "cleartype"),
		Roboto18 = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", resp(18), false, "antialiased"),

		OpenSansRegular16 = exports.nlrp_assets:loadFont("OpenSans-Regular.ttf", respc(16), false, "antialiased"),
		OpenSansLight16 = exports.nlrp_assets:loadFont("OpenSans-Light.ttf", respc(16), false, "antialiased"),

		RobotoBolder = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", resp(20), false, "antialiased"),
		RobotoRegular = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", resp(20), false, "antialiased"),
		RobotoBolder40 = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", resp(40), false, "antialiased"),
		RobotoLighter40 = exports.nlrp_assets:loadFont("Roboto-Light.ttf", resp(40), false, "antialiased"),
	}
...
local plateback = ":nlrp_textures/files/plateback.png"
...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedRes)
		if startedRes == getThisResource() then
			...
			local textures = getResourceFromName("nlrp_textures")
			if textures and getResourceState(textures) == "running" then
				if isElement(plateback) then
					destroyElement(plateback)
				end

				plateback = dxCreateTexture(":nlrp_textures/files/plateback.png")
			end
		...
		elseif getResourceName(startedRes) == "nlrp_textures" then
			if isElement(plateback) then
				destroyElement(plateback)
			end
...
widgets = {
	...
	speedo = {
		sizeX = respc(300),
		sizeY = respc(90),
		posX = screenX - respc(310),
		posY = screenY - respc(100),
...
render.money = function (x, y)
	...
	if tonumber(renderData.currentMoney) < 0 then
		moneyTextWidth = dxGetTextWidth("- " .. formatNumber(renderData.currentMoney) .. " Ft", 0.5, gtaFont)
	else
		moneyTextWidth = dxGetTextWidth(formatNumber(renderData.currentMoney) .. " Ft", 0.5, gtaFont)
	end
	...
	if renderData.moneyChangeTick and getTickCount() >= renderData.moneyChangeTick and getTickCount() <= renderData.moneyChangeTick + 5000 then
		...
		elseif tonumber(currentMoney) > 0 then
			currentMoney = "#71BA51+ " .. formatNumber(currentMoney)
		else
			currentMoney = 0
		end
		...
		if tonumber(currentMoney) < 0 then
			currentMoney = "#d75959" .. formatNumber(math.abs(currentMoney))
		elseif tonumber(currentMoney) > 0 then
			currentMoney = "#71BA51" .. formatNumber(math.abs(currentMoney))
		else
			currentMoney = 0
		end
	...
	dxDrawBorderText(moneyForDraw .. " Ft", x, y, x + widgets.money.sizeX, y + widgets.money.sizeY, tocolor(255, 255, 255), tocolor(0, 0, 0, 200), 0.5, gtaFont, "right", "center", false, false, false, true)
end
...
render.actionbar = function (x, y)
	...
	if lastActionBarX ~= x or lastActionBarY ~= y then
		lastActionBarX = x
		lastActionBarY = y
		exports.nlrp_inventory:changeItemStartPos(x, y)
	end
...
function processActionBarShowHide(state)
	if actionBarState ~= state then
		...
		exports.nlrp_inventory:processActionBarShowHide(state)
	end
end
...
function drawSideGUI()
	...
	if renderData.trashTrayAnimToAlpha > 0.1 then
		...
		k = drawRightSideIcon(k, renderData.canEditorMusic, "musicon", "musicoff", "togMusic", renderData.canEditorMusic and "Háttérzene kikapcsolása" or "Háttérzene bekapcsolása", {183,255,0})
		k = drawRightSideIcon(k, renderData.canEditorSoundEffects, "soundon", "soundoff", "togSounds", renderData.canEditorSoundEffects and "Hangeffektek kikapcsolása" or "Hangeffektek bekapcsolása", {183,255,0})
		...
		if #renderData.selectedHUD > 0 and not renderData.resizing then
			...
			k = drawRightSideIcon(k, true, "reset", "reset", "resetWidget", "Kijelölt elem(ek) visszaállítása", {183,255,0})
...
function renderTheLogoSARP()
	...
	if a > 0 then
--		dxDrawImage(theX - size2 / 2 - x * s, theY - size2 / 2 - y * s, size2, size2, "files/logo1g.png", 0, 0, 0, tocolor(183,255,0, 225 * a * renderData.alphaMul))
--		dxDrawImage(theX - size2 / 2 + x * s, theY - size2 / 2 + y * s, size2, size2, "files/logo2g.png", 0, 0, 0, tocolor(183,255,0, 225 * a * renderData.alphaMul))
	end
end
	...
	function (cmd, key)
		...
		if visibleHUD then
			if not renderData.editorActive then
				...
				playSoundEx(":nlrp_assets/audio/interface/7.ogg")
			end
		end
	end
)
...
function processExitEditor(save)
	if not renderData.trayBarActive then
		...
		playSoundEx(":nlrp_assets/audio/interface/10.ogg")
		...
		if isHudVisible() then
			if renderData.editorActive then
				...
				if not renderData.screenSrc then
					...
					dxSetShaderValue(renderData.screenShader, "blurStrength", 10)
					dxSetShaderValue(renderData.screenShader, "colorize", 50/255, 50/255, 50/255)
					...
					if renderData.selection then
						if math.floor(math.abs(cursorX - renderData.selection[1]) + math.abs(cursorY - renderData.selection[2])) > 2 then
							...
							if #renderData.selectedHUD > 0 then
								playSoundEx(":nlrp_assets/audio/interface/3.ogg")
							end
						end
					end
				end
...
local function encrypt(str)
	...
	table.insert(str2, string.char(28) .. "nlrp_DATA_FILE" .. randomBytes(8))
			...
			if loadedData then
				...
				if loadedData.settings.screenResolution then
					...
					if screenRes[1] and screenRes[2] and (tonumber(screenRes[1]) ~= screenX or tonumber(screenRes[2]) ~= screenY) then
						resetHudElement("all")
						savePositions()
						outputChatBox(exports.nlrp_core:getServerTag("info") .. "Új képernyőfelbontás észlelve! A HUD visszaállításra került az alapértelmezett állapotába.", 255, 255, 255, true)
					end
				end
