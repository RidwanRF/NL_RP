...
addEventHandler("onClientResourceStart", getRootElement(),
	function (res)
		if res == getThisResource() then
			local nlrp_weather = getResourceFromName("nlrp_weather")

			if nlrp_weather and getResourceState(nlrp_weather) == "running" then
				weather = getElementData(getResourceRootElement(nlrp_weather), "serverWeather")
			else
				weather = {1, 1}
			end
		else
...
function drawPhoneComponent(x, y, v)
	...
	elseif v[1] == "lockscreen" then
        ...
        local temp_c = math.floor(36 * 10) / 10
        ...
        dxDrawText(utf8.lower("Napos"), x + resp(10), y + resp(65), x + resp(170), y + resp(165), tocolor(230, 190, 130), 0.75, RobotoL18, "center", "bottom")
...
addEventHandler("onClientRender", getRootElement(), phone)
