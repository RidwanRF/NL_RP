...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedRes)
		if getResourceName(startedRes) == "nlrp_core" or startedRes == getThisResource() then
			maxPlayers = getElementData(getResourceRootElement(getResourceFromName("nlrp_core")), "server.maxPlayers") or 0
		end
	end
)
	...
	for k, v in ipairs(getElementsByType("player")) do
		if getElementData(v, "loggedIn") then
			...
			if adminDuty then
				...
				local adminTitleColor = exports.nlrp_core:getAdminLevelColor(adminLevel) or "#FFFFFF"
			...
			scoreboardPlayers[v] = {
				...
				level = exports.nlrp_core:getLevel(v)
			}
		else
...
--	dxDrawImage(panelX + logoOffset, panelY + logoOffset, logoSize, logoSize, ":nlrp_inventory/files/logok.png", 0, 0, 0, tocolor(183,255,0, alpha255))
	dxDrawText("Játékos lista", panelX + logoOffset*-10 + logoSize, panelY, 0, panelY + headerHeight, whiteColor, 1, RobotoL18, "left", "center")
	dxDrawText("#eaeaeaOnline: #b7ff00" .. playersCount .. "#eaeaea / #b7ff00" .. maxPlayers, panelX, panelY, panelX + panelWidth - respc(10), panelY + headerHeight, tocolor(200, 200, 200, alpha255), 0.75, chaletcomprime, "right", "center", false, false, false, true)

	dxDrawText("#0039a6ORFK: #b9b9b9" .. getOnlineFactionMembers(1) .. " #ffffff/#0039a6 " .. getOnlineFactionMembers(1,true) .. "  #ffffff|  #dc281eOMSZ: #b9b9b9" .. getOnlineFactionMembers(5) .. " #ffffff/#dc281e " .. getOnlineFactionMembers(5,true) .. "  #ffffff|  #fcff00Szervíz: #b9b9b9" .. getOnlineFactionMembers(3) .. " #ffffff / #fcff00" .. getOnlineFactionMembers(3,true),panelX+panelWidth/2,panelY+headerHeight/2,nil,nil,tocolor(200,200,200,222),1,Roboto,"center","center",false,false,false,true)
...
function getOnlineFactionMembers(groupID,duty)
	local count = 0
	for k,v in ipairs(getElementsByType("player")) do
		if exports.nlrp_groups:isPlayerInGroup(v,groupID) then
			if duty then
	...
	function (button, state)
		if state == "down" then
			if getElementData(localPlayer, "loggedIn") then
				...
				playSound(":nlrp_assets/audio/scoreboard/popup.wav")
			end
