...
addEventHandler("onClientRender", root, function()
    ...
    if panelState == "meals" then
        ...
        dxDrawImage(math.floor(panelMealsX + 3), math.floor(panelMealsY + 3), 24, 24, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(50, 179, 239))
    ...
    elseif panelState == "order" then
        ...
        dxDrawImage(math.floor(panelOrdersX + 3), math.floor(panelOrdersY + 3), 24, 24, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(50, 179, 239))
            ...
            elseif activeButton == "food:serve" then
                if createdMeals[selectedMeal] > 0 then
                    if createdDishes[localPlayer] and isElement(createdDishes[localPlayer]) then
                        exports.nlrp_hud:showAlert("error", "Már van a kezedben egy tálca")
                        return 
                    end

                    createdMeals[selectedMeal] = createdMeals[selectedMeal] - 1
                    triggerServerEvent("nlrp_serverS:attachDish", localPlayer, restaurant["meals"][selectedMeal][1])
                    
                else
                    exports.nlrp_hud:showAlert("error", "Nincs ebből a menüből elkészítve")
                end
            end
        ...
        elseif panelState == "order" then
            if activeButton == "food:give" then
                if createdDishes[localPlayer] and isElement(createdDishes[localPlayer]) then
                    ...
                    for k, v in pairs(createdOrders[selectedOrder]) do
                        if dishID == restaurant["meals"][v[1]][1] then
                            ...
                            if table.size(createdOrders[selectedOrder]) <= 0 then
                                ...
                                exports.nlrp_hud:showAlert("info", "Sikeresen kiszolgáltad a vevőt", "" .. price .. "$-t kerestél és " .. tip .. "$ borravalót kaptál")
                                exports.nlrp_core:giveMoney(localPlayer, price + tip)
                    ...
                    if not founded then
                        exports.nlrp_hud:showAlert("error", "A vevő nem rendelt ilyen menüt")
                        triggerServerEvent("nlrp_serverS:dettachDish", localPlayer)
                    end
                else
                    exports.nlrp_hud:showAlert("error", "Nincs nálad tálca")
                end
            end
        end
    end
