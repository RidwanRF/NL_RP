...
function startJob()
	...
	if getElementModel(localPlayer) ~= restaurant["skin"] then		
		exports.nlrp_hud:showAlert("error", "Nem vagy munkaruhában", "Menj a kijelölt helyre, és öltözz át!")
		return
	end
    ...
    exports.nlrp_hud:showAlert("info", "Sikeresen elkezdted a felszolgálást!", "Várj a vevőkre, majd kérdezd meg tőlük, hogy mit rendelnek")
	jobActive = true
	createOrderPed(getElementInterior(localPlayer))
end
...
function stopJob()
	if createdDishes[localPlayer] and isElement(createdDishes[localPlayer]) then
		triggerServerEvent("nlrp_serverS:dettachDish", localPlayer)
	end	

	exports.nlrp_hud:showAlert("info", "Befejezted a felszolgálást!", "Most már visszaöltözhetsz a hétköznapi ruhádba")
	jobActive = false
...
function createOrderPed(currentInterior)
	...
	exports.nlrp_hud:showAlert("warning", "Új vevő érkezett!")
	--setTimer(function()
			...
			for k, v in pairs(createdPeds) do
				if clickedWorld == v then
					if jobActive then
						...
						exports.nlrp_hud:showAlert("error", "Éppen nem dolgozol", "Menj a műszakvezetőhöz és jelentkezz munkára")
					end
				end
			end
		end
				...
				if key == "e" and press then
					if changeTick + cooldownTick <= getTickCount() then
						if not jobActive then
							local restaurant = restaurants[getElementInterior(localPlayer)]
							if getElementModel(localPlayer) == restaurant["skin"] then
								triggerServerEvent("nlrp_serverS:changeModel", localPlayer, oldCloth)
								--print("asd2")
							else
								oldCloth = getElementModel(localPlayer)
								--print("asd")
								triggerServerEvent("nlrp_serverS:changeModel", localPlayer, restaurant["skin"])
							end
							changeTick = getTickCount()
						else
							exports.nlrp_hud:showAlert("error", "Munka közben nem öltözhetsz át")
						end
					else
						exports.nlrp_hud:showAlert("error", "Várj 3 másodpercet mielőtt újra átöltözöl")
					end
				end
			end
		end
...
registerEvent("nlrp_serverC:attachDish", root, function(objID)
	...
	exports.nlrp_boneattach:attachElementToBone(createdDishes[source],source, 12, 0.45, -0.03, 0.15, 334, 22 + 100, -19 + 90 + 30)

	exports.nlrp_controls:toggleControl({"jump", "sprint", "fire"}, false) 
end)

registerEvent("nlrp_serverC:dettachDish", root, function()
	if isElement(createdDishes[source]) then
		destroyElement(createdDishes[source])
		createdDishes[source] = nil

		exports.nlrp_controls:toggleControl({"jump", "sprint", "fire"}, true) 
	end
end)