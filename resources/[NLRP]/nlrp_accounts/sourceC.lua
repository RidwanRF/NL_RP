...
function loadFonts()
	local fonts = {
		Roboto18 = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", 18, false, "cleartype"),
		Roboto18L = exports.nlrp_assets:loadFont("Roboto-Light.ttf", 18, false, "cleartype"),
		Roboto32B = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", 32, false, "cleartype"),
		SARPFont = exports.nlrp_assets:loadFont("SARP.ttf", 32, false, "cleartype"),
	}
...
function saveLoginFiles(username, password, remember)
	if fileExists("@nlrp_loginremember.xml") then
		fileDelete("@nlrp_loginremember.xml")
	end
	
	local loginRememberFile = xmlCreateFile("@nlrp_loginremember.xml", "logindatas")
...
function loadLoginFiles()
	local loginRememberFile = xmlLoadFile("@nlrp_loginremember.xml")
...
--			dxDrawImage(0, 0, screenX, screenY, ":nlrp_hud/files/lights.png", 0, 0, 0, tocolor(255, 255, 255, 255 * FFT[1]))
		end
	end
	...
	dxDrawText(exports.nlrp_core:formatDate("Y/m/d h:i:s", "'", tostring(banData.banTimestamp)), banPanelPosX + 10, y, banPanelPosX + banPanelWidth - 10, y + 30, tocolor(150, 150, 150), 0.65, Roboto18, "right", "center")
	y = y + 30

	dxDrawText("Lejár", banPanelPosX + 10, y, banPanelPosX + banPanelWidth - 10, y + 30, tocolor(255, 255, 255), 0.75, Roboto18, "left", "center")
	dxDrawText(exports.nlrp_core:formatDate("Y/m/d h:i:s", "'", tostring(banData.expireTimestamp)), banPanelPosX + 10, y, banPanelPosX + banPanelWidth - 10, y + 30, tocolor(150, 150, 150), 0.65, Roboto18, "right", "center")
	y = y + 30

	dxDrawRectangle(banPanelPosX + 10, y, banPanelWidth - 20, 2, tocolor(255, 255, 255, 50))
	y = y + 10

	dxDrawText("", banPanelPosX + 10, y, banPanelPosX + banPanelWidth - 10, y + 50, tocolor(183, 255, 0), 1, Roboto18L, "center", "center")
end
...
--				dxDrawImage(0, 0, screenX, screenY, ":nlrp_hud/files/lights.png", 0, 0, 0, tocolor(255, 255, 255, 255 * FFT[1]))
		...
		dxDrawText("NextLevel Roleplay", x + 10, y, x + sx - 10, y + 50, tocolor(255, 255, 255), 1, Roboto18L, "center", "center")
		y = y + 50
		...
		drawButton("registerEnd", "Regisztráció", x + 10, y, sx - 20, 40, {183,255,0})
...
--				dxDrawImage(0, 0, screenX, screenY, ":nlrp_hud/files/lights.png", 0, 0, 0, tocolor(255, 255, 255, 255 * FFT[1]))
		...
		drawButtonSlider("rememberMe", renderData.rememberMe, x + 10, y, 35, {50, 50, 50, 255}, {183,255,0, 255})
		dxDrawText("Jegyezzen meg", x + 20 + 64, y, 0, y + 35, tocolor(255, 255, 255), 0.65, Roboto18, "left", "center")
		y = y + 40

		drawButton("login", "Bejelentkezés", x + 10, y, sx - 20, 40, {183,255,0})
		...
		drawButton("register", "Regisztráció", x + sx - 10 - 125, y, 125, 40, {231,76,60})
...
function onClientCharacter(character)
	...
	if characterMaking.ageInputStart and tickCount >= characterMaking.ageInputStart and string.find(string.lower(character), "[0-9]") then
		...
		if utfLen(characterMaking.charAge) < 1 or utfLen(characterMaking.charAge) > 2 then
			...
			exports.nlrp_hud:showInfobox("warning", "A karaktered kora nem megfelelő! (12-90)")
		end
	end
end
					...
					if checkPassword(password) then
						if password == password2 then
							if email and email ~= "" and utf8.find(email, "@") and utf8.find(email, ".") then
								...
								exports.nlrp_hud:showInfobox("error", "Adj meg egy valós email címet!")
							end
						else
							exports.nlrp_hud:showInfobox("error", "A jelszavak nem egyeznek!")
						end
					else
						exports.nlrp_hud:showInfobox("error", "A jelszónak min. 6 karakter, Kis-és Nagybetű és legalább egy numerikus karaktert (szám 0-9) kell tartalmaznia!")
					end
				else
					exports.nlrp_hud:showInfobox("error", "A felhasználónévnek minimum 5, maximum 18 karakterből kell állnia!")
				end
			elseif selected[1] == "login" and not renderData.inputDisabled then
				...
				if utfLen(username) >= 5 and utfLen(username) <= 18 then
					if utfLen(password) >= 6 and utfLen(password) <= 32 then
						...
						exports.nlrp_hud:showInfobox("error", "A jelszónak minimum 6, maximum 32 karakterből kell állnia!")
					end
				else
					exports.nlrp_hud:showInfobox("error", "A felhasználónévnek minimum 5, maximum 18 karakterből kell állnia!")
				end
...
function processMinuteTimer()
	...
	if playTimeForPayday + 1 == 60 then
		...
		triggerServerEvent("onPayDay",resourceRoot,localPlayer,pFactions)
	else
...
addEventHandler("onSuccessLogin", getRootElement(),
	function (characters, pwchange)
		exports.nlrp_hud:showInfobox("success", "Sikeresen bejelentkeztél!")
...
local logoTexture = dxCreateTexture("files/logo2.png")
local logoSize = 128 * (1 / 75)
local logoRotation = 90
...
--			dxDrawImage(0, 0, screenX, screenY, ":nlrp_hud/files/lights.png", 0, 0, 0, tocolor(255, 255, 255, 255 * FFT[1]))
	...
	for i = 1, #pedData do
		if isElement(localCharacters[i]) then
			...
			if selectedChar == i then
				dxDrawMaterialLine3D(pedX, pedY + logoSize / 2, pedZ, pedX, pedY - logoSize / 2, pedZ, logoTexture, logoSize, tocolor(183, 255, 0), pedX, pedY, pedZ + 10)
			else
	...
	if forcePasswordChange then
		...
		dxDrawRectangle((screenX - nameWidth) / 2, screenY - 120, nameWidth, 2, tocolor(183,255,0))

		if #pedData > 1 then
			dxDrawText("Nyomj #B7FF00ENTER#FFFFFF-t a kiválasztáshoz.", 0, 0, screenX, screenY - 80, tocolor(255, 255, 255), 1, Roboto18, "center", "bottom", false, false, false, true)
...
addEventHandler("passwordChangeResult", getRootElement(),
	function (ok)
		if ok then
			exports.nlrp_hud:showInfobox("success", "Sikeres jelszóváltás! Mostantól ezzel léphetsz be.")
			...
			exports.nlrp_hud:showInfobox("error", "Nem adhatod meg a régi jelszavad!")
		end
	end
)
		...
		if forcePasswordChange then
			...
			elseif forcePasswordChange == "again" then
				if firstPw == renderData.newPw then
					...
					renderData.newPwHidden = ""
					forcePasswordChange = true
					exports.nlrp_hud:showInfobox("error", "A két jelszó nem egyezik!")
				end
			elseif checkPassword(renderData.newPw) then
				...
				exports.nlrp_hud:showInfobox("error", "A jelszó nem megfelelő!")
			end
		elseif not renderData.charSelectInterpolation then
			...
			elseif key == "enter" and not renderData.charGotSelected and selectedChar then
				...
				if pedData[selectedChar].charID then
					...
					exports.nlrp_loader:showTheLoadScreen(spawnTime - 200, {"Karakter inicilizálása", "Karakter adatok betöltése", "Felhasználói felület betöltése", "Szinkronizációk folyamatban"})
...
addEventHandler("onPlayerCharacterMade", getRootElement(),
	function (characters)
		if #pedData > 0 then -- ha új karakter létrehozására ment, tehát már volt fiókja
			...
			exports.nlrp_hud:showInfobox("success", "Sikeres regisztráció! Mostmár bejelentkezhetsz.")
...
addEventHandler("checkNameCallback", getRootElement(),
	function (rows)
		if rows < 1 then
			...
			exports.nlrp_hud:showInfobox("error", "Már van ilyen nevű karakter a szerveren!")
		end
...
addEventHandler("onClientRegister", getRootElement(),
	function (rows)
		if rows > 0 then
			exports.nlrp_hud:showInfobox("error", "Már létezik account ehhez a géphez társítva\nVárj 10 másodpercet az újrapróbálkozáshoz!")
		else
			renderData.canUseFakeInputs = false
	...
	function ()
		...
		elseif not characterMaking.finalFadeEnd or tickCount < characterMaking.finalFadeEnd then
			...
			if characterMaking.ageInputHide and characterMaking.finalFade and tickCount >= characterMaking.ageInputHide and tickCount < characterMaking.finalFade then
				...
				--outputDebugString ( progress .."% A Faszom" )
				...
				if progress >= 1 then
					...
					if not isElement(taxiStopColShape) then
						spawnAIs(2)
						taxiStopColShape = createColSphere(1474.7958984375, -1734.1341552734, 13.160967826843, 3)
					end
				end
			end
			...
			if characterMaking.cutsceneEnd and tickCount >= characterMaking.cutsceneEnd then
				...
				if progress >= 1 then
					...
					if #pedData == 0 then -- ** ha még nincs karaktere, tehát most regisztrál
						...
						exports.nlrp_hud:showInfobox("info", "Regisztrálj magadnak egy felhasználót a játék elkezdéséhez.")
...
addCommandHandler("gp",
    function()
        local x,y,z = getElementPosition( characterMaking.taxiCar )
		outputChatBox(x..", "..y..", "..z)
    end
)
...
function drawLogoAnimation(x, y, fftMul)
		...
		for i = 1, #logoNodes do
			for j = 1, #logoNodes[i] do
				...
				for k = 1, #mainNode do
					...
					--	dxDrawLine(x + node[1], y + node[2], x + x2, y + y2, tocolor(183, 255, 0), 1)
					...
					elseif node[6] then
						dxDrawLine(x + node[1], y + node[2], x + node[3], y + node[4], tocolor(183, 255, 0), 1)
					end
				end
			end
		end
	...
	dxDrawImage(x-45, y, 351, 225, "files/logo.png", 0, 0, 0, tocolor(183,255,0))
end
