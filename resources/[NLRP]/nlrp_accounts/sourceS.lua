						...
						if isElement(sourcePlayer) then
							...
							if result then
								if result.username == username then
									exports.nlrp_hud:showInfobox(sourcePlayer, "error", "Ez a felhasználónév már foglalt!")
								elseif result.email == email then
									exports.nlrp_hud:showInfobox(sourcePlayer, "error", "Ez az e-mail cím már használatban van!")
								end
							else
								dbQuery(
									function (qh)
			...
			if client and username and password then
				...
				dbQuery(
					function(qh, sourcePlayer)
						...
						if errno then
							exports.nlrp_hud:showInfobox(sourcePlayer, "error", errno .. "\nVárj 10 másodpercet az újrapróbálkozáshoz.")
						else
							dbQuery(
								function(qh, account)
									local result, rows = dbPoll(qh, 0)
									...
									setElementData(sourcePlayer, "adminKicks", account.adminKicks)
									setElementData(sourcePlayer, "adminBans", account.adminBans)
									setElementData(sourcePlayer, "adminJails", account.adminJails)
									setElementData(sourcePlayer, "adminDutyTime", account.adminDutyTime)
									setElementData(sourcePlayer, "buzievagy", account.buzievagy)
	...
	function (player, charID, data)
		if isElement(source) and isElement(player) then
			if player and charID and data then
				...
				setElementData(player, "char.slotCoins", data.coins)

				setElementData(player, "char.Money", data.money)
				setElementData(player, "char.PP", data.pp)
				...
				if data.jailed then
					exports.nlrp_groupscripting:loadPlayerJail(player, data.jailed)
				end
				...
				--exports.nlrp_paintjob:syncVehStickers(player)
				exports.nlrp_vehicles:loadPlayerVehicles(charID, player)

				exports.nlrp_inventory:loadItems(player, charID)
...
exports.nlrp_admin:addAdminCommand("triggersave", 10, "Játékosok mentése (debughoz)")
...
function autoSavePlayer(player, loggedOut)
	...
	if getElementData(player, "loggedIn") then
		...
		local datas = {
			...
			["coins"] = getElementData(player, "char.slotCoins") or 0,
			--// Vállalkozás
			["company"] = tonumber(getElementData(player, "char.CompanyID") or 0),
			["company_rank"] = tonumber(getElementData(player, "char.CompanyRank") or 0),
			["company_tax_payed"] = tonumber(getElementData(player, "char.CompanyTaxPayed") or 0),
		}
		...
		if not loggedOut then
			dbExec(connection, "UPDATE accounts SET buzievagy = ?, adminJailTime = ?, adminDutyTime = ?, adminBans = ?, adminJails = ?, adminKicks = ? WHERE accountID = ?; UPDATE characters SET " .. table.concat(columns, ", ") .. " WHERE charID = ?",(getElementData(player, "buzievagy") or 0), (getElementData(player, "acc.adminJailTime") or 0),(getElementData(player, "adminDutyTime") or 0),(getElementData(player, "adminBans")or 0),(getElementData(player, "adminJails")or 0),(getElementData(player, "adminKicks")or 0), getElementData(player, "acc.dbID"), unpack(columnValues))
		else
			...
			dbExec(connection, "UPDATE accounts SET buzievagy = ?, adminJailTime = ?, adminDutyTime = ?, adminBans = ?, adminJails = ?, adminKicks = ?, online = 'N', helperLevel = ? WHERE accountID = ?; UPDATE characters SET lastOnline = ?, " .. table.concat(columns, ", ") .. " WHERE charID = ?", (getElementData(player, "buzievagy") or 0), (getElementData(player, "acc.adminJailTime") or 0),(getElementData(player, "adminDutyTime") or 0),(getElementData(player, "adminBans")or 0),(getElementData(player, "adminJails")or 0),(getElementData(player, "adminKicks")or 0), helperLevel, getElementData(player, "acc.dbID"), getRealTime().timestamp, unpack(columnValues))
		end
	end
end
...
exports.nlrp_admin:addAdminCommand("payday", 9, "Fizetés/adó kiküldése magunknak (debughoz)")
addCommandHandler("payday",
	function (player, command)
		if getElementData(player, "acc.adminLevel") == 9 then
			triggerEvent("onPayDay",resourceRoot,player)
		end
	end
)
		...
		if getElementData(player, "loggedIn") then
			...
			if #playerGroups > 0 then
				for k,v in ipairs(playerGroups) do
					local rankId, rankName, rankPayment = exports.nlrp_groups:getPlayerRank(player, v.factionID)
			...
			-- Jövedelem adó
			local incomeTax = getElementData(player, "char.bankMoney") or 0

			if incomeTax > 0 then
				incomeTax = incomeTax * 0.0105
			...
			-- Jármű adó
			local vehicleTax = exports.nlrp_vehicles:getPlayerVehiclesCount(charID) * (taxesCache["vehicleTax"] and taxesCache["vehicleTax"][3] or 16175)
			
			--outputChatBox(exports.nlrp_vehicles:getPlayerVehiclesCount(charID))
			
			-- Ingatlan adó
			local interiors = exports.nlrp_interiors:requestInteriors(player) or {}
			local propertyTax = #interiors * (taxesCache["propertyTax"] and taxesCache["propertyTax"][3] or 15175)
			...
			outputChatBox(exports.nlrp_core:getServerTag() .. "Megérkezett a fizetésed.", player, 0, 0, 0, true)
			outputChatBox("#ffffff - Bruttó bér: #acd373" .. grossSalary .. " Ft", player, 0, 0, 0, true)
			outputChatBox("#ffffff - Jövedelem adó: #acd373" .. incomeTax .. " Ft", player, 0, 0, 0, true)
			outputChatBox("#ffffff - Jármű adó: #dc143c" .. vehicleTax .. " Ft", player, 0, 0, 0, true)
			outputChatBox("#ffffff - Ingatlan adó: #dc143c" .. propertyTax .. " Ft", player, 0, 0, 0, true)

			local currentBankMoney = getElementData(player, "char.bankMoney") or 0
			currentBankMoney = currentBankMoney + incomeTax + grossSalary - vehicleTax - propertyTax
			if currentBankMoney < 0 then currentBankMoney = 0 end

			local currentMoney = getElementData(player, "char.Money") or 0
			currentMoney = currentMoney

			setElementData(player, "char.bankMoney", currentBankMoney)
			setElementData(player, "char.Money", currentMoney)
		end
	end
)
