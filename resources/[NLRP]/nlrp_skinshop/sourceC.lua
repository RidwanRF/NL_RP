...
function Panel.show()
    ...
    elseif currentPage == "select" then
        widgets.skinList = UI:createCustomList({
            ...
            activeColor = tocolor(183,255,0,125),
            evenColor = tocolor(53, 53, 53, 100),
            oddColor = tocolor(53, 53, 53, 50),
            rowHoverColor = tocolor(53, 53, 53),

            scrollBgColor = tocolor(53, 53, 53),
            scrollGripColor = tocolor(93, 93, 93),
            scrollGripHover = tocolor(134, 134, 134),
            scrollGripSelected = tocolor(174, 174, 174),
        })
...
addEventHandler("sarpUI.click", resourceRoot, function (widget, button)
	if button == "left" then
        ...
        elseif widget == widgets.selectButton then
            exports.nlrp_alert:showAlert("warning", "Kiválasztottad a ruhádat. Most menj és fizesd ki.", "Ha nem fizeted ki, akkor nem kapod meg a ruhádat.")
            changePanel("main")
            Panel.hide()
		end
	end
...
function createSkinShop(interiorID)
    ...
    if shopAssistants[myInterior] and shopMarkers[myInterior] then
        if myInterior == interiorID then
            ...
            for k, v in ipairs(shopMarkers[myInterior]) do
                ...
                shopMarker[k] = createMarker(mX, mY, mZ - 1, "cylinder", 1, 183,255,0, 175)
        ...
        if clickedElement then
            ...
            for i = 1, #shopAssistant do 
                ...
                if data == clickedElement then
                    if activeItem then
                        triggerServerEvent("nlrp_skinshopS:buySkin", localPlayer, activeItem[1], activeItem[2][2], localPlayer)
                    else
                        exports.nlrp_alert:showAlert("error", "Nem választottál magadnak ruházatot")
                    end
                end
            end
        end
