...
function createJobNPC(id, skin, x, y, z, rot, int, dim, name)
    if skin and x and y and z and rot and int and dim and name then
		...
		setElementData(jobNPCs[id], "ped.type", 1)
        ...
        if ID ~= 0 then
            if tonumber(getElementData(player, "char.Job")) == 0 then
                if setPlayerJob(player, ID) then
                    exports["nlrp_alert"]:showAlert(player, "info", "Sikeresen elvállaltad a munkát!", "Mától " .. jobname .. " állásban dolgozol.")
                end
            else
                exports["nlrp_alert"]:showAlert(player, "error", "Már van aktív munkaviszonyod. Húzd az egeredet a", "jelenlegi munkádra és kattints a felmondásra")
            end
        else
            if setPlayerJob(player, ID) then
                exports["nlrp_alert"]:showAlert(player, "info", "Sikeresen felmondtad a munkaviszonyod!", "Mától nem vagy " .. jobname .. ".")
            end
        end
    end
end
