...
registerEvent("nlrp_jobsS:spawnJobVehicle", root, function(jobID, vehID, vehColor)
    if getPedOccupiedVehicle(source) then
        return 
    end

    if spawnedJobVehicles[source] and isElement(spawnedJobVehicles[source]) then
        exports.nlrp_hud:showAlert(source, "error", "Már van egy lehívott munka járműved")
        return 
    end
    ...
    setVehiclePlateText(spawnedJobVehicles[source], "NLRP-" .. getElementData(source, "char.ID"))
...
registerEvent("nlrp_jobsS:destroyJobVehicle", root, function()
    if spawnedJobVehicles[source] and isElement(spawnedJobVehicles[source]) then
...
addEventHandler("onPlayerQuit", root, function()
    triggerEvent("nlrp_jobsS:destroyJobVehicle", source)
end)
