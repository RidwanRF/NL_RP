...
-- Az optika felszerelés / leszerelés, előnézet, kiválasztott elem lüktetése, 
...
function infobox(text, type)
	...
	exports.nlrp_hud:showAlert("info", text)

end



Event.Add("hitMarker", true)

Event.AddHandler("hitMarker", root, function(element, key)
...
function renderPlateText()
	...
	if editPlateText then
		...
		dxRect(50, screenSize.y/2 - 347/2 + 100, 347, 3, tocolor(183,255,0, 180))



		dxPrint("Írd be a rendszámot (max 7 karakter)", 75, screenSize.y/2 - 347/2 + 25, 75 + 296, 0, tocolor(255, 255, 255), 1, dxFontBold, "center")
...
function renderTuningPanel()
	...
	if panelState then
		...
		dxPrint("Navigálás: #b7ff00Nyilak  #ffffffMegvétel: #b7ff00Enter  #ffffffVisszalépés: #b7ff00Backspace", 20, screenSize.y - 55, 0, screenSize.y, tocolor(255, 255, 255), 1, dxFontBigBold, "left", "center", false, false, false, true)

		dxPrint("Készpénz: #b7ff00"..localPlayer:getData("char.Money").." Ft", 0, screenSize.y - 55, screenSize.x - 20, screenSize.y, tocolor(255, 255, 255), 1, dxFontBig, "right", "center", false, false, false, true)

	

		dxRect(40, 40, screenSize.x - 80, 55, tocolor(0, 0, 0, 200))

		dxRect(40, 95, screenSize.x - 80, 3, tocolor(183,255,0, 180))
		...
		for key, value in ipairs(tuningMenu) do
			...
			if key == selectedMenu then
				...
				dxPrint(value["categoryName"], forStartX, 40, forStartX + 205, 95, tocolor(183,255,0), 1, dxFontBigBold, "center", "center")
		...
		if tuningMenu[selectedMenu]["subMenu"] then
			...
			for key, value in ipairs(tuningMenu[selectedMenu]["subMenu"]) do
				...
				if key == selectedSubMenu then
					...
					dxPrint(value["categoryName"], forStartX, forStartY, forStartX + 205, forStartY + 25, tocolor(183,255,0), 1, dxFontBold, "center", "center")
			...
			for key, value in ipairs(loopTable) do
				...
				if key == selectedSubMenuKey then
					...
					dxPrint(value["categoryName"], forStartX, forStartY, forStartX + 195, forStartY + 25, tocolor(183,255,0), 1, dxFont, "left", "center")
					...
					if equippedUpgrade ~= key then
						...
						if value["tuningPrice"] then
							...
							if value["tuningPrice"] == 0 then
								...
								dxPrint("#b7ff00 Ingyenes", forStartX, forStartY, forStartX + 250, forStartY + 25, tocolor(255, 255, 255), 1, dxFont, "right", "center", false, false, false, true)
						...
						dxPrint("Felszereve", forStartX, forStartY, forStartX + 250, forStartY + 25, tocolor(255, 255, 255), 1, dxFont, "right", "center", false, false, false, true)
					...
					if equippedUpgrade ~= key then
						...
						if value["tuningPrice"] then
							...
							if value["tuningPrice"] == 0 then
								...
								dxPrint("Ingyenes", forStartX, forStartY, forStartX + 250, forStartY + 25, tocolor(255, 255, 255), 1, dxFont, "right", "center", false, false, false, true)
								...
								dxPrint("#b7ff00"..value["tuningPrice"]..price, forStartX, forStartY, forStartX + 250, forStartY + 25, tocolor(255, 255, 255), 1, dxFont, "right", "center", false, false, false, true)
...
Event.AddHandler("onClientRender", root, renderTuningPanel)
			...
			elseif selectedMenu == 1 then
				...
				if tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["priceIgMoney"] then
					...
					if  hasMoney(localPlayer, tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningPrice"]) then
						...
						if isTimer(timer) then exports.nlrp_hud:showAlert("info", "Várj 2 másodpercet a következő megvétel előtt") return end
						...
						exports.nlrp_hud:showAlert("info", "Sajnálom, de nincs elég készpénzed a kiegészítő megvásárlásához.")
					...
					if hasPremium(localPlayer, tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningPrice"]) then
						...
						exports.nlrp_hud:showAlert("info", "Sajnálom, de nincs elég NLCoinod a kiegészítő megvásárlásához.")
			...
			elseif selectedMenu == 2 and not inSubMenu then 
				...
				if isGTAUpgradeSlot(tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["upgradeSlot"]) then
					...
					if compatibleUpgrades[1] == nil then
						...
						exports.nlrp_hud:showAlert("info", "A kiválasztott kiegészítő nem kompatibilis a járműveddel!")
			...
			elseif selectedMenu == 2 and inSubMenu then 
				...
				if tuningMenu[selectedMenu]["availableUpgrades"]["priceIgMoney"] or true then
					...
					if (localPlayer:getData("char.Money") < tonumber(tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["tuningPrice"])) then
						...
						infobox("Sajnálom, de nincs elég készpénzed a kiegészítő megvásárlásához.", 2)
						...
						if equippedUpgrade ~= loopTable[selectedSubMenuKey]["upgradeID"] then 
							...
							if equippedUpgrade ~= selectedSubMenuKey then 
								...
								if localPlayer:getData("acc.ID") == enteredVehicle:getData("vehicle.owner") then 

									if getElementData(enteredVehicle, "tuning.airRide") and selectedSubMenu == 9 then

										infobox("FIGYELMEZTETÉS!Az Air-Ride telepítve van a járművedbe.Kérlek távolítsd el a Hidraulika telepítése előtt.", 2)

									else

										if selectedSubMenuKey == 1 then

											triggerServerEvent("tuning->opticalTunings", localPlayer, enteredVehicle, "remove", equippedTuning, tuningMenu[selectedMenu]["subMenu"][selectedSubMenu])

										

										else

											triggerServerEvent("tuning->opticalTunings", localPlayer, enteredVehicle, "add", loopTable[selectedSubMenuKey]["upgradeID"], tuningMenu[selectedMenu]["subMenu"][selectedSubMenu])
											...
											infobox("Sikeresen megvásároltad a(z) " .. tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["categoryName"] .. " kiegészítőt a járművedre.", 1)
									...
									infobox("Sajnálom, de csak a jármű tulajdonosa veheti meg ezt a kiegészítőt.", 2)
								...
								infobox("Ez a kiegészítő már megtalálható a járműveden/járművedben.", 2)
					...
					if inSubMenu  then 
						...
						if selectedSubMenuKey == equippedTuning then
							...
							if (localPlayer:getData("char.Money") >= tonumber(tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningPrice"])) then
								...
								infobox("Sikeresen megvásároltad ezt a kiegészítőt.", 1)
				...
				elseif selectedSubMenu == 4 then
					...
					if inSubMenu then
						...
						if isComponentCompatible(enteredVehicle, "Automobile") then
							...
							paintjobs = exports.nlrp_paintjob:getVehiclePaintJobs(getElementModel(enteredVehicle))
				...
				elseif selectedSubMenu == 5 then 
					...
					if inSubMenu and selectedSubMenu == 5 then
						...
						if localPlayer:getData("acc.ID") == enteredVehicle:getData("vehicle.owner") then 
							...
							infobox("Sajnálom, de ezt csak a jármű tulajdonosa tudja.", 2)
					...
					if inSubMenu then
						...
						if type ~= tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningData"] then 
							...
							if (localPlayer:getData("char.Money") >= tonumber(tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningPrice"])) then
								...
								infobox("Sikeresen megvásároltad ezt a kiegészítőt a járművedre/járművedbe.", 1)
					...
					if inSubMenu then
						...
						if selectedSubMenu == equippedTuning then
							...
							if hasMoney(localPlayer, tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningPrice"]) then
								...
								infobox("Sikeresen megvásároltad a kiegészítőt a járművedre.", 1)
				...
				elseif selectedMenu == 4 then 
					...
					elseif selectedSubMenu == 4 then
						...
						if getElementData(enteredVehicle, "tuning.paintjob") > 0 then
							...
							exports.nlrp_paintjob:destroyShaderCache(enteredVehicle)

							exports.nlrp_paintjob:addVehiclePaintJob(enteredVehicle, getElementModel(enteredVehicle), getElementData(enteredVehicle, "tuning.paintjob"))
					...
					elseif selectedSubMenu == 7 then
						...
						--setVehicleEngineState(enteredVehicle,)
		...
		elseif selectedMenu ~= 3 then
			...
			if button == "arrow_d" then
				...
				if selectedMenu == 4 and selectedSubMenu == 4 and inSubMenu then
					...
					exports.nlrp_paintjob:addVehiclePaintJob(enteredVehicle, getElementModel(enteredVehicle), tuningMenu[selectedMenu]["subMenu"][selectedSubMenu]["subMenu"][selectedSubMenuKey]["tuningData"])
...
Event.AddHandler("onClientKey", root, clientKeyPressed)



Event.AddHandler("onClientKey", root, function(button, pressed)
	...
	if pressed and editPlateText then
		...
		elseif button == "enter" and #plateText >= 3 then
			...
			if localPlayer:getData("char.Money") >= 800 then

				if equippedTuning ~= plateText then

					if foundPlate(plateText) then

						infobox("Már létezik ilyen rendszám az adatbázisban.", 2)

					else

						localPlayer:setData("char.Money", localPlayer:getData("char.Money") - tonumber(800))

						infobox("Sikeresen telepítetted a kiegészítőt", 1)
	...
	if vehicle and vehicleType then
		...
		if type(vehicleType) == "string" then
			...
			if getVehicleType(vehicle) == vehicleType then
				...
				infobox("Ez a kiegészítő nem kompatibilis a járműveddel!", 2)
...
Event.AddHandler("onClientRender", root, updateColorRender)