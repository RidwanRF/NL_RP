...
exports.nlrp_admin:addAdminCommand("nearbyshoppeds", 6, "Közelben lévő Bolt NPC-k")
addCommandHandler("nearbyshoppeds",
	function(commandName, maxdist)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not maxdist then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. commandName .. " [ < távolság > ]", 0, 0, 0, true)
			else
				...
				if #peds > 0 then
					outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Közeledben lévő pedek (" .. maxdist .. " yard):", 0, 0, 0, true)
					...
					outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Nincs egyetlen ped sem a közeledben.", 0, 0, 0, true)
				end
			end
		end
	end)
...
addEventHandler("onClientResourceStart", getRootElement(),
	function(res)
		if getResourceName(res) == "nlrp_inventory" then
			itemNames = exports.nlrp_inventory:getItemNameList()
			nonStackableItems = exports.nlrp_inventory:getNonStackableItems()
		elseif source == getResourceRootElement() then
			local inventory = getResourceFromName("nlrp_inventory")
		...
		if isElement(element) and not currentPedId and button == "right" and state == "up" then
			...
			if pedId then
				...
				if isElementWithinColShape(localPlayer, colshape) then
					...
					RobotoFont = dxCreateFont(":nlrp_assets/fonts/Roboto-Regular.ttf", respc(15), false, "antialiased")
					RobotoLighter = dxCreateFont(":nlrp_assets/fonts/Roboto-Light.ttf", respc(15), false, "antialiased")
...
function checkOwnership()
	...
	if tonumber(currentPedInterior) then
		if exports.nlrp_interiors:getInteriorOwner(currentPedInterior) == getElementData(localPlayer, "char.ID") or (getElementData(localPlayer, "acc.adminLevel") >= 6 and getElementData(localPlayer, "adminDuty")) then
			isOwner = true
		end
	end
...
function renderTheShop()
	...
	elseif panelState == "setcategories" then
		...
		dxDrawImage(math.floor(x + logoOffset), math.floor(y + logoOffset), logoSize, logoSize, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(183,255,0))
		...
		for k = 1, maxVisible do
			...
			if v then
				...
				if not currentCategories[k + currentOffset] then
					...
					--drawButton("addcat:" .. k + currentOffset, "Hozzáadás", x2 + sx2 - respc(160) - respc(10), y + respc(2), respc(160), sizeForRow - respc(4), 183,255,0, 1, RobotoLighter, 0.75, "", IconsFont, 0.75)
				else
		...
		if #mainCategories > maxVisible then
			...
			dxDrawRectangle(x + sx - respc(10), y2 + (totalSize / #mainCategories) * math.min(currentOffset, #mainCategories - maxVisible), respc(5), (totalSize / #mainCategories) * maxVisible, tocolor(183,255,0))
		end
		...
		if haveChanges then
			...
			if activeButton == "savecats" then
				dxDrawRectangle(x, y, sx, titleSize, tocolor(183,255,0, 175))
			else
				dxDrawRectangle(x, y, sx, titleSize, tocolor(183,255,0, 125))
			end
	...
	elseif panelState == "ownermode" then
		...
		if isOwner then
			drawButton2("changeOwnerMode", "Vásárlói mód", buttonPosX, y + logoOffset, respc(230), logoSize, 183,255,0, 1, RobotoLighter, 1)
		...
		for k = 1, maxVisible do
			...
			if v then
				...
				elseif v[1] == "noitem" and v[2] then
					...
					dxDrawImage(math.floor(x2 + respc(35)), math.floor(y + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":nlrp_inventory/files/items/" .. itemId .. ".png")
				...
				elseif v[1] == "haveitem" and v[2] and currentItems[v[2]] then
					...
					if selectedActionItem == itemId then
						...
						drawButton2("applyitemaction", "OK", buttonPosX, buttonPosY, respc(50), buttonSizeY, 183,255,0, 1, RobotoLighter, 0.8)
						...
						if currentPedInterior > 0 then
							...
							drawButton2("addstock:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 183,255,0, 1, RobotoLighter, 0.8, "", IconsFont, 1)
						end

						buttonPosX = buttonPosX - respc(5) - respc(40)
						drawButton2("setprice:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 183,255,0, 1, RobotoLighter, 0.8, "", IconsFont, 1)

						if currentPedInterior > 0 then
							dxDrawText("Ár: " .. data[1] .. " Ft | Nagyker ár: " .. itemBasePrices[itemId] .. " Ft | Készleten: " .. data[2] .. " db", x2, y, x2 + (buttonPosX - x2 - respc(10)), y + sizeForRow, tocolor(225, 225, 225, 225), 0.8, RobotoFont, "right", "center")
						else
							dxDrawText("Ár: " .. data[1] .. " Ft | Nagyker ár: " .. itemBasePrices[itemId] .. " Ft", x2, y, x2 + (buttonPosX - x2 - respc(10)), y + sizeForRow, tocolor(225, 225, 225, 225), 0.8, RobotoFont, "right", "center")
						end
					end
				else
		...
		if visibleCount > maxVisible then
			...
			dxDrawRectangle(x + sx - respc(10), y2, respc(5), totalSize, tocolor(0, 0, 0, 100))
			dxDrawRectangle(x + sx - respc(10), y2 + (totalSize / visibleCount) * math.min(currentOffset, visibleCount - maxVisible), respc(5), (totalSize / visibleCount) * maxVisible, tocolor(183,255,0))
		end
		...
		if currentPedInterior > 0 then
			...
			dxDrawText("Jelenlegi egyenleg: " .. formatNumber(currentBalance) .. " Ft", x + respc(10), y, 0, y + titleSize, tocolor(255, 255, 255), 1, RobotoLighter, "left", "center")

			if selectedItemAction == "putInMoney" then
				local buttonPosX = x + sx - respc(10) - respc(140)

				drawButton2("putInMoneyOk", "Befizetés", buttonPosX, y, respc(140), titleSize, 183,255,0, 1, RobotoLighter, 1)
			...
			elseif selectedItemAction == "getOutMoney" then
				...
				buttonPosX = buttonPosX - respc(10) - respc(140)
				drawButton2("putInMoney", "Befizetés", buttonPosX, y, respc(140), titleSize, 183,255,0, 1, RobotoLighter, 1)
			end
		end
	-- vásárlói mód
	elseif panelState then
		...
		if isOwner then
			if currentPedInterior > 0 then
				drawButton2("changeOwnerMode", "Tulajdonosi mód", buttonPosX, y + logoOffset, respc(230), logoSize, 183,255,0, 1, RobotoLighter, 1)
			else
				drawButton2("changeOwnerMode", "Bolt kezelése", buttonPosX, y + logoOffset, respc(230), logoSize, 183,255,0, 1, RobotoLighter, 1)
			end
		...
		for k = 1, maxVisible do
			...
			if v then
				...
				dxDrawImage(math.floor(x2 + respc(5)), math.floor(y + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":nlrp_inventory/files/items/" .. itemId .. ".png")
				...
				if currentPedInterior > 0 and (not itemData[2] or itemData[2] == 0) then
					...
					dxDrawText("Ár: " .. formatNumber(itemData[1]) .. " Ft/db", x2, y, x2 + sx2 - respc(10), y + sizeForRow, tocolor(200, 200, 200, 200), 0.75, RobotoFont, "right", "center")
		...
		if #currentItemsEx > maxVisible then
			...
			dxDrawRectangle(x + respc(5) + holderSize - respc(5), y2 + (totalSize / #currentItemsEx) * math.min(currentOffset, #currentItemsEx - maxVisible), respc(5), (totalSize / #currentItemsEx) * maxVisible, tocolor(183,255,0))
		end
		...
		for k = 1, maxVisible do
			...
			if v then
				...
				dxDrawImage(math.floor(x2 + respc(5)), math.floor(y2 + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":nlrp_inventory/files/items/" .. itemId .. ".png")
				
				dxDrawText(itemNames[itemId], x2 + respc(10) + itemSize, y2, x2 + sx2, y2 + sizeForRow, tocolor(255, 255, 255), 0.8, RobotoFont, "left", "center")

				dxDrawText("Kosárban: " .. itemAmount .. " db | Ár: " .. formatNumber(itemAmount * currentItems[itemId][1]) .. " Ft", x2, y2, x2 + sx2 - respc(10), y2 + sizeForRow, tocolor(200, 200, 200, 200), 0.75, RobotoFont, "right", "center")
		...
		if #cartItems > maxVisible then
			...
			dxDrawRectangle(x + respc(15) + holderSize * 2 - respc(5), y3 + (totalSize / #cartItems) * math.min(cartOffset, #cartItems - maxVisible), respc(5), (totalSize / #cartItems) * maxVisible, tocolor(183,255,0))
		end
		...
		if #cartItems > 0 then
			...
			dxDrawText("Összesen: " .. formatNumber(totalPrice) .. " Ft", x + respc(10), y3, 0, y3 + titleSize, tocolor(255, 255, 255), 1, RobotoLighter, "left", "center")

			local buttonPosX = x + sx - respc(10) - respc(140)

			drawButton2("buyItems", "Fizetés", buttonPosX, y3, respc(140), titleSize - respc(5), 183,255,0, 1, RobotoLighter, 1)
		end
	end
		...
		elseif selected[1] == "buyItems" then
			if #cartItems > 0 then
				...
				for k = 1, #cartItems do
					...
					totalWeight = totalWeight + amount * exports.nlrp_inventory:getItemWeight(itemId)
				...
				if exports.nlrp_core:getMoney(localPlayer) >= totalPrice then
					if exports.nlrp_inventory:countEmptySlots() >= totalSlot then
						if exports.nlrp_inventory:getCurrentWeight() + totalWeight <= exports.nlrp_inventory:getWeightLimit("player", localPlayer) then
							showPrompt("Biztosan megvásárolod a kosárban lévő összes terméket?\nTeljes összeg: #32b3ef" .. formatNumber(totalPrice) .. " Ft",
								function()
									triggerServerEvent("buyItemsFromPed", localPlayer, currentPedId, cartItems, totalPrice, totalSlot, totalWeight)
									unloadShop()
								end)
						else
							exports.nlrp_hud:showInfobox("error", "Nem bírsz el ennyi tárgyat!")
						end
					else
						exports.nlrp_hud:showInfobox("error", "Nincs elég hely az inventorydban!")
					end
				else
					exports.nlrp_hud:showInfobox("error", "Nincs nálad elegendő pénz!")
				end
			end
		end
	end
