...
exports.nlrp_admin:addAdminCommand("deletevehicles", 9, "Összes jármű törlése")
addCommandHandler("deletevehicles",
	function (player, cmd, state)
		if getElementData(player, "acc.adminLevel") >= 9 then
			if not state then
				outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Biztos vagy benne, hogy törlöd az összes járművet? Ha igen, használd az #ffa600/" .. cmd .. " yes #ffffffparancsot.", player, 0, 0, 0, true)
			elseif state == "yes" then
				deleteVehicles("all")

				outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Az összes jármű sikeresen törölve.", player, 0, 0, 0, true)
			end
		end
	end
)

exports.nlrp_admin:addAdminCommand("setvehgroup", 6, "Jármű frakcióba tétele/kivétele")
	...
	function (player, cmd, vehId, groupId)
		if getElementData(player, "acc.adminLevel") >= 6 then
			if not tonumber(vehId) or not tonumber(groupId) then
				outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. cmd .. " [Jármű ID] [Csoport ID]", player, 0, 0, 0, true)
			else
				vehId = tonumber(vehId)
				groupId = tonumber(groupId)

				if loadedVehicles[vehId] then
					local groups = exports.nlrp_groups:getGroups()
					...
					if groups[groupId] or groupId == 0 then
						dbQuery(
							function (qh)
								...
								if isElement(player) then
									if groupId == 0 then
										outputChatBox(exports.nlrp_core:getServerTag("admin") .. "A kiválasztott jármű sikeresen eltávolítva a kiválasztott frakcióból.", player, 0, 0, 0, true)
									else
										outputChatBox(exports.nlrp_core:getServerTag("admin") .. "A kiválasztott jármű sikeresen hozzáadva a kiválasztott frakcióhoz. #ffff99(" .. groups[groupId].name .. ")", player, 0, 0, 0, true)
									end
								end
						...
						outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott frakció nem létezik!", player, 0, 0, 0, true)
					end
				else
					outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott jármű nem létezik vagy nincs lespawnolva!", player, 0, 0, 0, true)
				end
			end
		end
	end
...
addEventHandler("onResourceStart", getRootElement(),
	function (startedResource)
		if getResourceName(startedResource) == "nlrp_inventory" then
			for k, v in pairs(loadedVehicles) do
				exports.nlrp_inventory:loadItems(v, k)
			end
			...
			if isElement(sourcePlayer) then
				...
				if rows > 0 then
					...
					if #impoundVehicles > 0 and isElement(sourcePlayer) then
						outputChatBox(exports.nlrp_core:getServerTag("info") .. "Neked #ffff99" .. #impoundVehicles .. "#ffffff db lefoglalt járműved van.", sourcePlayer, 0, 0, 0, true)

						for i = 1, #impoundVehicles do
							local vehData = impoundVehicles[i]

							outputChatBox(" - Rendszám: #ffff99" .. vehData.licensePlate .. "#ffffff, Típus: #ffff99" .. exports.nlrp_vehs:getVehicleNameFromModel(vehData.model), sourcePlayer, 255, 255, 255, true)
						end

						outputChatBox(exports.nlrp_core:getServerTag("info") .. "További információkért látogasson el a kapitányságunkra.", sourcePlayer, 0, 0, 0, true)
					end
				end
			else
				dbFree(qh)
...
function loadVehicle(data)
	if data then
		...
		if not isElement(loadedVehicles[vehicleID]) then

			local licensePlate = data.licensePlate
			if not licensePlate or utfLen(licensePlate) <= 1 then
			...
			if vehicle then
				...
				setVehicleRespawnPosition(vehicle, parkedPosition[1], parkedPosition[2], parkedPosition[3], parkedPosition[4], parkedPosition[5], parkedPosition[6])
				setElementData(vehicle, "vehicle.parkedPosition", parkedPosition)
				...
				setElementData(vehicle, "vehicle.engine", data.engineState == 1)
				...
				setElementData(vehicle, "vehicle.carshop", data.carshop)
				...
				local c = fromJSON(data.color)
				setVehicleColor(vehicle,255,255,255,c[4],c[5],c[6],c[7],c[8],c[9],c[10],c[11],c[12])

				setElementData(vehicle,"danihe->vehicles->bodyColor",{c[1],c[2],c[3]})
				...
				local kms = data.traveled
				if not kms then kms = 0 end
				setElementData(vehicle,"danihe->kms",kms)
				...
				local optics = fromJSON(data.optics)
				for k,v in pairs(optics) do
				...
				if data.custom_paintjob > 0 then
					setElementData(vehicle,"tuning.paintjob",data.custom_paintjob)
				end
				...
				exports.nlrp_inventory:loadItems(vehicle, vehicleID)

				loadedVehicles[vehicleID] = vehicle
			end
		else
			setElementDimension(loadedVehicles[vehicleID],position[8])
		end
	end
end
...
function saveVehicle(vehicle)
	...
	if getElementDimension(vehicle) < 15000 then
		...
		if vehicleID > 0 then
			do
				...
				local optics = {}
				for i = 0,16 do
					optics[i] = getVehicleUpgradeOnSlot(vehicle,i)
				end

				local datas = {
					position = toJSON({x, y, z, rx, ry, rz, getElementInterior(vehicle), getElementDimension(vehicle)}),
					engineState = getElementData(vehicle, "vehicle.engine") and 1 or 0,
					...
					maxFuel = getElementData(vehicle, "vehicle.maxFuel") or exports.nlrp_hud:getTheFuelTankSizeOfVehicle(model),
					...
					headlight = toJSON({getVehicleHeadLightColor(vehicle)}),
					...
					traveled = getElementData(vehicle, "danihe->kms") or 0,
					...
					color = toJSON({bodyColor[1],bodyColor[2],bodyColor[3],c[4],c[5],c[6],c[7],c[8],c[9],c[10],c[11],c[12]}),
					optics = toJSON(optics),
					...
					custom_paintjob = getElementData(vehicle,"tuning.paintjob") or 0
				}
...
function makeVehicle(model, owner, group, position, r, g, b, cs)
	...
	if model < 400 or model > 611 then model = 400 end
	r = r or 255
	g = g or 255
	b = b or 255
	...
	local fuelCapacity = exports.nlrp_hud:getTheFuelTankSizeOfVehicle(model)

	dbExec(connection, "INSERT INTO vehicles (model, owner, groupID, position, parkedPosition, color, health, fuel, maxFuel, unit, handBrake, carshop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", model, charID, group, position, position, toJSON({r, g, b, 255,255,255, 255,255,255, 255,255,255}), 1000, fuelCapacity, fuelCapacity, "", 0, tonumber(carshop))
	dbQuery(
		function (qh, targetPlayer)
			...
			if result then
				...
				if isElement(targetPlayer) then
					exports.nlrp_inventory:giveItem(targetPlayer, 2, 1, result.vehicleID)
				end
			end
	...
	if vehicleIds == "all" then
		...
		for k, v in ipairs(getElementsByType("player")) do
			if getElementData(v, "char.ID") then
				exports.nlrp_inventory:removeAllItem(v, "itemId", 2)
			end
		end
					...
					if ownerID and vehiclesCache[ownerID] then
						...
						for k, v in ipairs(getElementsByType("player")) do
							if getElementData(v, "char.ID") == ownerID then
								exports.nlrp_inventory:removeItemByData(v, 2, "data1", vehicleID)
							end
						end
					end
	...
	if vehiclesCache[charID] then
		for _, vehicleID in pairs(vehiclesCache[charID]) do
			if loadedVehicles[vehicleID] then
				...
				if isElement(loadedVehicles[vehicleID]) then
					setElementDimension(loadedVehicles[vehicleID],15700+charID)
					--destroyElement(loadedVehicles[vehicleID])
				end
...
addEventHandler("onVehicleStartExit", getRootElement(),
	function (player)
		if getElementData(player, "player.seatBelt") then
			cancelEvent()
			exports.nlrp_hud:showAlert(player, "error", "Előbb csatold ki a biztonsági öved!")
		elseif getVehicleType(source) ~= "Bike" and getVehicleType(source) ~= "BMX" and getVehicleType(source) ~= "Boat" then
			if getElementData(source, "vehicle.locked") then
				cancelEvent()
				exports.nlrp_hud:showAlert(player, "error", "Nem szállhatsz ki amíg az ajtók zárva vannak!")
			elseif getElementData(player, "player.Cuffed") then
				cancelEvent()
				exports.nlrp_hud:showAlert(player, "error", "Nem szállhatsz ki amíg bilincs van a kezeden!")
			end
		end
	end
)
	...
	function (loss)
		...
		if health < 320 or (health - loss) < 320 then
			...
			if isElement(theDriver) then
				exports.nlrp_hud:showAlert(theDriver, "error", "Lerobbant a járműved!")
			end
		else
			setVehicleDamageProof(source, false)
		end
	...
	function (vehicle, players, task)
		if isElement(source) then
			if isElement(vehicle) then
				...
				if not (exports.nlrp_inventory:hasItemWithData(source, 2, "data1", vehicleID) or getElementData(vehicle, "vehicle.group") == getElementData(source, "groupDutyID") or getElementData(source, "adminDuty")) then
					exports.nlrp_hud:showAlert(source, "error", "Nincs kulcsod ehhez a járműhöz!")
					return
				end
				...
				if getElementData(vehicle, "vehicle.locked") then
					...
					if getVehicleOccupant(vehicle) == source and not task[1] then
						exports.nlrp_chat:sendLocalMeAction(source, "kinyit belülről egy " .. exports.nlrp_vehs:getVehicleName(vehicle) .. " típusú járművet.")
						triggerClientEvent(getVehicleOccupants(vehicle), "playVehicleSound", vehicle, "simple", ":nlrp_assets/audio/vehicles/lockin.ogg")
						return
					end

					triggerClientEvent(players, "onVehicleLockEffect", vehicle)
					exports.nlrp_chat:sendLocalMeAction(source, "kinyit egy " .. exports.nlrp_vehs:getVehicleName(vehicle) .. " típusú járművet.")
				else
					setElementData(vehicle, "vehicle.locked", true)
					setVehicleLocked(vehicle, true)

					if getVehicleOccupant(vehicle) == source and not task[1] then
						exports.nlrp_chat:sendLocalMeAction(source, "bezár belülről egy " .. exports.nlrp_vehs:getVehicleName(vehicle) .. " típusú járművet.")
						triggerClientEvent(getVehicleOccupants(vehicle), "playVehicleSound", vehicle, "simple", ":nlrp_assets/audio/vehicles/lockin.ogg")
						return
					end

					triggerClientEvent(players, "onVehicleLockEffect", vehicle)
					exports.nlrp_chat:sendLocalMeAction(source, "bezár egy " .. exports.nlrp_vehs:getVehicleName(vehicle) .. " típusú járművet.")
				end

				triggerClientEvent(players, "playVehicleSound", vehicle, "3d", ":nlrp_assets/audio/vehicles/lockout.ogg")
			end
		end
	end
)
		...
		if isElement(source) then
			if isElement(vehicle) then
				...
				if getElementData(vehicle,"company.vehicleID") then
					if getElementData(vehicle,"company.driver") ~= source and not getElementData(source, "adminDuty") then
						exports.nlrp_hud:showAlert(source, "error", "Ez nem a te munka járműved!")
						return
					end
				end

				local nlrp_vehs = getResourceFromName("nlrp_vehs")
				local vehicleNames = nlrp_vehs and getResourceState(nlrp_vehs) == "running"

				if toggle then
					if getElementHealth(vehicle) <= 320 then
						exports.nlrp_hud:showAlert(source, "error", "A jármű motorja túlságosan sérült.")
						exports.nlrp_chat:sendLocalMeAction(source, "megpróbálja beindítani a jármű motorját, de nem sikerül neki.")
						return
					elseif getElementData(vehicle, "vehicle.fuel") <= 0 then
						exports.nlrp_hud:showAlert(source, "error", "A járműből kifogyott az üzemanyag.")
						exports.nlrp_chat:sendLocalMeAction(source, "megpróbálja beindítani a jármű motorját, de nem sikerül neki.")
						return
					elseif getElementData(vehicle, "vehicle.impound") then
						exports.nlrp_hud:showAlert(source, "error", "Ez a jármű le van foglalva, ezért nem tudod beindítani!")
						return
					end

					if vehicleNames then
						exports.nlrp_chat:sendLocalMeAction(source, "beindítja egy " .. exports.nlrp_vehs:getVehicleName(vehicle) .. " típusú jármű motorját.")
					else
						exports.nlrp_chat:sendLocalMeAction(source, "beindítja egy jármű motorját.")
					end
					...
					if vehicleNames then
						exports.nlrp_chat:sendLocalMeAction(source, "leállítja egy " .. exports.nlrp_vehs:getVehicleName(vehicle) .. " típusú jármű motorját.")
					else
						exports.nlrp_chat:sendLocalMeAction(source, "leállítja egy jármű motorját.")
					end
...
addEventHandler("toggleVehicleLights", getRootElement(),
	function (vehicle)
		if getElementData(vehicle, "vehicle.light") then
			...
			exports.nlrp_chat:sendLocalMeAction(source, "lekapcsolja a jármű lámpáit.")
		else
			setVehicleOverrideLights(vehicle, 2)
			setElementData(vehicle, "vehicle.light", true)
			exports.nlrp_chat:sendLocalMeAction(source, "felkapcsolja a jármű lámpáit.")
		end

		triggerClientEvent(getVehicleOccupants(vehicle), "playVehicleSound", vehicle, "simple", ":nlrp_assets/audio/vehicles/lightswitch.ogg")
	end
)
...
addCommandHandler("park",
	function (player)
		...
		if not isElement(vehicle) then
			exports.nlrp_hud:showAlert(player, "error", "Nem ülsz járműben!")
			return
		end

		local vehicleID = tonumber(getElementData(vehicle, "vehicle.dbID")) or -65535

		if not (exports.nlrp_inventory:hasItemWithData(player, 2, "data1", vehicleID) or getElementData(player, "adminDuty")) then
			exports.nlrp_hud:showAlert(player, "error", "Nincs kulcsod ehhez a járműhöz!")
			return
		end
		...
		if dbExec(connection, "UPDATE vehicles SET parkedPosition = ? WHERE vehicleID = ? ", toJSON(currentPosition), vehicleID) then
			...
			exports.nlrp_hud:showAlert(player, "success", "Járműved sikeresen leparkolva.")
		end
	end
)
...
function getPlayerVehiclesCount(charID)
	...
	if vehiclesCache[charID] then
		for _, vehicleID in pairs(vehiclesCache[charID]) do
			if getElementData(loadedVehicles[vehicleID], "vehicle.group") == 0 then
				if loadedVehicles[vehicleID] then
					count = count + 1
				end
			end
