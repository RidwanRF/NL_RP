...
addCommandHandler("oldcar",
	function ()
		outputChatBox(exports.nlrp_core:getServerTag("info") .. "Előző járműved: #ffff99" .. getElementData(localPlayer, "theOldCar") or "-", 0, 0, 0, true)
	end
)
...
function windowsFunction()
	...
	if isElement(vehicle) and not nonSeatBeltsVehicle[getElementModel(vehicle)] then
		...
		if not windowState then
			...
			exports.nlrp_chat:sendLocalMeAction(localPlayer, "lehúzza a jármű ablakait.")
		else
			windowState = false
			setElementData(vehicle, "vehicle.windowState", false)
			exports.nlrp_chat:sendLocalMeAction(localPlayer, "felhúzza a jármű ablakait.")
		end
					...
					if getVehicleType(playerVehicle) ~= "BMX" then
						if state == "down" then
							if not isTimer(engineStartTimer) and getTickCount() - lastEngineStart >= 1500 then
								triggerServerEvent("syncVehicleSound", playerVehicle, "3d", ":nlrp_assets/audio/vehicles/starter.ogg", getElementsByType("player", root, true))
	...
	function (player, seat)
		...
		if player == localPlayer then
			...
			if seat == 0 then
				...
				outputChatBox(exports.nlrp_core:getServerTag().."A járművet a J billentyű lenyomásával tudod beindítani, a biztonsági övet pedig az F6 lenyomásával tudod bekötni.",255,255,255,true)
			else
				outputChatBox(exports.nlrp_core:getServerTag().."A biztonsági övet az F6 lenyomásával tudod bekötni.",255,255,255,true)
			end
		end
	end
)
		...
		if player == localPlayer then
			if getVehicleType(source) == "Bike" or getVehicleType(source) == "BMX" or getVehicleType(source) == "Boat" then
				if getElementData(source, "vehicle.locked") then
					cancelEvent()
					exports.nlrp_hud:showAlert("error", "Ez a jármű zárva van!")
				end
			end
		end
	end
...
function informatePlayerAboutHandbrake()
	...
	if isElement(playerVehicle) and getVehicleEngineState(playerVehicle) then
		exports.nlrp_hud:showInfobox("error", "Amíg be van húzva a kézifék, nem indulhatsz el.")
	end
end
...
function informatePlayerAboutWheelClamp()
	...
	if isElement(playerVehicle) and getVehicleEngineState(playerVehicle) then
		exports.nlrp_hud:showInfobox("error", "Amíg kerékbilincs van a járművön nem indulhatsz el.")
	end
end
...
exports.nlrp_admin:addAdminCommand("nearbyvehicles", 1, "Közelben lévő járművek")
			...
			for k, v in ipairs(getElementsByType("vehicle", getRootElement(), true)) do
				...
				if getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, vehiclePosX, vehiclePosY, vehiclePosZ) <= distance then
					...
					table.insert(nearbyVehicles, {
						model,
						exports.nlrp_vehs:getVehicleNameFromModel(model),
						getElementData(v, "vehicle.dbID") or 0,
						getVehiclePlateText(v),
						getElementData(v, "vehicle.owner")
					})
				end
			end
			
			if #nearbyVehicles > 0 then
				outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Közeledben lévő járművek #ffff99(" .. distance .. " yard):", 0, 0, 0, true)

				for k, v in ipairs(nearbyVehicles) do
					outputChatBox("    * #b7ff00Típus: #ffffff" .. v[1] .. " (" .. v[2] .. ") | #b7ff00Azonosító: #ffffff" .. (v[3] == 0 and "Nincs (ideiglenes)" or v[3]) .. " | #b7ff00Rendszám: #ffffff" .. (v[4] or "Nincs") .." | #b7ff00Tulajdonos ID: #ffffff" .. (v[5] or "Nincs"), 255, 255, 255, true)
				end
			else
				outputChatBox(exports.nlrp_core:getServerTag("admin") .. "Nincs egyetlen jármű sem a közeledben.", 0, 0, 0, true)
			end
		end
	end
)
...
exports.nlrp_admin:addAdminCommand("dl", 1, "Jármű adatok mutatása a járművek felett")
	...
	if vehicle then
		...
		if seat == 0 then
			...
			if int == 0 and dim == 0 then
				if firstCount then
					ox,oy,oz = getElementPosition(vehicle)
					firstCount = false
				end
				local kms = getElementData(vehicle,"danihe->kms") or 0
				kms = kms + (getDistanceBetweenPoints3D(ox,oy,oz,getElementPosition(vehicle)))/500
