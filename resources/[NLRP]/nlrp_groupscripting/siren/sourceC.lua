...
function processStrobe(vehicle, state)
	if isElement(vehicle) then
		if state then
			...
			setVehicleHeadLightColor(vehicle, 0, 0, 255) -- 0, 0, 255
		else
			setVehicleLightState(vehicle, 0, 0)
			setVehicleLightState(vehicle, 3, 0)
			setVehicleLightState(vehicle, 1, 1)
			setVehicleLightState(vehicle, 2, 1)
			setVehicleHeadLightColor(vehicle, 255, 0, 0) -- 255, 0, 0
		end
	...
	if isElement(vehicle) then
		if getElementData(vehicle, "vehicle.group") and getElementData(vehicle, "vehicle.group") > 0 then
			...
			if playerGroups[getElementData(v, "vehicle.group")] then
				...
				setElementData(gpsBlips[v], "blipTooltipText", "[" .. exports.nlrp_groups:getGroupPrefix(getElementData(v, "vehicle.group")) .. "] " .. unitNum .. " (" .. statusCodes[currentStatus] .. ")")
				...
				if currentStatus == 3 then
					...
					outputChatBox("#32b3ef[" .. exports.nlrp_groups:getGroupPrefix(getElementData(v, "vehicle.group")) .. "]#ffffff " .. unitNum .. " #ff4646erősítést #ffffffkért.", 0, 0, 0, true)
				end
			end
		end
	end
