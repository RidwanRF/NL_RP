...
addEvent("nlrp_sirenS:toggleSirenSound", true)
addEventHandler("nlrp_sirenS:toggleSirenSound", root, function(soundID)
    local modelID = getElementModel(getPedOccupiedVehicle(source)) 
    if allowedVehicles[modelID] then
        triggerClientEvent("nlrp_sirenC:toggleSirenSound", source, soundID)
    end
end)

addEvent("nlrp_sirenS:toggleSirenLights", true)
addEventHandler("nlrp_sirenS:toggleSirenLights", root, function(lightID)
...
function playAirhorn(player)
    if isPedInVehicle(player) and allowedVehicles[getElementModel(getPedOccupiedVehicle(player))] then
        triggerClientEvent("nlrp_sirenC:useAirhorn", player, true)
    end
end

function stopAirhorn(player)
    if isPedInVehicle(player) and allowedVehicles[getElementModel(getPedOccupiedVehicle(player))] then
        triggerClientEvent("nlrp_sirenC:useAirhorn", player, false)
    end
end
...
function toggleGPSTracker(state)
    local vehicle = getPedOccupiedVehicle(source)
    if exports.nlrp_groups:isPlayerInGroup(player, getElementData(vehicle, "vehicle.group")) then
    ...
    for k, v in pairs(getElementsByType("player")) do
        if exports.nlrp_groups:isPlayerInGroup(v, getElementData(vehicle, "vehicle.group")) then
            if vehicle and isElement(vehicle) then
                if state then
...
addEvent("nlrp_sirenS:toggleGPS", true)
addEventHandler("nlrp_sirenS:toggleGPS", root, toggleGPSTracker)--]]