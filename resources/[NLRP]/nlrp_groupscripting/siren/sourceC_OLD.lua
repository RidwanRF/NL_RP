...
function triggerSirenFunctions(type, id)
    ...
    if type == "sound" then
        ...
        if sirenData.sound ~= id then
            sirenData.sound = id
            triggerServerEvent("nlrp_sirenS:toggleSirenSound", localPlayer, id)
        elseif not id or sirenData.sound == id then
            sirenData.sound = false
            triggerServerEvent("nlrp_sirenS:toggleSirenSound", localPlayer, false)
        end
    ...
    elseif type == "lights" then
        ...
        if sirenData.light ~= id then
            sirenData.light = id
            triggerServerEvent("nlrp_sirenS:toggleSirenLights", localPlayer, id)
        elseif not id or sirenData.light == id then
            sirenData.light = false
            triggerServerEvent("nlrp_sirenS:toggleSirenLights", localPlayer, false)
        end
...
addEvent("nlrp_sirenC:toggleSirenSound", true)
addEventHandler("nlrp_sirenC:toggleSirenSound", root, function(soundID)
    ...
    if soundID and tonumber(soundID) then
        if allowedVehicles[modelID] and vehiclesSiren[modelID] then
            ...
            if vehiclesSiren[modelID][soundID] then
                ...
                exports.nlrp_alert:showAlert("error", "Nincs beállítva megkülönböztető hangjelzés erre a gombra")
            end
        end
    else
        if isElement(sirenSounds[vehicle]) then
...
addEvent("nlrp_sirenC:useAirhorn", true)
addEventHandler("nlrp_sirenC:useAirhorn", root, function(state)
    if source == localPlayer and not isPedInVehicle(source) then
        return
    end
    ...
    if state then
        if allowedVehicles[modelID] and vehiclesSiren[modelID] then
            ...
            if vehiclesSiren[modelID]["horn"] then
                ...
                exports.nlrp_alert:showAlert("error", "Nincs beállítva légkürt ezen a járművön")
            end
        end
    else
        if isElement(airhornSounds[vehicle]) then
                        ...
                        if getDistanceBetweenPoints3D(vX, vY, vZ, pX, pY, pZ) <= 10 then
                            ...
                            if worldX then
                                ...
                                if isLineOfSightClear(cameraX, cameraY, cameraZ, cX, cY, cZ, true, false, false, true, false, true, false) then
                                    exports.nlrp_UI:dxDrawBorderedText(getElementData(v, "siren.unit"), worldX, worldY, worldX, worldY, tocolor(255, 255, 255), 1, fonts.Roboto11)
                                end
                            end
                        end
                    end
            ...
            if playerGroups[getElementData(v, "vehicle.group")] and getElementData(v, "siren.status") then
                ...
                if status == 1 then -- Nem elérhető
                    ...
                    if playerGroups[getElementData(v, "vehicle.group")] then
                        ...
                        setElementData(gpsBlips[v], "blipTooltipText", "[" .. exports.nlrp_groups:getGroupPrefix(getElementData(v, "vehicle.group")) .. "] " .. getElementData(v, "siren.unit") or "Ismeretlen")
                ...
                elseif status == 3 then -- Erősítés
                    ...
                    if playerGroups[getElementData(v, "vehicle.group")] then
                        ...
                        if playerGroups[getElementData(v, "vehicle.group")] then
                            ...
                            setElementData(gpsBlips[v], "blipIcon", "cp")
                            --setElementData(gpsBlips[v], "blipSize", 30)
                            setElementData(gpsBlips[v], "blipTooltipText", "[ERŐSÍTÉS][" .. exports.nlrp_groups:getGroupPrefix(getElementData(v, "vehicle.group")) .. "] " .. getElementData(v, "siren.unit") or "Ismeretlen")
                            attachElements(gpsBlips[v], v)
                            setElementData(gpsBlips[v], "blipColor", tocolor(255, 66, 66))
                            setElementData(gpsBlips[v], "blipFarShow", true)
                            playSound("siren/sounds/backup.mp3" )
                            outputChatBox("#32b3ef[" .. exports.nlrp_groups:getGroupPrefix(getElementData(v, "vehicle.group")) .. "]#ffffff " .. getElementData(v, "siren.unit") .. " #ff4646erősítést #ffffffkért.", 0, 0, 0, true)
                        end
                    end
                end
            end
    ...
    if getElementType(source) == "vehicle" and dataName == "siren.status" then
        ...
        elseif newValue == 1 then -- Nem elérhető
            ...
            if playerGroups[getElementData(source, "vehicle.group")] then
                ...
                setElementData(gpsBlips[source], "blipTooltipText", "[" .. exports.nlrp_groups:getGroupPrefix(getElementData(source, "vehicle.group")) .. "] " .. getElementData(source, "siren.unit") or "Ismeretlen")
        ...
        elseif newValue == 3 then -- Erősítés
            ...
            if playerGroups[getElementData(source, "vehicle.group")] then
                ...
                if playerGroups[getElementData(source, "vehicle.group")] then
                    ...
                    setElementData(gpsBlips[source], "blipIcon", "cp")
                    --setElementData(gpsBlips[source], "blipSize", 30)
                    setElementData(gpsBlips[source], "blipTooltipText", "[ERŐSÍTÉS][" .. exports.nlrp_groups:getGroupPrefix(getElementData(source, "vehicle.group")) .. "] " .. getElementData(source, "siren.unit") or "Ismeretlen")
                    attachElements(gpsBlips[source], source)
                    setElementData(gpsBlips[source], "blipColor", tocolor(255, 66, 66))
                    setElementData(gpsBlips[source], "blipFarShow", true)
                    playSound("siren/sounds/backup.mp3" )
                    outputChatBox("#32b3ef[" .. exports.nlrp_groups:getGroupPrefix(getElementData(source, "vehicle.group")) .. "]#ffffff " .. getElementData(source, "siren.unit") .. " #ff4646erősítést #ffffffkért.", 0, 0, 0, true)
                end
            end
        end

        if oldValue == 3 then
            outputChatBox("#32b3ef[" .. exports.nlrp_groups:getGroupPrefix(getElementData(source, "vehicle.group")) .. "]#ffffff " .. getElementData(source, "siren.unit") .. " #7cc576lemondta#ffffff az erősítést.", 0, 0, 0, true)
        end
    end
...
addEventHandler("onClientResourceStart", resourceRoot, function()
    for _, player in ipairs(getElementsByType("player")) do
        local playerGroups = exports.nlrp_groups:getPlayerGroups(player)
        for k, v in ipairs(getElementsByType("vehicle")) do
            local status = getElementData(v, "siren.status")
            if status == 1 then -- Nem elérhető
                local playerGroups = exports.nlrp_groups:getPlayerGroups(localPlayer)
