...
Impound.Show = function()
    if not exports.nlrp_groups:isPlayerHavePermission(localPlayer, "impoundVehicle") then
            ...
            if isPedInVehicle(localPlayer) then
                ...
                if canGet then
                    if utf8.len(UI:getText(widgets.priceInput)) <= 0 or tonumber(UI:getText(widgets.priceInput)) <= 0 then
                        exports.nlrp_alert:showAlert("error", "A kiváltási árnak nagyobbnak kell lennie mint 0.")
                        return
                    end
                end

                if utf8.len(UI:getText(widgets.timeInput)) <= 0 or time < 0 then
                    exports.nlrp_alert:showAlert("error", "Az idő csak 0 és pozitív szám lehet.")
                    return
                end
                ...
                triggerServerEvent("nlrp_impoundS:impoundVehicle", getPedOccupiedVehicle(localPlayer), localPlayer, getPedOccupiedVehicle(localPlayer), reason, price, canGet, impoundedDate, expireDate, impoundedBy)
                Impound.Close()
            end
        end
	end
