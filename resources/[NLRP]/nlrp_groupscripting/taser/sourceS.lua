	...
	function (targetPlayer)
		if isElement(source) and isElement(targetPlayer) then
			if not getElementData(targetPlayer, "player.Tazed") then
					...
					function(player)
						if isElement(player) then
							...
							setTimer(
								function(player)
									if isElement(player) then
										exports.nlrp_controls:toggleControl(player, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
				...
				exports.nlrp_chat:sendLocalMeAction(source, "lesokkolt valakit. ((" .. getElementData(targetPlayer, "visibleName"):gsub("_", " ") .. "))")
			end
		end
	end
)