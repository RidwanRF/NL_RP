    ...
    if button == "left" and state == "down" then
        if doorComponents[activeButton] then
            ...
            if getDistanceBetweenPoints3D(pX, pY, pZ, cX, cY, cZ) <= 1 then
                exports.nlrp_minigames:startMinigame("balance", "removeDoorSuccess", "removeDoorFailed", 0.5, 10000)
                triggerServerEvent("nlrp_cutterS:applyAnimation", localPlayer, true)
                triggerServerEvent("nlrp_cutterS:playSoundEffect", localPlayer, pX, pY, pZ, true)
...
addEventHandler("removeDoorSuccess", getRootElement(), function()
    ...
    triggerServerEvent("nlrp_cutterS:removeDoor", localPlayer, vehicle, doorComponents[selectedComponent], cuttingState)
    triggerServerEvent("nlrp_cutterS:applyAnimation", localPlayer, false)
    triggerServerEvent("nlrp_cutterS:playSoundEffect", localPlayer, nil, nil, nil, false)
...
addEventHandler("removeDoorFailed", getRootElement(), function()
    exports.nlrp_hud:showAlert("error", "Nem sikerült megfelelően levágni a kijelölt pontot")
    triggerServerEvent("nlrp_cutterS:applyAnimation", localPlayer, false)
    triggerServerEvent("nlrp_cutterS:playSoundEffect", localPlayer, nil, nil, nil, false)
end)
...
addEvent("nlrp_cutterC:playSoundEffect", true)
addEventHandler("nlrp_cutterC:playSoundEffect", getRootElement(), function(x, y, z, state)
    if not state then
