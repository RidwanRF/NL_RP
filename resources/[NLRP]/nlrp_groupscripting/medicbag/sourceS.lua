...
function createMedicbag(player, cmd)
    ...
    exports.nlrp_boneattach:attachElementToBone(medicbags[player], player, 12, 0, -0.03, 0.3, 30 , 180, 0)
...
function dropMedicbag(player, pos)
    if medicbags[player] then
        ...
        if isElement(medicbags[player]) then
            exports.nlrp_boneattach:detachElementFromBone(medicbags[player])
...
addEvent("nlrp_medicbagS:dropMedicbag", true)
addEventHandler("nlrp_medicbagS:dropMedicbag", root, dropMedicbag)

--addCommandHandler("db", function(player)
    if isElement(medicbags[player]) then
        triggerClientEvent(player, "nlrp_medicbagS:calculateMedicbag", player)
    else
...
function pickupMedicbag(player, medicbag)
    if isElement(medicbag) then
        exports.nlrp_boneattach:attachElementToBone(medicbag, player, 12, 0, -0.03, 0.3, 30 , 180, 0)
    end
end
