...
function arrestPlayer(id, reason, time, bail, bailPrice)
    local targetPlayer, targetPlayerName = exports.nlrp_core:findPlayer(source, id)
    if exports.nlrp_core:inDistance3D(source, targetPlayer, 5) then
        dropPlayerToTheCell(targetPlayer, time, reason, bail, bailPrice, getPlayerName(source):gsub("_", " "))
		outputChatBox("#2185d0[RENDŐRSÉG]#FFFFFF A rendőrség bebörtönzött egy bűnözőt.",getRootElement(), 255, 255, 255, true)
		outputChatBox("#2185d0[RENDŐRSÉG]#FFFFFF Elkövető neve: #db2828"..targetPlayerName,getRootElement(), 255, 255, 255, true)
		outputChatBox("#2185d0[RENDŐRSÉG]#FFFFFF Kiszabott büntetés hossza: #db2828"..time.."#FFFFFF perc",getRootElement(), 255, 255, 255, true)
		outputChatBox("#2185d0[RENDŐRSÉG]#FFFFFF Büntetés indok: #db2828"..reason,getRootElement(), 255, 255, 255, true)
		outputChatBox("#2185d0[RENDŐRSÉG]#FFFFFF Bebörtönözte: #2185d0"..getPlayerName(source):gsub("_", " "),getRootElement(), 255, 255, 255, true)
    else
        exports.nlrp_alert:showAlert(source, "error", "A játékos nincs a közeledben")
    end
end
addEvent("nlrp_arrestS:arrestPlayer", true)
addEventHandler("nlrp_arrestS:arrestPlayer", root, arrestPlayer)
...
addCommandHandler("release", function(player, cmd, targetPlayer)
    if not exports.nlrp_groups:isPlayerHavePermission(player, "jail") then
        exports.nlrp_alert:showAlert(player, "error", "Nincs engedélyed a parancs használatához")
        return
    end
    ...
    if not targetPlayer then
        ...
        local targetPlayer, targetPlayerName = exports.nlrp_core:findPlayer(player, targetPlayer)
        if arrestedPlayers[targetPlayer] then
            ...
            exports.nlrp_alert:showAlert(player, "error", "Nincs a játékos börtönben")
        end
    end
end)
...
outputUsageText = function(text, element)
	assert(type(text) == "string", "Bad argument @ 'outputInfoText' [expected string at argument 1, got "..type(text).."]")
	outputChatBox(exports.nlrp_core:getServerTag("usage") .. text, element, 0, 0, 0, true)
end