    ...
    if button == "left" and state == "down" then
        if clickedElement then
            if getElementData(clickedElement, "isDrugPed") then
                if exports.nlrp_core:inDistance3D(localPlayer, clickedElement, 2) then
...
function getInfoFromPed(clickedPed)
    if exports.nlrp_core:takeMoney(localPlayer, infoPrice) then
        ...
        exports.nlrp_chat:sendLocalMessage(clickedPed, "Te most viccelsz?! Na húzzá' innen")
    end
end
    ...
    --if #freshPlants > 0 then
        ...
        for k, v in pairs(freshPlants) do
            for k2,v2 in pairs(v) do
                local drugCol = createColSphere(v2[1], v2[2], v2[3], 1) --createMarker(v[1], v[2], v[3] - 1, "cylinder", 3)
...
addEventHandler("onClientColShapeHit", resourceRoot, function(element)
    if element == localPlayer and drugPlants[source] then
        ...
        exports.nlrp_minigames:startMinigame("buttons", "harvestSuccess", "harvestFailed", 0.15, 0.2, 115, 20)
        triggerServerEvent("nlrp_drugS:applyAnimation", localPlayer, true, "take")
    end
end)
...
addEventHandler("harvestSuccess", root, function()
    ...
    triggerServerEvent("nlrp_drugS:applyAnimation", localPlayer)
...
addEvent("nlrp_drugC:startDrugMaking", true)
addEventHandler("nlrp_drugC:startDrugMaking", root, startDrugMaking)
...
function stopDrugMaking(type)
    ...
    triggerServerEvent("nlrp_drugS:applyAnimation", localPlayer, false)
    setElementFrozen(localPlayer, false)
    startTick = nil
    selectedDrug = nil
    setElementData(localPlayer, "drugProcessing", false)
end
addEvent("nlrp_drugC:stopDrugMaking", true)
addEventHandler("nlrp_drugC:stopDrugMaking", root, stopDrugMaking)
