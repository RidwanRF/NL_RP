						...
						if getElementData(targetPlayer, "player.Cuffed") then
							if not getElementData(targetPlayer, "player.Grabbed") then
								...
								if not isElement(grabbingPlayer) then
									...
									outputChatBox(exports.nlrp_core:getServerTag("error") .. "Egyszerre csak egy embert tudsz vezetni!", source, 0, 0, 0, true)
								end
							else
								outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékost már vezeti valaki!", source, 0, 0, 0, true)
							end
						else
							outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos nincs megbilincselve!", source, 0, 0, 0, true)
						end
					end
				else
					outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", source, 0, 0, 0, true)
				end
			end
		end
	end
...
function cuffFunction(sourcePlayer, commandName, targetPlayer)
	if exports.nlrp_groups:isPlayerHavePermission(sourcePlayer, "cuff") then
		if not targetPlayer then
			outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. commandName .. " [Játékos név / ID]", sourcePlayer, 0, 0, 0, true)
		else
			targetPlayer, targetName = exports.nlrp_core:findPlayer(sourcePlayer, targetPlayer)
			...
			if isElement(targetPlayer) then
				...
				if distance <= 5 then
					if sourceInterior == targetInterior and sourceDimension == targetDimension then
						...
						if cuffed then
							exports.nlrp_controls:toggleControl(targetPlayer, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, false)
						else
							exports.nlrp_controls:toggleControl(targetPlayer, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
						end
					end
				else
					outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", source, 0, 0, 0, true)
...
addCommandHandler("cuff",
	function (player, command, target)
		if getElementData(player, "loggedIn") then
			if exports.nlrp_groups:isPlayerHavePermission(player, "cuff") then
				if not target or utfLen(target) < 1 then
					outputChatBox(exports.nlrp_core:getServerTag("usage") .. "/" .. command .. " [Játékos név/ID]", player, 0, 0, 0, true)
				else
					target, targetName = exports.nlrp_core:findPlayer(player, target)
					...
					if target then
						...
						if getDistanceBetweenPoints3D(targetX, targetY, targetZ, localX, localY, localZ) <= 3 and getElementInterior(player) == getElementInterior(target) and getElementDimension(player) == getElementDimension(target) then
							if not getElementData(target, "player.Cuffed") then
								if exports.nlrp_inventory:hasItem(player, 77) then
									exports.nlrp_controls:toggleControl(target, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, false)
									triggerClientEvent("cuffThePlayer", player, target, true)
									setElementData(target, "player.Cuffed", true)
								else
									outputChatBox(exports.nlrp_core:getServerTag("error") .. "Nincs nálad bilincs!", player, 0, 0, 0, true)
								end
							else
								outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos már meg van bilincselve!", player, 0, 0, 0, true)
							end
						else
							outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", player, 0, 0, 0, true)
						end
					end
				end
			end
					...
					if target then
						...
						if getDistanceBetweenPoints3D(targetX, targetY, targetZ, localX, localY, localZ) <= 3 and getElementInterior(player) == getElementInterior(target) and getElementDimension(player) == getElementDimension(target) then
							if getElementData(target, "player.Cuffed") then
								if exports.nlrp_inventory:hasItem(player, 78) then
									exports.nlrp_controls:toggleControl(target, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
									triggerClientEvent("cuffThePlayer", player, target, false)
									setElementData(target, "player.Cuffed", false)
								else
									outputChatBox(exports.nlrp_core:getServerTag("error") .. "Nincs nálad bilincs kulcs!", player, 0, 0, 0, true)
								end
							else
								outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos nincs bilincselve!", player, 0, 0, 0, true)
							end
						else
							outputChatBox(exports.nlrp_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", player, 0, 0, 0, true)
						end
