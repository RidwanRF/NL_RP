    ...
    if Detain.Active then
        ...
        if Detain.TargetPlayer then
            ...
            if button == "left" and state == "down" then
                print("2")
                if exports.nlrp_core:inDistance3D(localPlayer, clickedElement, 10) then
                    print("3")
                    if clickedElement ~= localPlayer then
                        ...
                        if getElementType(clickedElement) == "vehicle" then
                            ...
                            if isElement(Detain.TargetVehicle) then
                                triggerServerEvent("nlrp_detainS:detainPlayer", localPlayer, Detain.TargetVehicle, Detain.TargetPlayer)
...
addEvent("nlrp_detainC:detainMode", true)
addEventHandler("nlrp_detainC:detainMode", root, function(target)
    Detain.Active = not Detain.Active
...
outputErrorText = function(text, element)
    triggerEvent("playClientSound", element, ":nlrp_assets/audio/admin/error.ogg")
...
outputInfoText = function(text, element)
	...
	outputChatBox(exports.nlrp_core:getServerTag("info") .. text, 0, 0, 0, true)
end