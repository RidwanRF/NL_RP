...
function createSnowBall()
	if getElementDimension(localPlayer) ~= 0 then
		exports.nlrp_hud:showInfobox("error", "Interiorban nem tudsz hógolyót gyúrni!")
	else
				...
				if state == 0 then
					...
					if isElement(snowballObject) then
						...
						if source == localPlayer then
							exports.nlrp_controls:toggleControl({"fire"}, false)
						end
					end
				elseif state == 1 then
					...
					setTimer(
						function(player)
							if player == localPlayer then
								...
								exports.nlrp_controls:toggleControl({"fire"}, true)
							end
