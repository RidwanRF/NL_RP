		...
		if source == localPlayer then
			...
			elseif dataName == "loggedIn" and getElementData(localPlayer, "loggedIn") and getElementData(localPlayer, "isPlayerDeath") then
				...
				if getElementData(localPlayer, "player.Cuffed") then
					exports.nlrp_controls:toggleControl({"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
				end
...
addEventHandler("onClientPlayerWasted", localPlayer,
	function (killer, weapon, bodyPart)
		if source == localPlayer then
			...
			triggerServerEvent("takeMoneyPrecentOnDeath",resourceRoot,localPlayer)
			exports.nlrp_inventory:removeWeaponsOnDeath()
				...
				elseif bloodLevel > 0 and bloodLevel <= 40 then
					if math.random(10) <= 5 then
						if not cameraShake then
							exports.nlrp_hud:showInfobox("warning", "Az alacsony vérszinted miatt elkezdtél szédülni!")
	...
	function(sourcePlayer)
		if getTickCount() - lastTryTick >= 1000 then
			...
			if getElementHealth(sourcePlayer) <= 20 and getElementHealth(sourcePlayer) > 0 then
				...
				-- van defi -> sokkolás
				if exports.nlrp_inventory:hasItem(107) then
				...
				exports.nlrp_hud:showAlert("error", "A kiválasztott játékos nem halott vagy már nem lehet megmenteni!")
			end
		end
	end)
			...
			if damages > 0 then
				...
				for k, v in pairs(bulletDamages) do
					...
					elseif damageType == "cut" then
						exports.nlrp_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg vágás van a testén!")
						return
					elseif tonumber(damageType) >= 25 and tonumber(damageType) <= 27 then
						exports.nlrp_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg sörétek vannak a testében!")
						return
					else
						exports.nlrp_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg golyó vannak a testében!")
						return
					end
				end

				if (getElementData(sourcePlayer, "bloodLevel") or 100) ~= 100 then
					exports.nlrp_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg nem állítod el a vérzést!")
					return
				end
				]]

				-- van eset táska
				if exports.nlrp_inventory:hasItem(108) then
					
				else
					exports.nlrp_hud:showAlert("error", "Nincs nálad eset táska, ezért nem tudod ellátni a sérültet!")
				end
			else
				exports.nlrp_hud:showAlert("error", "A kiválasztott játékos nincs megsérülve!")
			end
		end
	end)
