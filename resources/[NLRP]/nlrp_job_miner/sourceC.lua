...
addEventHandler("onClientResourceStart", resourceRoot, function ()
	loadFonts()
	jobPed = createPed(260, -776.89227294922, -1858.7603759766, 11.92, 180)
    ...
    dropPed = createPed(27, -18.906414031982, -270.22393798828, 5.4296875, 180)
...
function startJob()
    exports.nlrp_hud:showAlert("info", "Sikeresen elkezdted a bányászatot! Menj a bányába", "majd kezd el a bányászatot a megfelelő helyen")
    jobActive = true
end

function stopJob()
    exports.nlrp_hud:showAlert("info", "Befejezted a bányászatot!")
    jobActive = false
end
...
function startMining()
    if currentMiningValue < 100 then
        ...
        triggerServerEvent("nlrp_miningS:startMining", localPlayer)
        if currentMiningValue >= 100 then
...
function stopMining()
    triggerServerEvent("nlrp_miningS:stopMining", localPlayer)
    --print("STOP")
    showMiningGame()
end
...
registerEvent("nlrp_miningC:playMiningSound", root, function()
    local x, y, z = getElementPosition(source)
	playSound3D(":nlrp_assets/audio/jobs/miner/mining.mp3", x, y, z)
end)
...
addEventHandler("onClientRender", root, function()
    if isElementWithinColShape(localPlayer, mineColshape) and jobActive and not showMinigame and not showOres then
        ...
        if startTick then
            ...
            dxDrawRectangle(barX + 2, barY + 2, (barW - 4) * barFill, barH - 4, tocolor(183,255,0, 240))  
        end  
    end
    ...
    elseif showOres then
		...
		if isCursorShowing() then
			...
			local cursorX, cursorY = relX * screenX, relY * screenY
			setCursorAlpha(0)
			dxDrawImage(cursorX - respc(10), cursorY - respc(10), pickaxeW, pickaxeH, ":nlrp_assets/images/jobs/miner/zsak.png", pickaxeRX, 0, 0)
		end
                ...
                if getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, pedPosX, pedPosY, pedPosZ) <= 3 then
                    ...
                    for k, v in pairs(ores) do
                        ...
                        if exports.nlrp_inventory:hasItem(itemID) then
                            triggerServerEvent("nlrp_miningS:takeOres", localPlayer) 
                            hasOres = true
                            break
                        end
                    end    
                    ...
                    if not hasOres then
                        exports.nlrp_alert:showAlert("error", "Nincs nálad semmilyen érc")
                    end
                end
            end
        end
            ...
            if state == "down" then
                if cursorInBox(crosshairX, crosshairY, crosshairW, crosshairH) then
                    if getTickCount() - lastClickTick >= 350 then -- 250 ms
                        ...
                        -- Kő zúzása
                        playSound(":nlrp_assets/audio/jobs/miner/pickaxe.mp3")
        ...
        elseif showOres then
            for k, v in pairs(receivedOres) do
                ...
                if cursorInBox(v[2], v[3], oreW, oreH) then
                    ...
                    triggerServerEvent("nlrp_miningS:giveOre", localPlayer, ores[v[1]][4])
                end
            end
        end
    end
