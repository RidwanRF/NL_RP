...
local settingsMenu = {"Háttérkép", "Csengőhang"}

--local alsoMenu = {"phone", "news_alt", "contacts2", "sms"}

local alsoMenu = {"phone", "news_alt", "contacts2", "comic"}
...
local menuNames = {"settings", "pd", "es", "rep"}
...
setElementData(getLocalPlayer(),"phoneShowing", false)



function showPhoneFunction(itemValue)

	if showPhone or itemValue == nil then

		exports.nlrp_chat:sendLocalMeAction(localPlayer,"elrakja a telefonját a zsebébe.")
		...
		exports.nlrp_chat:sendLocalMeAction(localPlayer,"előveszi a telefonját a zsebéből.")
...
function renderPhone()
	...
	if (tonumber(menu) == 1) then
		...
		local celsius = getElementData(root,"weather.Winter") or "Nincs"
		...
		dxDrawRectangle(pX+200,pY+475,230,50, tocolor(70, 70, 70, 200))
...
showChat(false)
        ...
        setElementData(getLocalPlayer(),"phoneShowing", true)
		...
		if tonumber(currentChat) < 0 then
			...
			if (messages[currentChat]) then
				...
				dxDrawText("Telefonszám: " .. currentChat,pX +210 + 210/2, pY + 165, pX +210-15 + 210/2, 50, tocolor(0, 0, 0, 255), 1, "default", "center", "top", false, false, true, true)
			...
			dxDrawText("Elküld",pX-15,pY+377.5,bgImgSize[1] + pX-15,22.5 + pY+377.5, tocolor(255, 255, 255, 255), 1, "sans", "center", "center", false, false, true, true)
	...
	elseif (tonumber(menu) == 3) then
		...
		dxDrawText("Telenor",pX+210,pY+147,1,1, tocolor(29,172,248, 255), 0.9, "default-bold", "left", "top", false, false, true, true)
	...
	elseif (tonumber(menu) == 5) then
		...
		dxDrawImage(pX+100,pY,bgImgSize[1],bgImgSize[2], "files/ad1.png", angle, 0, -120, tocolor(255,255,255,255), false)--Háttérkép
	...
	elseif (tonumber(menu) == 4) then
		...
		if addContact or editContact then
			...
			local addContactTable = {"Telefonszám: ", guiGetText(createdGuis[1]), "Név: ", guiGetText(createdGuis[2])}
			...
			if editContact then
				...
				if isInSlot(pX+210,pY+505,210,20) then
					...
					dxDrawText("Szerkeszt", pX+210+210/2,pY+502,pX+210+210/2,0, tocolor(0, 0, 0, 255), 1.0, "default", "center", "top", false, false, true, true)

				else

					dxDrawText("Szerkeszt", pX+210+210/2,pY+502,pX+210+210/2,0, tocolor(255, 255, 255, 255), 1.0, "default", "center", "top", false, false, true, true)
				...
				if isInSlot(pX+210,pY+505,210,20) then
					...
					dxDrawText("Felvétel", pX+210+210/2,pY+502,pX+210+210/2,0, tocolor(0, 0, 0, 255), 1.0, "default", "center", "top", false, false, true, true)

				else

					dxDrawText("Felvétel", pX+210+210/2,pY+502,pX+210+210/2,0, tocolor(255, 255, 255, 255), 1.0, "default", "center", "top", false, false, true, true)
			...
			if isInSlot(pX+210,pY+505,210,20) then
				...
				dxDrawText("Felvétel", pX+210+210/2,pY+502,pX+210+210/2,0, tocolor(0, 0,0, 255), 1, "default", "center", "top", false, false, true, true)

			else

				dxDrawText("Felvétel", pX+210+210/2,pY+502,pX+210+210/2,0, tocolor(255, 255, 255, 255), 1, "default", "center", "top", false, false, true, true)
	...
	elseif (tonumber(menu) == 6) then
		...
		dxDrawText("\n" .. getNumber(numberCall),pX+210+210/2,pY+235,pX+210+210/2,1, tocolor(255, 255, 255, 255), 1.0, "sans", "center", "top", false, false, true, true)
	...
	elseif (tonumber(menu) == 7) then
		...
		dxDrawText(getNumber(numberCall) .." hívása...",pX+210+210/2,pY+235,pX+210+210/2, 0, tocolor(255, 255, 255, 255), 1.0, "sans", "center", "top", false, false, true, true)
	...
	elseif (tonumber(menu) == 8) then
		...
		dxDrawText("Hívásban: " .. numberCall,pX+255,pY+165,bgImgSize[1] + pX-15,20 + pY+50, tocolor(0, 0, 0, 255), 1, "sans", "left", "top", false, false, true, true)
	...
	elseif (tonumber(menu) == 10) then
		...
		if isInSlot(pX+205,pY+495,213,30) then
			...
			dxDrawText("Beállítás",pX+205 + 50,pY+495,213 - 100 + pX+205 + 50,30 + pY+495, tocolor(2124, 197, 118, 255), 1, "default-bold", "center", "center", false, false, true, true)

		else

			dxDrawText("Beállítás",pX+205 + 50,pY+495,213 - 100 + pX+205 + 50,30 + pY+495, tocolor(255, 255, 255, 255), 1, "default-bold", "center", "center", false, false, true, true)
	...
	elseif (tonumber(menu) == 11) then
		...
		for i = 1, 10 do
			...
			if currentRing == i then
				...
				dxDrawText("Csengőhang " .. i,pX+205,pY+150 + i * 30,210 + pX+205,26 + pY+150 + i * 30, tocolor(255, 255, 255, 255), 1, "default", "center", "center", false, false, true, true)
		...
		if currentRing > 0 then
			...
			if isInSlot(pX+204,pY+490, 30,214,35) then
				...
				dxDrawText("Beállítás",pX+204,pY+500,214 + pX+204,0, tocolor(124, 197, 118, 255), 1, "sans", "center", "center", false, false, true, true)

			else

				dxDrawRectangle(pX+204,pY+490,214,35, tocolor(0, 0, 0, 170), true) -- alsó

				dxDrawText("Beállítás",pX+204,pY+500,214 + pX+204,0, tocolor(255, 255, 255, 255), 1, "sans", "center", "top", false, false, true, true)
	...
	if (button == "left") and (state == "down") and (showPhone) and loadedPhoneInterFace then
			...
			for k, v in ipairs(menuNames) do
				...
				if isInSlot(pX +215 + column * (iconSize[1] + 10),pY+170 + rows * (iconSize[1] + 10),iconSize[1], iconSize[2]) then
                    ...
                    elseif tonumber(k) == 2 then
						...
						triggerServerEvent("sendMessageToPD", localPlayer, localPlayer, "Értesítettük a Rendőrséget!")

                    elseif tonumber(k) == 3 then

						triggerServerEvent("sendMessageToES", localPlayer, localPlayer, "Értesítettük a Mentőket!")

                    elseif tonumber(k) == 4 then

						triggerServerEvent("sendMessageToSERV", localPlayer, localPlayer, "Értesítettük a Szerelőket!")
		...
		elseif (tonumber(menu) == 3) then
			...
			elseif isInSlot(pX+205,pY+502,210,20) then
				...
				if tonumber(#guiGetText ( createdGuis[1] )) >= 4 then
					...
					if klikkTimer then outputChatBox("#d75959[Hiba]: #ffffffPercenként csak egy hirdetést adhatsz fel!", 255, 255, 255, true) return end
					...
					outputChatBox("#d75959[Hiba]: #ffffffMinimum 4 karaktert meg kell adnod a hirdetésben!", 255, 255, 255, true)
		...
		elseif (tonumber(menu) == 5) then
			...
			elseif isInSlot(pX+205,pY+502,210,20) then
				...
				if tonumber(#guiGetText ( createdGuis[1] )) >= 4 then
					...
					triggerServerEvent("onClientCallForAdData2", localPlayer, localPlayer,tostring(guiGetText(createdGuis[1])),(tonumber(#guiGetText ( createdGuis[1] ))*0.58), actualPhoneID)
				...
				if isInSlot(pX+205,pY+502,210,20) then
					...
					if string.len(guiGetText(createdGuis[1])) > 2 then
						...
						if string.len(guiGetText(createdGuis[2])) > 5 then
							...
							triggerServerEvent("sendMessagesInServer", localPlayer, localPlayer, actualPhoneID, guiGetText(createdGuis[1]), guiGetText(createdGuis[2]), getTime(), getDate())
				...
				if isInSlot(pX+205,pY+502,210,20) then
					...
					if tonumber(currentChat) > 0 then
						...
						if string.len(guiGetText(createdGuis[1])) > 2 then
							...
							triggerServerEvent("sendMessagesInServer", localPlayer, localPlayer, actualPhoneID, currentChat, guiGetText(createdGuis[1]), getTime(), getDate())
		...
		elseif (tonumber(menu) == 4) then
			...
			if addContact or editContact then
				...
				for i = 1, 2 do
					...
					if isInSlot(pX+206, pY + 210 + (i) * 70 - 50, 210, 30) then-- Ez kell még
		...
		elseif (tonumber(menu) == 10) then
			...
			if isInSlot(pX+205,pY+495,213,30) then
				...
				outputChatBox("Sikeresen beállítottad háttérképnek.", 255, 255, 255, true)
		...
		elseif (tonumber(menu) == 11) then
			...
			if isInSlot(pX+204,pY+490,214,35) then
				...
				outputChatBox("Sikeresen beállítottad a csengőhangodat.", 255, 255, 255, true)
...
function callMemberClient(number)
	...
	if tonumber(number) then
		...
		if tonumber(number) > 0 then
			...
			triggerServerEvent("callTargetInServer", localPlayer, localPlayer, number, actualPhoneID)
	...
	if message and ownerPhone and date and time then
		...
		elseif (tonumber(numberCall) == 908) then
			...
			setTimer(function()
				...
				callMessages[#callMessages + 1] = {"Rendben, kérem maradjon a helyszínen, egy vontatót máris küldünk!", 908}
...
function addContactMemberC(name, number, id)
	...
	if (name) and tonumber(number) and tonumber(id) then
		...
		currentContactPos = currentContactPos + 2
...
local callMarkerOwner = {}



function acceptCallInClient(pX, pY, pZ, player)

	local blip_ID = exports.nlrp_hud:addCustomBlip({pX,pY,pZ,"package.png",22,true})

	callBlip[#callBlip + 1] = blip_ID

	--callBlip[#callBlip + 1] = createBlip(pX, pY, pZ, 56)
	...
	callMarkerOwner[#callMarkerOwner + 1] = player

	setElementData(callMarker[#callMarker], "callMarker:id", #callMarker)

	setElementData(callMarker[#callMarker], "callMarker:blipID",blip_ID)
...
function onMarkerHit(playerSource)
	...
	if callMarker[getElementData(source, "callMarker:id")] and (callMarkerOwner[getElementData(source, "callMarker:id")] == playerSource) then

		exports.nlrp_hud:delCustomBlip(getElementData(source, "callMarker:blipID"))
		...
		outputChatBox("#F89406[Riasztás] #ffffffSikeresen megérkeztél a helyszínre!",255,255,255,true)
