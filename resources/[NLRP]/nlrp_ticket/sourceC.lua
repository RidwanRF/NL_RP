...
function showTicket(itemDbId, state, ticket, data, edit)	
	...
	if activeTicket then
		...
		if ticketItemDatabaseId then
			exports.nlrp_inventory:unuseItem(ticketItemDatabaseId)
		end
	end
...
registerEvent("nlrp_ticketC:showTicket", root, showTicket)

payPed = nil

addEventHandler("onClientResourceStart", resourceRoot, function()
	payPed = createPed(287,246.52331542969, 120.40056610107, 1003.2637329102, 180)
	setElementFrozen(payPed, true)
    setElementDimension(payPed, 187)
    setElementInterior(payPed, 10)
	setElementData(payPed, "invulnerable", true)
	setElementData(payPed, "visibleName", "Madison Alvarez")
