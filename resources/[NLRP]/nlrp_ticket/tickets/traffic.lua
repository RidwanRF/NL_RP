...
Tickets["Traffic"]["Render"] = function(TicketData, isEdit)
	...
	if isEdit then
		...
		drawInput("money|num-only|9", ticketX + respc(365), ticketY + respc(210), respc(355), inputH, fonts.handFont, 0.7, tocolor(0, 84, 166), "Ft")
		...
		if signStart then
			...
			if progress > 1.25 then
				...
				exports.nlrp_chat:sendLocalMeAction(localPlayer, "kitépi a bírságot a füzetből.")	
			end
		end
	else
		...
		if inEdit then
			if cursorInBox(ticketX + respc(355 + 7 + 147), ticketY + respc(291), respc(180), inputH) then
				...
				if utf8.len(inputValues["name|-|28"]) <= 4 then
					exports.nlrp_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Név\" rész kevesebb, mint 4 karakter")
					return
				end

				if utf8.len(inputValues["reason|-|40"]) <= 5 then
					exports.nlrp_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Szabálysértés/Vétség\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["officer|-|28"]) <= 5 then
					exports.nlrp_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Kiállító személy\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["agency|-|40"]) <= 5 then
					exports.nlrp_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Hatóság\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["money|num-only|9"]) <= 1 then
					exports.nlrp_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Bírság összege\" rész kevesebb, mint 1 karakter")
					return
				end

				if utf8.len(inputValues["date|-|28"]) <= 7 then
					exports.nlrp_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Kiállítás időpontja\" rész kevesebb, mint 7 karakter")
					return
				end
				...
				if targetPlayer == localPlayer then
					exports.nlrp_hud:showAlert("error", "Magadnak nem állíthatsz ki csekket!")
					return
				end

				if not targetPlayer then
					exports.nlrp_hud:showInfobox("error", "A \"Név\" résznél beírt személy nincs a közeledben!")
					return
				end

				exports.nlrp_chat:sendLocalMeAction(localPlayer, "elkezd megírni egy csekket.")
				...
				local data = {
					...
					["fine"] = inputValues["money|num-only|9"],
