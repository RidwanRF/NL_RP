...
local loadPoint = {2625.2048339844, 1191.6026611328, 31.632537841797} -- 992.74938964844, -1353.0140380859, 13.354413032532
local sellPoints = {
...
addEventHandler("onClientResourceStart", resourceRoot, function ()
	loadFonts()
	jobPed = createPed(20,  992.74938964844, -1353.0140380859, 13.354413032532, 90) --1027.8881835938, -1358.2156982422, 13.7265625
...
function startIceCreamJob()
	exports.nlrp_hud:showAlert("info", "Sikeresen felvetted a fagylaltos munkát! Menj a telepre, szállj be egy járműbe", "majd kezd el a fagylaltok árúsítását a megfelelő helyen")
	generateFillPoint()
	generateSellPoint()
	jobActive = true
	exports.nlrp_jobs:createVehiclePoint(2, 1015.725402832, -1355.8988037109, 13.373134613037, 423, {255, 255, 255}, 2)
end
...
function stopIceCreamJob()
	...
	exports.nlrp_jobs:destroyVehiclePoint()
	jobActive = false
end
	...
	if button == "left" and state == "down" then
		if inSellingMode then
			for k, v in pairs(containers) do
				if activeContainer == k then
					if iceCreams[k][4] > 0 then 
						if selectedCone then
							if scoopInHand then
								exports.nlrp_hud:showInfobox("error", "Már van egy gombóc a kezedben")
							else
								iceCreams[k][4] = iceCreams[k][4] - 10	
								scoopInHand = k
							end
						else 
							exports.nlrp_hud:showInfobox("error", "Először válasz tölcsért")
						end
					else
						exports.nlrp_hud:showInfobox("error", "Ez az ízű fagylalt elfogyott, menj vissza a telepre és töltsd fel a készleteidet!")
					end
				end
			end
			...
			for k, v in pairs(cones) do
				if activeCone == k then
					if iceCreamCones[k][3] > 0 then
						...
						exports.nlrp_hud:showInfobox("error", "Ez a tölcsér elfogyott")
					end
					break
				end
			end
			...
			if activeButton == "button:noserve" then
				...
				selectedCone = nil
				scoopInHand = nil
				exports.nlrp_hud:showInfobox("info", "Nem szolgáltad ki a vevőket")
				currentOrder = nil
			...
			if selectedCone then
				if cursorInBox(iceCreamX, iceCreamY, iceCreamSize, iceCreamSize) then
					if #scoops >= 6 then
						exports.nlrp_hud:showInfobox("error", "Több gombócot már nem tudsz rátenni")
						clearHand()
					else
		...
		if clickedWorld == jobPed then
			if getElementData(localPlayer, "char.Job") == JOB_ID then
				...
				if getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, pedPosX, pedPosY, pedPosZ) <= 5 then
					if jobActive then
						stopIceCreamJob()
						exports.nlrp_hud:showAlert("info", "Befejezted a munkát!")
					else
						startIceCreamJob()
					end
				end
			else
				exports.nlrp_hud:showAlert("error", "Nem vagy fagylaltos!")
			end
		end
 	end
end)
...
function giveIceCreamToCustomer()
	if currentOrder.cone == selectedCone then
		if deepcompare(scoops, currentOrder.scoops) then
			selectedCone = nil
			scoopInHand = nil
			exports.nlrp_hud:showInfobox("info", "Átadtad a fagyit! Kaptál " .. #currentOrder.scoops * math.random(10,200) .. "$")
			exports.nlrp_core:giveMoney(localPlayer, #currentOrder.scoops * math.random(10,200))
			scoops = {}
			currentOrder = nil
			createOrder()
		else
			exports.nlrp_hud:showInfobox("error", "A fagyi nem egyezik meg a vevő kérésével!")
		end
	else
		exports.nlrp_hud:showInfobox("error", "A tölcsér nem egyezik a kért tölcsérrel!")
	end
end
		...
		if isPedInVehicle(localPlayer) then
			...
			if getElementModel(playerVehicle) == 423 then
				if source == fillMarker then
					...
					exports.nlrp_hud:showAlert("info", "Sikeresen feltöltötted a készleteket")
