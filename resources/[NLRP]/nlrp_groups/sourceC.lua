...
function loadFonts()
	local fonts = {
		...
		Roboto16 = exports.nlrp_assets:loadFont("Roboto-Regular.ttf", resp(16), false, "cleartype"),
		...
		gtaFont2 = exports.nlrp_assets:loadFont("gtaFont2.ttf", resp(40), false, "default"),
		Themify60 = exports.nlrp_assets:loadFont("Themify.ttf", respc(60), false, "cleartype"),
		RobotoB18 = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", respc(18), false, "antialiased"),
		Themify18 = exports.nlrp_assets:loadFont("Themify.ttf", respc(18), false, "cleartype"),
		RobotoL14 = exports.nlrp_assets:loadFont("Roboto-Light.ttf", respc(14), false, "cleartype"),
		RobotoB14 = exports.nlrp_assets:loadFont("Roboto-Bold.ttf", respc(14), false, "cleartype"),

		metro18 = exports.nlrp_assets:loadFont("metro.ttf", respc(18), false, "cleartype"),
		metro60 = exports.nlrp_assets:loadFont("metro.ttf", respc(60), false, "cleartype"),
	}
...
local boundColor = tocolor(170,170,170, 75)
	...
	function (hitElement, matchingDimension)
		if hitElement == localPlayer and matchingDimension and getElementData(source, "dutyColShape") then
			...
			if getElementData(hitElement, "groupDuty") then
				exports.nlrp_hud:showInfobox("info", "Nyomj [E] gombot a szolgálat leadásához.")
			else
				exports.nlrp_hud:showInfobox("info", "Nyomj [E] gombot a szolgálat felvételéhez.")
			end
		end
	end
)
...
bindKey("E", "down",
	function ()
		if standingColShape and isPlayerInGroup(localPlayer, standingColShape) then
			if getTickCount() >= lastDutyTick + 10000 then
				lastDutyTick = getTickCount()
				triggerServerEvent("requestDuty", localPlayer, standingColShape)
			else
				exports.nlrp_hud:showInfobox("error", "Csak 10 másodpercenként dutyzhatsz.")
			end
		end
	end
)
	...
	function (dataName)
		...
		if getElementType(source) == "vehicle" and monitoredDatasForVehicle[data] and monitoredGroupVeh[source] then
			if not vehicleDatas[source] then
				vehicleDatas[source] = {}
				vehicleDatas[source].vehicleName = exports.nlrp_mods_veh:getVehicleName(source)
			end
	...
	if playerGroups then
		...
		for k, v in ipairs(getElementsByType("vehicle")) do
			...
			if groupVehicles[groupID] then
				...
				vehicleDatas[v].vehicleName = exports.nlrp_vehs:getVehicleName(v)
			end
		end
...
function groupsCommandHandler()
	if getElementData(localPlayer, "loggedIn") then
		...
		elseif groupsIsLoading then
			exports.nlrp_hud:showAlert(":nlrp_assets/images/cross_x.png;error;215;89;89", "Frakciók betöltése folyamatban van!", "Próbáld pár másodperc múlva.")
		elseif getTickCount() - openedTime >= 2000 then
			...
			if playerGroupsCount > 0 then
				...
				exports.nlrp_hud:showAlert(":nlrp_assets/images/cross_x.png;error;215;89;89", "Nem tartozol egy frakció tagjai közé sem.")
			end

			openedTime = getTickCount()
		else
			exports.nlrp_hud:showAlert(":nlrp_assets/images/cross_x.png;error;215;89;89", "Maximum 2 másodpercenként nyithatod meg a frakciópanelt!")
		end
	end
end
...
exports.nlrp_admin:addAdminCommand("factions", 6, "Frakciók kezelése")
addCommandHandler("factions",
	function ()
		if monitoredLocalDatas["acc.adminLevel"] >= 7 then
			...
			elseif getTickCount() - openedTime >= 2000 then
				...
				exports.nlrp_hud:showAlert(":nlrp_assets/images/cross_x.png;error;215;89;89", "Maximum 2 másodpercenként nyithatod meg a frakció kezelő panelt!")
			end
		end
	end
)
...
function drawGroupCreator()
	...
	--dxDrawImage(math.ceil(x + marginOffset), math.ceil(y + marginOffset), logoSize, logoSize, "files/headerLogo.png", 0, 0, 0, tocolor(183,255,0, 255 * alphaAnim[3]))
	dxDrawText("NextLevel", x + logoSize + marginOffset*2, y, 0, y + headerHeight, tocolor(255, 255, 255, 255 * alphaAnim[3]), 1, Roboto14, "left", "center")
	...
	dxDrawImage(x + sx - closeSize - marginOffset, y + (headerHeight - closeSize) / 2, closeSize, closeSize, ":nlrp_assets/images/cross_x.png", 0, 0, 0, tocolor(closeColor[1], closeColor[2], closeColor[3], closeColor[4] * alphaAnim[3]))
	...
	if renderData.editGroup then
		...
		if errorToDisplay == "deleteGroup" and string.len(errorText) > 0 then
			...
			drawButton("errorOk", "Nem", x + sx - closeSize - marginOffset*2 - acceptWidth, y + (headerHeight - buttonHeight) / 2, acceptWidth, buttonHeight, {183,255,0})
		else
	...
	for i = 1, #groupCreatorMenu do
		...
		if container then
			...
			if adminTabAfterInterpolation == i then
				colorOfContainer = {processColorSwitchEffect("menuTab:" .. i, {183,255,0, 255})}
	...
	if selectedAdminTab == 1 then
		...
		drawButton("addDutyPos", "Duty pozíció felvétele", x, y, listWidth, respc(35), {183,255,0, 255})
		...
		for i = 1, 8 do
			...
			if thisItem then
				...
				dxDrawImage(x + listWidth - removeIconSize - 10, itemY + (oneItemHeight - removeIconSize) / 2, removeIconSize, removeIconSize, ":nlrp_assets/images/cross_x.png", 0, 0, 0, tocolor(removeIconColor[1], removeIconColor[2], removeIconColor[3], removeIconColor[4] * alphaMultipler))
		...
		drawButton("addDutySkin", "Hozzáad", x2 + listWidth - acceptWidth, y, acceptWidth, respc(35), {183,255,0, 255})
		...
		for i = 1, 8 do
			...
			if thisItem then
				...
				dxDrawImage(x2 + listWidth - removeIconSize - 10, itemY + (oneItemHeight - removeIconSize) / 2, removeIconSize, removeIconSize, ":nlrp_assets/images/cross_x.png", 0, 0, 0, tocolor(removeIconColor[1], removeIconColor[2], removeIconColor[3], removeIconColor[4] * alphaMultipler))
		...
		for i = 1, 8 do
			...
			if activeButton == "togDutyPermission:" .. i + renderData.permissionsOffset then
				if renderData.dutyPermissions[thisItem] then
					...
					colorOfText = tocolor(183,255,0, 255 * alphaMultipler)
				end
			end
	...
	if selectedAdminTab == 2 then
		...
		if not renderData.availableItems then
			renderData.availableItems = exports.nlrp_inventory:getItemList()
		...
		for i = 1, 10 do
			...
			if activeButton == "addItem:" .. i + renderData.availableItemsOffset then
				colorOfRow = tocolor(183,255,0, 150 * alphaMultipler)
			elseif i % 2 ~= 0 then
				colorOfRow = tocolor(53, 53, 53, 100 * alphaMultipler)
			else
				colorOfRow = tocolor(53, 53, 53, 50 * alphaMultipler)
		...
		for i = 1, 10 do
			...
			if thisItem then
				...
				dxDrawImage(x + listWidth - removeIconSize - 10, itemY + (oneItemHeight - removeIconSize) / 2, removeIconSize, removeIconSize, ":nlrp_assets/images/cross_x.png", 0, 0, 0, removeIconColor)
		...
		if renderData.selectedDutyItem then
			...
			if errorToDisplay == "acceptItemSettings" and string.len(errorText) > 0 then
				...
				drawButton("errorOk", "OK", x + inputWidth, y, acceptWidth, respc(35), {183,255,0})
			else
				...
				drawButton("acceptItemSettings", "Mentés", x + inputWidth*2 + respc(5)*2, y, acceptWidth, respc(35), {183,255,0, 255})
			end
		end
	end
...
function drawGroupPanel()
	...
	dxDrawImage(x + sx - closeSize - marginOffset, y + marginOffset, closeSize, closeSize, ":nlrp_assets/images/cross_x.png", 0, 0, 0, tocolor(closeColor[1], closeColor[2], closeColor[3], closeColor[4] * alphaAnim[3]))
	...
	if selectedTab == 1 then
		...
		-- ** Rangok
		dxDrawRectangle(cardX, y, cardWidth, cardHeight, tocolor(183,255,0, 200 * alphaMultipler))
		...
		for i = 1, #thisRanks do
			...
			if thisRankCount[i] then
				...
				dxDrawRectangle(tileX, tileY, w, tileHeightProgress, tocolor(183,255,0, 150 * alphaMultipler))
	...
	if selectedTab == 2 then
		...
		for i = 1, renderData.visibleRanks do
			...
			if thisRank then
				...
				dxDrawText("Ft " .. thousandsStepper(thisRank.pay, ","), x + listWidth, rowY, x + listWidth - 10, rowY + listRowHeight, tocolor(255, 255, 255, 255 * alphaMultipler), 0.75, Roboto16, "right", "center")
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			if #thisRanks < maxGroupRank then
				drawButton("newRank", "+ Új rang hozzáadása", x, y, listWidthOrig, listRowHeight, {183,255,0})
		...
		if selectedRank and thisRanks[selectedRank] then
			...
			dxDrawText(rank.name .. " (" .. selectedRank .. ")", x, y, 0, 0, tocolor(183,255,0, 255 * alphaMultipler), 1, RobotoB18, "left", "top")

			y = y + RobotoB18H

			dxDrawText("Fizetés: ", x, y, 0, 0, tocolor(255, 255, 255, 255 * alphaMultipler), 1, Roboto16, "left", "top")
			dxDrawText("#32b3ef" .. (rank.pay or 0) .. " #eaeaeaFt", x + dxGetTextWidth("Fizetés: ", 1, Roboto16) + resp(5), y - resp(5), 0, 0, tocolor(255, 255, 255, 255 * alphaMultipler), 0.5, gtaFont2, "left", "top", false, false, false, true)
				...
				for i = 0, #thisRanks > 1 and 2 or 1 do
					...
					if i == errorToDisplay and string.len(errorText) > 0 then
						if errorToDisplay < 2 then
							...
							drawButton("errorOk", "OK", buttonX + buttonWidth, buttonY, acceptWidth, listRowHeight, {183,255,0})
						else
							dxDrawText(errorText, buttonX, buttonY, buttonX + buttonWidth, buttonY + listRowHeight, tocolor(255, 255, 255, 230 * alphaMultipler), 1, Roboto14, "left", "center", true, false, false, true)

							drawButton("promptDecline", "Nem", buttonX + buttonWidth, buttonY, acceptWidth, listRowHeight, {183,255,0})
						...
						if i < 2 then
							...
							drawButton(caption, "OK", buttonX + buttonWidth, buttonY, acceptWidth, listRowHeight, {183,255,0})
						else
	...
	if selectedTab == 3 then
			...
			if thisMember then
				...
				elseif thisMember.isLeader == "Y" then
					if isHovered then
						...
						dxDrawText("", x + 15, rowY, 0, rowY + listRowHeight, tocolor(183,255,0, 255 * alphaMultipler), 0.5, metro18, "left", "center")
					end
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			drawInputWithErrorDisplay("addNewMember", "Hozzáadandó játékos névrészlet/ID-je", x, y, listWidthOrig, respc(40), {183,255,0}, "", "Hozzáad", false, metro18, 0.75, 0.5)
		...
		if selectedMember and thisMembers[selectedMember] then
			...
			elseif member.isLeader == "Y" then
				dxDrawText("", x, y, 0, y + RobotoL18H, tocolor(183,255,0, 255 * alphaMultipler), 0.75, metro18, "left", "center")
			...
			if meInGroup[selectedGroup].isLeader == "Y" then
				...
				if not thisIsMe and member.charID ~= thisGroup.mainLeader then
					if member.isLeader == "Y" then
						drawButtonWithPrompt("removeLeaderFromMember", "Leader elvétele", buttonX, buttonY, buttonWidth, buttonHeight, {183,255,0}, "")
					else
						drawButtonWithPrompt("addLeaderToMember", "Hozzáadás a Leaderekhez", buttonX, buttonY, buttonWidth, buttonHeight, {183,255,0}, "", false, metro18)
					end
			...
			y2 = drawDataRow(x2, y2, dataRowHeight, 1, "Fizetés: ", "#32b3ef" .. thisRanks[member.rank].pay .. " #eaeaeaFt", gtaFont2, 0.5, x, x + listWidthOrig - buttonWidth)
		...
		for i = 1, renderData.visibleVehicles do
			...
			if isElement(thisVehicle) then
				...
				if datas then
					...
					dxDrawText(exports.nlrp_vehs:getVehicleName(thisVehicle), x + offsetX, rowY, 0, rowY + listRowHeight, tocolor(255, 255, 255, 255 * alphaMultipler), 0.75, Roboto16, "left", "center")
	...
	if selectedTab == 5 then
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			...
			drawButtonWithPrompt("groupDesc", "Mentés", x, y + descHeight, listWidth, respc(35), {183,255,0})
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			...
			y = y + respc(5)
			drawDataRow(x, y, rowHeight, 1, "Egyenleg: ", "#32b3ef" .. thousandsStepper(thisGroup.balance, ",") .. " #eaeaeaFt", gtaFont2, 0.5)
			...
			elseif errorToDisplay == "groupBalance" and string.len(errorText) > 0 then
				...
				drawButton("errorOk", "OK", getOutX, y, acceptWidth*2 + respc(5), rowHeight, {183,255,0})
			else
				...
				drawButton("groupBalance:putBack", "+ Berakás", putBackX, y, acceptWidth, rowHeight, {183,255,0})
				drawButton("groupBalance:getOut", "- Kivétel", getOutX, y, acceptWidth, rowHeight, {183,255,0})
			end
		...
		if standingColShape then
			...
			drawButton("setDutySkin", "Módosítás", x + listWidth - respc(150), y, respc(150), rowHeight, {183,255,0})
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			...
			--drawButtonSlider("swatDutyMode", thisGroup.swatDutyMode, x + listWidth - 64, y, rowHeight, {50, 50, 50, 255}, {183,255,0, 255})
		end
	end
...
function drawGroupSelector(isAdminMode)
	...
	for i = 1, renderData.visibleGroups do
		...
		if thisGroup and selectedGroup == thisGroup.groupID then
			colorOfRow = tocolor(183,255,0, 150 * alphaAnim[3])
	...
	if isAdminMode then
		if availableGroupsCount > renderData.visibleGroups then
			drawScrollbar("groups", x/2 - 12, y, 12, headerHeight * renderData.visibleGroups, renderData.visibleGroups, availableGroupsCount)
		end
	elseif playerGroupsCount > renderData.visibleGroups then
		drawScrollbar("groups", x/2 - 12, y, 12, headerHeight * renderData.visibleGroups, renderData.visibleGroups, playerGroupsCount)
	end
	...
	if activeButton == "selectGroup:next" then
		buttonColor = {processColorSwitchEffect("selectGroup:next", {183,255,0, 175})}
	else
		buttonColor = {processColorSwitchEffect("selectGroup:next", {183,255,0, 125})}
	end
	...
	if button == "left" and state == "down" then
		...
		if activeButton then
			...
			if selected[1] == "selectDropdownItem" then
				...
				renderData.activeDropDown = false
				playSound(":nlrp_assets/audio/interface/10.ogg")
			else
				renderData.activeDropDown = false
			end
			...
			if selected[1] == "setDutySkin" then
				...
				togglePanel(false)
				playSound(":nlrp_assets/audio/admin/notify.ogg")
			...
			elseif selected[1] == "deleteGroup" then
				showError("deleteGroup", "#d75959Biztos, hogy törlöd ezt a frakciót?", ":nlrp_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "acceptItemSettings" and renderData.selectedDutyItem then
				...
				showError("acceptItemSettings", "#7cc576Item adatok sikeresen megváltoztatva.", ":nlrp_assets/audio/admin/notify.ogg")
			...
			elseif selected[1] == "addItem" then
				...
				table.insert(renderData.dutyItems, {
					itemID,
					exports.nlrp_inventory:getItemName(itemID),
					1,
					""
				})
			...
			elseif selected[1] == "addDutyPos" then
				if #renderData.dutyPoses < 8 then
					...
					exports.nlrp_hud:showInfobox("error", "Elérted a maximális duty pozíció számát!")
				end
			elseif selected[1] == "addDutySkin" then
				if selected[2] == "input" then
					...
					if skinID < 0 or skinID > 312 then
						exports.nlrp_hud:showInfobox("error", "Érvénytelen skin ID! Minimum 0 és maximum 312 lehet.")
					else
			...
			elseif selected[1] == "addNewMember" then
				if selected[2] == "input" then
					...
					if string.len(inputText) < 1 then
						...
						playSound(":nlrp_assets/audio/admin/error.ogg")
					else
						local found = false
			...
			elseif selected[1] == "rankPayManaging" then
				if selected[2] == "input" then
					...
					elseif tonumber(inputValue) < 0 then
						...
						playSound(":nlrp_assets/audio/admin/error.ogg")
					elseif tonumber(inputValue) > 300000 then
						errorText = "#d75959Maximum " .. thousandsStepper(300000, ",") .. "!"
						playSound(":nlrp_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "rankRemove" and not errorToDisplay then
				showError(2, "Biztos, hogy törlöd a rangot?", ":nlrp_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "rankRemove_promptAccept" then
				...
				hideError()
				playSound(":nlrp_assets/audio/admin/inmsg.ogg")
			elseif selected[1] == "promptDecline" then
				hideError()
				playSound(":nlrp_assets/audio/admin/outmsg.ogg")
			elseif selected[1] == "newRank" and selectedGroup and #availableGroups[selectedGroup].ranks <= maxGroupRank then
				if getTickCount() - lastNewRankTime >= 1000 then
					...
					exports.nlrp_hud:showInfobox("error", "Ne ilyen gyorsan!")
				end
			elseif selected[1] == "addLeaderToMember" and not errorToDisplay then
				showError(selected[1], "Biztos, hogy hozzáadod a Leaderekhez?", ":nlrp_assets/audio/admin/error.ogg")
			elseif selected[1] == "removeLeaderFromMember" and not errorToDisplay then
				showError(selected[1], "Biztos, hogy eltávolítod a Leaderek közül?", ":nlrp_assets/audio/admin/error.ogg")
			elseif selected[1] == "addLeaderToMember_promptAccept" and selectedMember and groupMembers[selectedGroup][selectedMember] then
				if getTickCount() - lastSetLeaderTick >= 2000 then
					...
					exports.nlrp_hud:showInfobox("error", "Maximum 2 másodpercenként módosíthatod!")
				end
			...
			elseif selected[1] == "removeMemberFromGroup" and not errorToDisplay then
				showError(selected[1], "Biztos, hogy eltávolítod a csoportból?", ":nlrp_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "groupDesc" then
				if selected[2] == "input" then
					...
					showError(selected[1], "Biztos, hogy felülírod a jelenlegi megjegyzést?", ":nlrp_assets/audio/admin/error.ogg")
				end
			...
			elseif selected[1] == "groupBalance" then
				...
				elseif selected[2] == "putBack" or selected[2] == "getOut" then
					...
					elseif tonumber(inputValue) < 1000 then
						errorText = "#d75959Minimum " .. thousandsStepper(1000, ",") .. "Ft-t kezelhetsz!"
						playSound(":nlrp_assets/audio/admin/error.ogg")
					else
...
addEventHandler("setErrorDisplay", getRootElement(),
	function (soundType, display, text)
		showError(display, text, ":nlrp_assets/audio/admin/" .. soundType .. ".ogg")
...
function drawButtonWithPrompt(key, label, x, y, w, h, activeColor, icon, labelFont, iconFont, labelScale, iconScale)
	if errorToDisplay == key and string.len(errorText) > 0 then
		...
		drawButton("promptDecline", "Nem", x2, y, acceptWidth, h, {183,255,0})
...
function drawInputWithErrorDisplay(key, label, x, y, w, h, activeColor, icon, acceptLabel, labelFont, iconFont, labelScale, iconScale)
	if errorToDisplay == key and string.len(errorText) > 0 then
		...
		drawButton("errorOk", "OK", x + inputWidth, y, acceptWidth, h, {183,255,0}, icon)
	else
...
function processGroupCreation(state)
	...
	if utf8.len(groupName) < 1 then
		exports.nlrp_hud:showInfobox("error", "A csoport nevének legalább 1 karakterből kell állnia!")
		return
	end

	if utf8.len(groupPrefix) > 10 then
		exports.nlrp_hud:showInfobox("error", "A csoport prefix nem lehet hosszabb 10 karaktertől!")
		return
	end

	if groupArmor > 100 then
		exports.nlrp_hud:showInfobox("error", "A duty armor nem lehet nagyobb 100-nál!")
		return
	end

	if not groupRadio or groupRadio <= 0 then
		exports.nlrp_hud:showInfobox("error", "A védett rádió frekvenciának nagyobbnak kell lennie nullánál!")
		return
	else
		...
		if tuneRadioExists then
			exports.nlrp_hud:showInfobox("error", "A kiválasztott rádió frekvenciát már használja egy másik frakció. (" .. tuneRadioExists.name .. ")")
			return
		end
	end
...
function setGroupCreationValues(data)
	if not data then
		...
		for i = 1, #data.duty.items do
			table.insert(renderData.dutyItems, {
				data.duty.items[i][1],
				exports.nlrp_inventory:getItemName(data.duty.items[i][1]),
				data.duty.items[i][2],
...
function toggleDutySkinSelect(state, groupID)
	if dutySkinSelectState ~= state then
		if state then
			...
			exports.nlrp_controls:toggleControl("all", false)
			...
			skinForGroup = false
			exports.nlrp_controls:toggleControl("all", true)
...
function processDutySkinSelection()
	...
	exports.nlrp_hud:showInfobox("success", "Sikeresen megváltoztattad a csoporthoz tartozó szolgálati öltözékedet.")
end
			...
			if isElement(colShape) then
				...
				if screenX and screenY then
					if 100 >= getDistanceBetweenPoints3D(colShapeX, colShapeY, colShapeZ, localX, localY, localZ) then
						dxDrawMaterialLine3D(colShapeX, colShapeY, colShapeZ + 0.5, colShapeX, colShapeY, colShapeZ - 0.5, dutyTexture, 1, tocolor(183,255,0, 255))
