		...
		if groupId and amount then
			...
			if currentBalance < 0 then
				...
				if amount > 0 and currentMoney < amount then
					...
					if amount < 0 then
						exports.nle_discord:sendDiscordMessage("frakciolog","```" .. getElementData(source,"char.Name"):gsub("_","") .. " [" .. getElementData(source,"acc.ID") .. "] levett " .. format(math.abs(amount)) .. " Ft összeget a(z) '" .. availableGroups[groupId].name .. "' frakció számlájáról!```")
					else
						exports.nle_discord:sendDiscordMessage("frakciolog","```" .. getElementData(source,"char.Name"):gsub("_","") .. " [" .. getElementData(source,"acc.ID") .. "] betett " .. format(math.abs(amount)) .. " Ft összeget a(z) '" .. availableGroups[groupId].name .. "' frakció számlájára!```")
					end
	...
	function (datas, mainLeaderElement)
		...
		dbQuery(
			function (qh, creator)
				...
				exports.nlrp_hud:showInfobox(creator, "success", "Frakció sikeresen létrehozva!")
	...
	function (groupID, datas, mainLeaderElement, oldLeaderElement)
		...
		dbQuery(
			function (qh, creator)
				...
				exports.nlrp_hud:showInfobox(creator, "success", "Frakció sikeresen módosítva!")
...
addEventHandler("deleteGroup", getRootElement(),
	function (groupID)
		if groupID then
			...
			exports.nlrp_vehicles:deleteGroupVehicles(groupID)
			...
			exports.nlrp_hud:showInfobox(source, "success", "Frakció sikeresen törölve!")
		end
	end
)
	...
	function (groupId)
		if groupId and availableGroups[groupId] then
			...
			if group then
				...
				if characterId then
					exports.nlrp_inventory:removePlayerDutyItems(source)
					...
					if not getElementData(source, "groupDuty") then
						...
						for k,v in ipairs(group.duty.items) do
							...
							if v[1] == 86 then -- jelvény
								...
								if not exports.nlrp_inventory:hasItemWithData(source, v[1], "data2", badgeId) then
									data1 = group.prefix
									data2 = badgeId
								else
									give = false
							...
							if give then
								exports.nlrp_inventory:giveItem(source, v[1], v[2], data1, "duty", data3)
							end
						end

						setElementData(source, "groupDuty", true)
						setElementData(source, "groupDutyID", groupId)
						exports.nlrp_hud:showInfobox(source, "success", "Sikeresen felvetted a szolgálatot.")
					else
						...
						setElementData(source, "groupDutyID", false)
						exports.nlrp_hud:showInfobox(source, "success", "Sikeresen leadtad a szolgálatot.")
					end
				end
				exports.nlrp_fixpoints:checkFixPoints()
			end
		end
	end
)
	...
	function (oldModel, newModel)
		if getElementType(source) == "player" and not getElementData(source, "adminDuty") then
			...
			if not modelEnabled then
				...
				exports.nlrp_hud:showInfobox(source, "error", "Ez a skint csak az adott frakció tagjai hordhatják!")
			end
		end
	end
)
