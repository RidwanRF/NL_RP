...
function outputErrorText(text, element)
	if text and isElement(element) then
		outputChatBox("#dc143c>> NextLevel: #ffffff" .. text, element, 0, 0, 0, true)
...
function outputInfoText(text, element)
	if text and isElement(element) then
		outputChatBox("#4aabd0>> NextLevel: #ffffff" .. text, element, 0, 0, 0, true)
	end
end
...
addCommandHandler("id",
	function (sourcePlayer, commandName, targetPlayer)
		if not targetPlayer then
			outputChatBox("#b7ff00>> Használat: #ffffff/" .. commandName .. " [Játékos név / ID]", sourcePlayer, 0, 0, 0, true)
		else
