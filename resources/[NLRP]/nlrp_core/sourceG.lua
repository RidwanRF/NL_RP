	...
	if not matchPlayer or not isElement(matchPlayer) then
		if isElement(player) then
			if #candidates == 0 then
				outputChatBox("#b7ff00>> NextLevel: #FFFFFFA kiválasztott játékos #d75959nem található.", player, 255, 255, 255, true)
			else
				outputChatBox("#b7ff00>> NextLevel: #d75959" .. #candidates .. " #FFFFFFjátékos található ezzel a névrészlettel:", player, 255, 255, 255, true)
			
				for k = 1, #candidates do
					local v = candidates[k]

					if isElement(v) then
						outputChatBox("#cdcdcd>> #b7ff00(" .. tostring(getElementData(v, "playerID")) .. ") #fffffff" .. string.gsub(getPlayerName(v), "_", " "), player, 255, 255, 255, true)
					end
				end
			end
		end
		...
		if getElementData(matchPlayer, "loggedIn") then
			...
			outputChatBox("#b7ff00>> NextLevel: #FFFFFFA kiválasztott játékos #d75959nincs bejelentkezve.", player, 255, 255, 255, true)
			return false
		end
	end
end
...
function takeMoney(player, amount, saveLog)
	if amount then
		if getElementData(player, "char.Money") - tonumber(amount) >= 0 then
			...
			if saveLog then
				exports.nlrp_logs:logEconomy(player, "take:" .. saveLog, -amount)
			end
...
function takeMoneyEx(player, amount, saveLog)
	if amount then
		...
		if saveLog then
			exports.nlrp_logs:logEconomy(player, "takeex:" .. saveLog, -amount)
		end
	end
end
...
function giveMoney(player, amount, saveLog)
	if amount then
		...
		if saveLog then
			exports.nlrp_logs:logEconomy(player, "give:" .. saveLog, amount)
        end
...
local serverTags = {
	...
	["usage"] = "#b7ff00>> Használat: #ffffff",
	["sarp"] = "#b7ff00>> NextLevel: #ffffff",
