	...
	function()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 or getElementData(localPlayer, "acc.helperLevel") >= 1 then
			if not panelState then
				...
				RobotoFont = dxCreateFont(":nlrp_assets/fonts/Roboto-Regular.ttf", 16, false, "antialiased")
				RobotoLighter = dxCreateFont(":nlrp_assets/fonts/Roboto-Light.ttf", 12, false, "cleartype")
...
function renderTheHelpPanel()
	if panelState then
		...
		dxDrawImage(math.floor(panelPosX + 3), math.floor(panelPosY + 3), 24, 24, ":nlrp_hud/files/logo.png", 0, 0, 0, tocolor(50, 179, 239))
		...
		for i = 1, maxVisibleCommand do
			...
			if data then
				if data[3] < 0 then
					dxDrawText("[AS LVL:" .. math.abs(data[3]) .. "] #b7ff00/" .. data[1] .. "#ffffff - " .. data[2], x + 5, y, 0, y + sy, tocolor(255, 255, 255), 0.75, RobotoFont, "left", "center", false, false, false, true)
				else
					dxDrawText("[LVL:" .. data[3] .. "] #b7ff00/" .. data[1] .. "#ffffff - " .. data[2], x + 5, y, 0, y + sy, tocolor(255, 255, 255), 0.75, RobotoFont, "left", "center", false, false, false, true)
				end
...
local logoTexture = ":nlrp_accounts/files/logo.png"
...
function createJailFont(destroy)
	...
	if not destroy then
		RobotoFont = dxCreateFont(":nlrp_assets/fonts/Roboto-Regular.ttf", 17.5, false, "antialiased")
	end
end
	...
	function(startedRes)
		if startedRes == getThisResource() then
			...
			if textures and getResourceState(textures) == "running" then
				...
				logoTexture = dxCreateTexture(":nlrp_accounts/files/logo.png")
			end
...
function renderJail()
	...
	dxDrawText("Hátralévő idő: #b7ff00" .. adminJailTime .. " perc", 0, screenY - 128, screenX, screenY - 64, tocolor(255, 255, 255), 1, RobotoFont, "center", "center", false, false, false, true)
...
addCommandHandler("jailinfo",
	function()
		...
		if jailData ~= 0 then
			...
			outputChatBox(" - Időpont: #d75959" .. exports.nlrp_core:formatDate("Y/m/d h:i:s", "'", tostring(datas[1])), 255, 255, 255, true)
		else
			exports.nlrp_hud:showAlert("error", "Nem vagy adminbörtönben!")
		end
	end)