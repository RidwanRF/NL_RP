...
local util = exports["nlrp_core"]

local dxFont = dxCreateFont(":nlrp_assets/fonts/CenturyGothicBold.ttf", util:respc(14), false, "cleartype")
local dxFont2 = dxCreateFont(":nlrp_assets/fonts/BebasNeue.otf", util:respc(18), false, "cleartype")
local dxFont3 = dxCreateFont(":nlrp_assets/fonts/CenturyGothicBold.ttf", util:respc(12), false, "cleartype")
local dxFont4 = dxCreateFont(":nlrp_assets/fonts/CenturyGothicBold.ttf", util:respc(11), false, "cleartype")
...
function showCarshopPanel(state)
    ...
    if state then
        if isElement(car) then
            ...
            exports["nlrp_3dview"]:processVehiclePreview(car, 0, 0, 0, 0, false)
            exports["nlrp_3dview"]:setVehicleAlpha(254)
        end
        addEventHandler("onClientRender", root, renderCarshopPanel)
        exports["nlrp_3dview"]:setPreviewVehicleModel(vehiclesInShop[shopType][markak[selectedBrand]][currentVehicle][1])
    else
...
function renderCarshopPanel()
    if showVehicleshopPanel then
        ...
        dxDrawImage(buyButtonX + util:respc(5), buyButtonY + util:respc(5), buyButtonH - util:respc(10), buyButtonH - util:respc(10), ":nlrp_assets/images/carshop/coin.png",0,0,0, tocolor(255,255,255) )
        ...
        dxDrawImage(closeButtonX + util:respc(5), closeButtonY + util:respc(5), closeButtonH - util:respc(10), closeButtonH - util:respc(10), ":nlrp_assets/images/carshop/close.png",0,0,0, tocolor(255,255,255) )
        ...
        exports["nlrp_3dview"]:setVehicleProjection(bckX, bckY, bckW, bckH, 255)
    ...
    if state == "down" then
        if showVehicleshopPanel then
            ...
            if cursorInBox(buyButtonX, buyButtonY, buyButtonW, buyButtonH) then
                ...
                triggerServerEvent("nlrp_carshopS:buyVehicle", localPlayer, localPlayer, id, cost)
            end
            ...
            if clickedElement and isElement(clickedElement) then
                if getElementData(clickedElement, "ped.type") == 3 then
                    if exports.nlrp_core:inDistance3D(getLocalPlayer(), clickedElement, 2) then
