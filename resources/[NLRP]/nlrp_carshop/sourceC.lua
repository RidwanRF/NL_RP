...
VehicleShop.FactionSelectPanel = function()
    ...
    for k, v in pairs(selectButtons) do
        ...
        dxDrawMetroButton(v[1], v[2], buttonX, buttonY, selectButtonW, selectButtonH, {183,255,0}, nil, fonts.Roboto14, nil, 1, nil)
    end
    ...
    for i = 1, 8 do
        ...
        if selectedFaction[1] == i then
            dxDrawRectangle(rowX, rowY, rowW, rowH, tocolor(183,255,0, 255))
        elseif i % 2 == 0 then
...
VehicleShop.ShopPanel = function()
    ...
    --dxDrawImage(shopPanelX + 5, shopPanelY + 5, respc(403 * 0.075), respc(459 * 0.075), ":nlrp_assets/images/sarplogo_big.png", 0, 0, 0, tocolor(183,255,0))
    if cursorInBox(shopPanelX + shopPanelW - respc(24) - 10, shopPanelY + 5 + (respc(34.425) - respc(24)) / 2, respc(24), respc(24)) then
        dxDrawImage(shopPanelX + shopPanelW - respc(24) - 10, shopPanelY + 5 + (respc(34.425) - respc(24)) / 2, respc(24), respc(24), ":nlrp_assets/images/cross_x.png", 0, 0, 0, tocolor(215, 89, 89))
    else
        dxDrawImage(shopPanelX + shopPanelW - respc(24) - 10, shopPanelY + 5 + (respc(34.425) - respc(24)) / 2, respc(24), respc(24), ":nlrp_assets/images/cross_x.png", 0, 0, 0, tocolor(255, 255, 255))
    end
    ...
    if vehiclesInShop[selectedBrand[2]] then
        ...
        exports["nlrp_3dview"]:setVehicleProjection(vehicleX, vehicleY, vehicleW, vehicleH, 255)

        dxDrawText(exports.nlrp_vehs:getVehicleNameFromModel(vehiclesInShop[selectedBrand[2]][currentVehicle][1]), vehicleX, vehicleY + vehicleH + respc(20), vehicleX + vehicleW, vehicleH, tocolor(255, 255, 255), 1, fonts.Roboto16, "center")
        
		if vehiclesInShop[selectedBrand[2]][currentVehicle][4] then
			local limitedBuyed = 0
			for k, v in ipairs(getElementsByType("vehicle")) do
				if getElementModel(v) == vehiclesInShop[selectedBrand[2]][currentVehicle][1] and getElementData(v, "vehicle.carshop") == 1 then
					limitedBuyed = limitedBuyed + 1
				end
			end
			back = vehiclesInShop[selectedBrand[2]][currentVehicle][4] - limitedBuyed
			dxDrawText("Ár: " .. vehiclesInShop[selectedBrand[2]][currentVehicle][2] .. "Ft - Még "..back.." db vásárolható", vehicleX, vehicleY + vehicleH + respc(20) + dxGetFontHeight(1, fonts.Roboto16), vehicleX + vehicleW, vehicleH, tocolor(255, 255, 255), 1, fonts.Roboto14, "center")
		else
			dxDrawText("Ár: " .. vehiclesInShop[selectedBrand[2]][currentVehicle][2] .. "Ft", vehicleX, vehicleY + vehicleH + respc(20) + dxGetFontHeight(1, fonts.Roboto16), vehicleX + vehicleW, vehicleH, tocolor(255, 255, 255), 1, fonts.Roboto14, "center")
		end
        dxDrawMetroButton("purchase:buy", "Megvesz", buttonX, buttonY, buttonW, buttonH, {183,255,0}, nil, fonts.Roboto14, nil, 1, nil)

        if cursorIsMoving then
            exports["nlrp_3dview"]:rotateVehicle(cursorX)
...
VehicleShop.ChangeVehicle = function(vehID)
    print(vehID)
    exports["nlrp_3dview"]:processVehiclePreview()
    ...
    exports["nlrp_3dview"]:processVehiclePreview(vehicleElement, 0, 0, 0, 0, false) -- utolsó érték a karosszéria sérülések ki- és bekapcsolása, a többi nulla, a render majd úgy is beállítja
    exports["nlrp_3dview"]:setVehicleAlpha(0)
end

VehicleShop.ProcessPlayerGroups = function()
    local factions = exports.nlrp_groups:getPlayerGroups(localPlayer)
    playerGroups = {}

    for id, group in pairs(factions) do
        if group[3] == "Y" then
            table.insert(playerGroups, {id, exports.nlrp_groups:getGroupName(id)})
        end
    end
end
    ...
    if button == "left" and state == "down" then
        if page == "select" then
            ...
            if activeButton == "select:faction" then
                ...
                elseif #selectedFaction == 0 then
                    exports.nlrp_alert:showAlert("error", "A tovább lépéshez válasz ki egy frakciót!")
                end
			      end
            if activeButton == "select:private" then
                removeEventHandler("onClientRender", root, VehicleShop.FactionSelectPanel)
                addEventHandler("onClientRender", root, VehicleShop.ShopPanel)
                selectedFaction = {}
                page = "purchase"
        ...
        elseif page == "purchase" then
            ...
            if activeButton == "purchase:buy" then
                ...
                if selectedFaction[1] and selectedFaction[2] then
					if selectedVehicle[4] then
						local limitedBuyed = 0
						for k, v in ipairs(getElementsByType("vehicle")) do
							if getElementModel(v) == selectedVehicle[1] and getElementData(v, "vehicle.carshop") == 1 then
								limitedBuyed = limitedBuyed + 1
							end
						end
						if limitedBuyed > selectedVehicle[4] or limitedBuyed == selectedVehicle[4] then
							outputChatBox("Sajnáljuk, ebből az autóból már mind elfogyott.")
						else
							triggerServerEvent("nlrp_carshopS:buyVehicle", localPlayer, localPlayer, selectedVehicle[1], selectedVehicle[2], selectedFaction[2][1])
						end
					else
						triggerServerEvent("nlrp_carshopS:buyVehicle", localPlayer, localPlayer, selectedVehicle[1], selectedVehicle[2], selectedFaction[2][1])
					end
                    ...
                    if selectedVehicle[3] then
                        exports.nlrp_alert:showAlert("error", "Ezt a járművet csak frakció vezetők vehetik meg!")
                        return
                    end
					...
					if selectedVehicle[4] then
						...
						if limitedBuyed > selectedVehicle[4] or limitedBuyed == selectedVehicle[4] then
							...
							triggerServerEvent("nlrp_carshopS:buyVehicle", localPlayer, localPlayer, selectedVehicle[1], selectedVehicle[2])
						end
					else
						triggerServerEvent("nlrp_carshopS:buyVehicle", localPlayer, localPlayer, selectedVehicle[1], selectedVehicle[2])
					end
