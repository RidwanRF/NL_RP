--HACK PROTECT ~ 2017 Xenius, Mark
local events = {}

local function getKey()
	return exports.ex_protect:getKey()
end
			...
			f = function( ... )
				if client and source then
					if not (source == client or source == resourceRoot) then
						local ac = getResourceFromName("ex_anticheat");
						if ac and getResourceState(ac) == "running" then
							exports.ex_anticheat:reportTrigger(eventName, client, getResourceName(getThisResource()));
						end
						return false;
					end
				end
